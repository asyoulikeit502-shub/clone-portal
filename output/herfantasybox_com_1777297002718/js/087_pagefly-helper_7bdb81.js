let _pf_pageflyHelperContentLoaded = new CustomEvent("PageFlyHelperContentLoaded", {
  ...
  _pf_initZoomImage = t => {
    ...
    if (_) {
      ...
      i.className = "zoom-image", i.style.cssText = `
		...
		position: absolute;
		top: 0;
	`, !!t.querySelector("div.zoom-image") || t.insertBefore(i, t.firstElementChild.nextSibling), t.addEventListener("mouseenter", function() {
        n.style.display = "none", i.style.backgroundSize = "150%", i.style.width = "100%", i.style.height = "100%"
      }), t.addEventListener("mousemove", function(e) {
        var t = this.getBoundingClientRect(),
          n = e.clientX - t.left,
          e = e.clientY - t.top,
          n = Math.round(100 / (t.width / n)),
          t = Math.round(100 / (t.height / e));
        i.style.backgroundPosition = n + `% ${t}%`
      }), t.addEventListener("mouseleave", function() {
        "none" !== t.querySelector("div.zoom-image").style.display && (n.style.display = "inline"), i.style.backgroundSize = "contain", i.style.backgroundPosition = "center", i.style.width = e.width + "px", i.style.height = e.height + "px"
      })
    }
  },
  _pf_initZoomImage2 = r => {
    if (r) {
      ...
      if (o) {
        ...
        if (_) {
          let i = () => "image" === r.getAttribute("data-media-type"),
            e = () => {
              i() && (o.style.opacity = "0", _.style.backgroundImage = `url("${o.currentSrc}")`, _.style.backgroundRepeat = "no-repeat", _.style.backgroundSize = "200%")
            },
            t = e => {
              var t, n;
              i() && (t = e.currentTarget, n = e.offsetX || e.touches?.[0].pageX || 0, e = e.offsetY || e.touches?.[0].pageY || 0, n = n / t.offsetWidth * 100, t.style.backgroundPosition = n + `% ${e/t.offsetHeight*100}%`)
            },
            n = () => {
              i() && (o.style.opacity = "1", _.style.backgroundImage = "")
            };
          return _.addEventListener("mouseenter", e), _.addEventListener("mousemove", t), _.addEventListener("mouseleave", n), () => {
            _.removeEventListener("mouseenter", e), _.removeEventListener("mousemove", t), _.removeEventListener("mouseleave", n)
          }
        }
      }
    }
  ...
  _pf_truncateText = (t, o, r) => {
    if (o) {
      ...
      let e = t,
        i = t.replace(/<[^>]*>/g, " ").trim(),
        _ = (l && (d = new RegExp(`(${a})`, "g"), e = e.replace(d, " $1"), i = i.replace(d, " $1")), i = i.match(/\S+/g), []);
      if (i && i.length > o) {
        var p = e.replace(/</g, " <").replace(/>/g, "> ").split(/\s/);
        let n = 0;
        for (let t = 0; t < p.length && (n !== o && i.length); t++) {
          let e = p[t];
          var f, c = e.replace(/<[^>]*>/g, "").replace(/^[^>]*>/g, "").replace(/<[^>]*$/g, "");
          i[0] === c && (f = i.splice(0, 1)[0], c.match(/^&[a-z0-9]+;$/i) || !(1 < c.length || c.match(/[a-z0-9]/i) || c.match(s)) || ["", " ", "`", "-", "=", "~", "!", "@", "#", "$", "%", "^", "&", "*", "(", ")", "_", "+", "[", "]", "\\", ";", "'", ",", ".", "/", "{", "}", "|", ":", '"', "<", ">", "?"].includes(c) || (n++, r && n === o && (e = e.replace(f, f + "&hellip;")))), _.push(e)
        }
        _ = _.join(" ").replace(/ </g, "<").replace(/> /g, ">");
        var d = (_ = l ? _.replace(new RegExp(` (${a})`, "g"), "$1") : _).match(/<[^>]*>/g);
        if (d) {
          let n = [];
          d.forEach(t => {
            if (!t.match(/\/>$/) && !t.match(/<br\s*(((\w|[.-])+="[^"]*"\s*|\s+(\w)*))*\/?>/)) {
              let e = t.match(/^<([a-z0-9\-_]+)/i);
              return e ? n.push(e[1].toLowerCase()) : (e = t.match(/^<\/([a-z0-9\-_]+)>/i)) && e[1].toLowerCase() === n[n.length - 1] ? n.pop() : void 0
            }
          }), n.length && n.reverse().forEach(e => {
            _ += `</${e}>`
          })
        }
        return _
  ...
  _pf_modalDataCss = '.pf-m{display:none;position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:99999;max-height:min(calc(9/16*100vw), calc(100% - 6px - 2em));-webkit-overflow-scrolling:touch;overflow:hidden;background:0 0;border:none;margin:0;padding:0;flex-direction:column;align-items:flex-end}.pf-m-overlay{position:fixed !important;inset:0 !important;z-index:99998 !important;display:block !important;width:100vw !important;height:100vh !important;background:rgba(0,0,0,.9) !important;opacity:1 !important}.pf-m iframe{border:0;}.pf-m>img{max-width:90vw;max-height:90vh;vertical-align:middle}.pf-close-btn{height:16px;background:none;border:none;margin-bottom:8px;padding:0;cursor:pointer}.pf-close-btn:focus-visible{box-shadow:none;}svg{pointer-events:none}[data-action="modal"],[data-action="popup-element"]{cursor:pointer}',
...
function _pf_mergeDeep(e, t) {
  ...
  for (n of Object.keys(t)) t[n] instanceof Object && !Array.isArray(t[n]) && Object.assign(t[n], _pf_mergeDeep(i[n], t[n]));
...
async function _pf_getHttpRequest(i) {
  return new Promise((e, t) => {
    let n = new XMLHttpRequest;
    n.open("GET", i, !0), n.onload = () => 200 === n.status ? e(n.response) : t(Error(n.statusText)), n.onerror = e => t(Error("Network Error: " + e)), n.send()
  })
}
...
function _pf_hideElementIfNotFound(t, n) {
  var i = t.length,
    _ = n.length;
  if (_ < i)
    for (let e = _; e < i; e++) t[e].style.visibility = "hidden", t[e].style.height = "0", t[e].style.overflow = "hidden";
  if (_ === i)
    for (let e = 0; e < i; e++) t[e].style.visibility = t[e].style.visibility || "visible", t[e].style.height = n[e].style.height || "unset", t[e].style.overflow = t[e].style.overflow || "auto"
}

function _pf_collectionHas(t, n) {
...
function _pf_findParentBySelector(e, t) {
  ...
  let i = e.parentNode;
  for (; i && !_pf_collectionHas(n, i);) i = i.parentNode;
  return i
}
...
function _pf_loadJS(i, _ = document.body, o = !0, r = !1) {
  return new Promise((e, t) => {
    var n = _.ownerDocument;
    if (n.querySelector(`script[src="${i}"]`)) return e(!0);
    n = n.createElement("script");
    n.src = i, n.async = o, n.defer = r, n.onload = e, n.onerror = t, _.appendChild(n)
  })
}
...
function _pf_uid(e) {
  let t = "",
    n = e || 11;
  for (; n--;) t += _pf_HEX[36 * Math.random() | 0];
  return t
}
...
function _pf_uuid() {
  ...
  let e = 0,
    t, n = "";
  for (; e < 16; e++) t = _pf_BUFFER[e + _pf_IDX2], 6 == e ? n += _pf_HEX2[15 & t | 64] : 8 == e ? n += _pf_HEX2[63 & t | 128] : n += _pf_HEX2[t], 1 & e && 1 < e && e < 11 && (n += "-");
  return _pf_IDX2++, n
}
async function _pf_loadScript({
  ...
  Object.assign(i, {
    src: n,
    ...t
  }), i.defer && i.setAttribute("defer", "defer");
  t = new Promise((e, t) => {
    i.onload = () => {
      console.log("Script loaded successfully:", i), e(i)
    }, i.onerror = e => t(new Error(`Failed to load script: ${n}: ` + e))
  });
  e.body.appendChild(i), await t
}
...
function _pf_processChunk(e) {
  var {
    ...
    chunkSize: _,
    method: o
  } = e, r = (r = t + _) > n.length ? n.length : r;
  for (let e = t; e < r; e++) i(n[e], e);
  r < n.length && (e.start = r, o(() => _pf_processChunk(e)))
}

function _pf_executeChunk(e, t, n = 10) {
  let i = e => setTimeout(e);
  window.requestAnimationFrame && (i = e => requestAnimationFrame(e)), _pf_processChunk({
    start: 0,
    list: e,
    callback: t,
    chunkSize: n,
    method: i = window.requestIdleCallback ? e => requestIdleCallback(e) : i
  })
}
...
function _pf_setAttributes(e, t) {
  for (var n in t) Object.hasOwn(t, n) && e.setAttribute(n, t[n])
}

function _pf_getSessionKey() {
  var e = void 0 !== _pf_pfSetting ? _pf_pfSetting.shopDomain : "";
  return e ? "PF_SESSION:" + e : "PF_SESSION_STATE"
}

function _pf_getWebPixelPageContext(e) {
  var t = {
    shopDomain: _pf_pfSetting.shopDomain,
    pageId: _pf_pfPageSetting.pageId,
    pageTitle: _pf_pfPageSetting.pageTitle,
    pageType: _pf_pfPageSetting.pageType
  };
  return e?.product ? Object.assign({}, t, {
    pf_product_id: e.product.id
  }) : e?.collection ? Object.assign({}, t, {
    pf_collection_id: e.collection.id
  }) : t
}

function _pf_generateSessionId() {
...
function _pf_createNewSession(e) {
  return {
    sessionId: _pf_generateSessionId(),
    sessionStartAt: new Date(e).toISOString()
  }
}

function _pf_normalizeSessionState(e) {
  return {
    sessionId: String(e.sessionId ?? ""),
    lastActivityAt: Number(e.lastActivityAt) || 0,
    sessionStartAt: "string" == typeof e.sessionStartAt ? e.sessionStartAt : "",
    lastEventAt: "number" == typeof e.lastEventAt && 0 < e.lastEventAt ? e.lastEventAt : null
  }
}
async function _pf_getOrRefreshSession() {
  var e, t, n, i, _, o, r = Date.now();
  try {
    return window?.localStorage ? (a = window.localStorage.getItem(_pf_getSessionKey())) ? (n = null != (t = _pf_normalizeSessionState(JSON.parse(a))).lastEventAt ? r - t.lastEventAt : 0, i = {
      ...t,
      lastActivityAt: r,
      lastEventAt: r
    }, window.localStorage.setItem(_pf_getSessionKey(), JSON.stringify(i)), {
      sessionId: t.sessionId,
      sessionStartAt: t.sessionStartAt,
      duration_ms: n
    }) : (o = {
      sessionId: (_ = _pf_createNewSession(r)).sessionId,
      lastActivityAt: r,
      sessionStartAt: _.sessionStartAt,
      lastEventAt: r
    }, window.localStorage.setItem(_pf_getSessionKey(), JSON.stringify(o)), {
      sessionId: _.sessionId,
      sessionStartAt: _.sessionStartAt,
      duration_ms: 0
    }) : {
      sessionId: (e = _pf_createNewSession(r)).sessionId,
      sessionStartAt: e.sessionStartAt,
      duration_ms: 0
    }
  } catch {
    var a = _pf_createNewSession(r);
    return {
      sessionId: a.sessionId,
      sessionStartAt: a.sessionStartAt,
      duration_ms: 0
    }
  }
}
