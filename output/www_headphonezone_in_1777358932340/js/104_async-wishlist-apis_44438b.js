(function() {
  ...
  SwymStorefrontLayoutContext.CustomEvents = {
    ...
    SavedForYouFetched: "SwymSavedForYouFetched"
  }
  ...
  const swymInitWishlistAsyncApi = (swat) => {
    ...
    const SwymWLAsyncApis = {
      ...
      'fetchProductDataForList': async function(list = [], fromCache = false) {
        ...
        list.forEach((item) => {
          if (useCache(item)) {
            const cached = SwymStorefrontLayoutContext.ShopifyProductData[item.epi];
            const cloned = isMultipleWishlistEnabled ? cached : JSON.parse(JSON.stringify(cached));
            resultsMap[item.epi] = Object.assign({}, item, {
              productData: cloned
            });
          } else {
      ...
      'updateList': async function() {
        ...
        try {
          ...
          defaultList = isMultiListEnabled ?
            (lists.find((item) => item.lname !== 'Saved For Me') || lists[0]) :
            lists.find((item) => item.lname?.toLowerCase() === SwymWishlistDefaultName?.toLowerCase()) || lists[0];
    ...
    if (SwymStorefrontLayoutContext?.Settings?.EnableStorefrontLayout) {
      ...
      if (SwymStorefrontLayoutContext?.CustomEvents?.AsyncApisInitialized) {
        const event = new CustomEvent(SwymStorefrontLayoutContext.CustomEvents.AsyncApisInitialized, {
          detail: {
            SwymStorefrontLayoutAPI
          }
        });
        document.dispatchEvent(event);
      } else {
