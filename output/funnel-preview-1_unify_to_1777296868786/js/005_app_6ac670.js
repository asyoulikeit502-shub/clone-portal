...
function getNgAppId() {
  ...
  try {
    return window.UniConfig.ng_app_id;
  } catch (e) {
    return "73da550b-6eec-4247-80f2-92e13f0fc552";
  }
}

/**global:angular**/
var app = angular.module(getNgAppId(), []);
app.run(function($rootScope, $http, $q, $compile, $timeout) {
  // Default parameters
  $rootScope.statesInputFieldShow = false;
  ...
  $rootScope.configData = window.UniConfig || null;
  ...
  $rootScope.validateSliderCaptcha = false;
  $rootScope.hasGoogleCaptcha = false;
  $rootScope.hasSliderCaptcha = false;
  $rootScope.googleCaptchaPublicKey = "6Le4rF8UAAAAAAUNcAJgB6T75T5Fd4r8ekZ0Ws16";
  $rootScope.pival = null;
  $rootScope.controller_name = 'NA';
  $rootScope.isPaypalFlow = false;
  ...
  $rootScope.FullFormData = {
    ...
    expyear: null,
    cvv: '',
    creditCardType: 'online'
  };
  if (document.getElementsByName('agreeCheckbox').length > 0) {
    $rootScope.FullFormData.agreeCheckbox = false;
  }
  if (document.getElementsByName('dynamic_shipping_id').length > 0 && document.getElementById('dynamic_shipping_id').value !== null) {
    $rootScope.FullFormData.dynamic_shipping_id = document.getElementById('dynamic_shipping_id').value;
  }
  ...
  // Custom Mail form validation added
  $rootScope.emailRegex = /^[a-z0-9-_\.]+@[a-z0-9-_]+\.[a-z]{2,}(?:\.[a-z]{2,})?$/;
  $rootScope.telephoneValidRegex = /^(?:(?:\(?(?:00|\+)([1-4]\d\d|[1-9]\d?)\)?)?[\-\.\ \\\/]?)?((?:\(?\d{1,}\)?[\-\.\ \\\/]?){0,})(?:[\-\.\ \\\/]?(?:#|ext\.?|extension|x)[\-\.\ \\\/]?(\d+))?$/i;
  // Custom Mail form validation ends
  $rootScope.SquarePayment = {};
  $rootScope.setNonce = function() {

    if ($rootScope.configData.api_type == 'Square') {
      $rootScope.SquarePayment.nonce = '';
      $rootScope.CheckoutData.nonce = '';
      $rootScope.FullFormData.nonce = '';
    }
  }
  // Get config data
    ...
    return $q(function(resolve, reject) {
      if ($rootScope.configData) {
        ...
        $http({
          ...
          if (!$rootScope.configData.countries) {
            $rootScope.configData.countries = '';
          }
          resolve($rootScope.configData);
          $rootScope.setNonce();
        }).catch(function() {
  ...
  $rootScope.statesExists = function(cid) {
    var obj = {};
    var statesData = [];
    var islands = ["American Samoa", "Virgin Islands of the U.S.", "Guam", "Northern Mariana Islands", "Puerto Rico", "U.S. Virgin Islands", "Minor Outlying Islands", "Bajo Nuevo Bank", "Baker Island", "Howland Island", "Jarvis Island", "Johnston Atoll", "Kingman Reef", "Midway Islands", "Navassa Island", "Palmyra Atoll", "Serranilla Bank", "Wake Island", "Federated States of Micronesia", "Republic of Marshall Islands"];
  ...
  // Show hide state input field
  $rootScope.toggleStatesInputFieldShow = function() {
    if (typeof $rootScope.CheckoutData.billingSameAsShipping !== 'undefined' && $rootScope.CheckoutData.billingSameAsShipping !== null && $rootScope.CheckoutData.billingSameAsShipping != 'yes') {
      if ($rootScope.statesData[0].state_name == "N/A") {
        $rootScope.billingStateInputFieldShow = ($rootScope.CheckoutData.billingCountry) ? true : false;
      } else {
        $rootScope.billingStateInputFieldShow = false;
      }
      $rootScope.CheckoutData.billingState = '';
    } else {
      if ($rootScope.statesData[0].state_name == "N/A") {
        $rootScope.statesInputFieldShow = ($rootScope.FormData.country) ? true : false;
      } else {
        $rootScope.statesInputFieldShow = false;
      }
    }
  ...
  $rootScope.getStates = function() {
    $rootScope.statesData = [];
    var cid;
    if (['', null, false].indexOf($rootScope.FormData.country) === -1) {
      cid = $rootScope.FormData.country + '-';
    } else if (['', null, false].indexOf($rootScope.FullFormData.country) === -1) {
      cid = $rootScope.FullFormData.country + '-';
    }

    $rootScope.statesData = $rootScope.statesExists(cid);
    $rootScope.updateStateTax();
    $rootScope.toggleStatesInputFieldShow();
  };

  $rootScope.fetchGeolocationData = function() {
    if (!$rootScope.configData.enable_geoloacation_data) return;
    $http.post(SITE_BASE_URL + '/ajax/geolocation-data', {
        controller: 'NA'
      })
      .then(function(response) {
        if (response.status !== 200) {
          throw "Unable to fetch geolocation data!";
        }

        if (['US', 'CA'].indexOf(response.data.countryCode) === -1) {
          state_code = response.data.countryCode + "-" + response.data.stateCode;
        } else {
          state_code = response.data.stateCode;
        }

        if ($rootScope.controller_name === 'LandingPageCtrl') {
          $rootScope.FormData.city = response.data.cityCode;
          $rootScope.FormData.country = response.data.countryCode;
          $rootScope.FormData.state = state_code;
        } else {
          $rootScope.FullFormData.city = response.data.cityCode;
          $rootScope.FullFormData.country = response.data.countryCode;
          $rootScope.FullFormData.state = state_code;
        }
        $rootScope.getStates();
      }).catch(function(response) {
  ...
  $rootScope.getBillingStates = function($event) {
    $rootScope.billingStatesData = [];
    var cid = '';

    if (['', null, false].indexOf($rootScope.CheckoutData.billingCountry) === -1) {
      cid = $rootScope.CheckoutData.billingCountry + '-';
    } else if (['', null, false].indexOf($rootScope.FullFormData.billingCountry) === -1) {
      cid = $rootScope.FullFormData.billingCountry + '-';
    }
    $rootScope.billingStatesData = $rootScope.statesExists(cid);
  }
  ...
  $rootScope.errorHandler = function(formFields, formType) {
    return $q(function(resolve, reject) {
      ...
      if (formType == 'coupon') {
        ...
        var alphaNumericSpaceDashValidRegex = /^(?=.{5,12}$)[a-z\d\-_\s]+$/i;
        var telephoneValidRegex = /^(?:(?:\(?(?:00|\+)([1-4]\d\d|[1-9]\d?)\)?)?[\-\.\ \\\/]?)?(?=(?:\D*\d){10,13}\D*$)((?:\(?\d{1,}\)?[\-\.\ \\\/]?){1,})(?:[\-\.\ \\\/]?(?:#|ext\.?|extension|x)[\-\.\ \\\/]?(\d+))?$/i
        var emailValidRegex = /^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,10}(?:\.[a-z]{2})?)$/i;
        ...
        var hasSquareNonce = formFields.hasOwnProperty('nonce') ? true : false;
        var isPaypalFlow = formFields.hasOwnProperty('paypal_payment') ? formFields.paypal_payment : false;
        var ignoreCardFields = hasSquareNonce || isPaypalFlow;

        var ignoreAddressFields = false;
        if (isPaypalFlow && ($rootScope.configData.paypal_express != null && $rootScope.configData.paypal_express.address != false)) {
          ignoreAddressFields = true;
        }
        var isOnlinePayment = formFields.creditCardType == 'offline' ? false : true;

        angular.forEach(formFields, function(fieldValue, fieldKey) {
          if (formType == 'full') {
            if (billingSameAsShipping == 'yes') {
              if ((fieldKey == 'firstName' || fieldKey == 'lastName' || fieldKey == 'address' || fieldKey == 'city' || fieldKey == 'country' || fieldKey == 'state') && (!fieldValue || fieldValue == '') && !ignoreAddressFields) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (fieldKey == 'zip' && (!fieldValue || fieldValue == '' || !fieldValue.match(alphaNumericSpaceDashValidRegex)) && !ignoreAddressFields) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (fieldKey == 'phone' && (!fieldValue || fieldValue == '' || !fieldValue.match(telephoneValidRegex)) && !ignoreAddressFields) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (fieldKey == 'email' && (!fieldValue || fieldValue == '' || !fieldValue.match(emailValidRegex)) && !ignoreAddressFields) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (isOnlinePayment && (((fieldKey == 'cc_type' || fieldKey == 'cc_number' || fieldKey == 'expmonth' || fieldKey == 'expyear' || fieldKey == 'cvv') && (!fieldValue || fieldValue == '')) && !ignoreCardFields)) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if ((document.getElementsByName('agreeCheckbox').length > 0) && fieldKey == 'agreeCheckbox' && !fieldValue) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (isOnlinePayment && (((fieldKey == 'cc_type' || fieldKey == 'cc_number' || fieldKey == 'expmonth' || fieldKey == 'expyear') && (!fieldValue || fieldValue == '')) && !ignoreCardFields)) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (isOnlinePayment && ((fieldKey == 'cvv' && (!fieldValue || fieldValue == '' || !fieldValue.match(cvvValidRegex))) && !ignoreCardFields)) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (isOnlinePayment && ((fieldKey == 'cc_number' && !formFields.cc_type) && !ignoreCardFields)) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (isOnlinePayment && ((fieldKey == 'cc_number' && (!fieldValue || !fieldValue.match($rootScope.cardPatternFullValidRegex[formFields.cc_type]) || !$rootScope.validateBIN(fieldValue)) && !$rootScope.hasThisTestCard(formFields.cc_number)) && !ignoreCardFields)) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (hasSquareNonce && (fieldKey == 'nonce' && (!fieldValue || fieldValue == ''))) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              }
            } else {
              if ((fieldKey == 'firstName' || fieldKey == 'lastName' || fieldKey == 'address' || fieldKey == 'city' || fieldKey == 'country' || fieldKey == 'state' || fieldKey == 'billingFirstName' || fieldKey == 'billingLastName' || fieldKey == 'billingAddress' || fieldKey == 'billingCity' || fieldKey == 'billingCountry' || fieldKey == 'billingState') && (fieldValue == '' || !fieldValue) && !ignoreAddressFields) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if ((fieldKey == 'zip' || fieldKey == 'billingZip') && (!fieldValue || fieldValue == '' || !fieldValue.match(alphaNumericSpaceDashValidRegex)) && !ignoreAddressFields) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (fieldKey == 'phone' && (!fieldValue || fieldValue == '' || !fieldValue.match(telephoneValidRegex)) && !ignoreAddressFields) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (fieldKey == 'email' && (!fieldValue || fieldValue == '' || !fieldValue.match(emailValidRegex)) && !ignoreAddressFields) {
                ...
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (isOnlinePayment && (((fieldKey == 'cc_type' || fieldKey == 'cc_number' || fieldKey == 'expmonth' || fieldKey == 'expyear' || fieldKey == 'billingAgreeCheckbox') && (!fieldValue || fieldValue == '')) && !ignoreCardFields)) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (isOnlinePayment && ((fieldKey == 'cvv' && (!fieldValue || fieldValue == '' || !fieldValue.match(cvvValidRegex))) && !ignoreCardFields)) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (isOnlinePayment && ((fieldKey == 'cc_number' && !formFields.cc_type) && !ignoreCardFields)) {
          ...
          } else if (formType == 'WithoutShippingFull') {
            if ((fieldKey == 'firstName' || fieldKey == 'lastName') && (!fieldValue || fieldValue == '') && !ignoreAddressFields) {
              $rootScope.errorMessages.push(error_messages[fieldKey]);
            } else if (fieldKey == 'email' && (!fieldValue || fieldValue == '' || !fieldValue.match(emailValidRegex)) && !ignoreAddressFields) {
              $rootScope.errorMessages.push(error_messages[fieldKey]);
            } else if (((fieldKey == 'cc_type' || fieldKey == 'cc_number' || fieldKey == 'expmonth' || fieldKey == 'expyear' || fieldKey == 'cvv') && (!fieldValue || fieldValue == '')) && !ignoreCardFields) {
              $rootScope.errorMessages.push(error_messages[fieldKey]);
            } else if ((document.getElementsByName('agreeCheckbox').length > 0) && fieldKey == 'agreeCheckbox' && !fieldValue) {
              $rootScope.errorMessages.push(error_messages[fieldKey]);
            } else if (((fieldKey == 'cc_type' || fieldKey == 'cc_number' || fieldKey == 'expmonth' || fieldKey == 'expyear') && (!fieldValue || fieldValue == '')) && !ignoreCardFields) {
              $rootScope.errorMessages.push(error_messages[fieldKey]);
            } else if ((fieldKey == 'cvv' && (!fieldValue || fieldValue == '' || !fieldValue.match(cvvValidRegex))) && !ignoreCardFields) {
              $rootScope.errorMessages.push(error_messages[fieldKey]);
            } else if ((fieldKey == 'cc_number' && !formFields.cc_type) && !ignoreCardFields) {
              $rootScope.errorMessages.push(error_messages[fieldKey]);
            } else if ((fieldKey == 'cc_number' && (!fieldValue || !fieldValue.match($rootScope.cardPatternFullValidRegex[formFields.cc_type]) || !$rootScope.validateBIN(fieldValue)) && !$rootScope.hasThisTestCard(formFields.cc_number)) && !ignoreCardFields) {
              $rootScope.errorMessages.push(error_messages[fieldKey]);
            } else if (hasSquareNonce && (fieldKey == 'nonce' && (!fieldValue || fieldValue == ''))) {
              $rootScope.errorMessages.push(error_messages[fieldKey]);
            }
            ...
            if (fieldKey == 'email' && (!fieldValue || fieldValue == '' || !fieldValue.match(emailValidRegex))) {
              $rootScope.errorMessages.push(error_messages[fieldKey]);
            } else if (!fieldValue || fieldValue == '') {
              if (error_messages[fieldKey] != undefined) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
          ...
          } else if (formType == 'OrderCancellation') {
            if (fieldKey == 'email' && (!fieldValue || fieldValue == '' || !fieldValue.match(emailValidRegex))) {
              $rootScope.errorMessages.push(error_messages[fieldKey]);
            } else if (fieldKey == 'orderId' && (!fieldValue || fieldValue == '')) {
              $rootScope.errorMessages.push(error_messages[fieldKey]);
            } else if (!fieldValue || fieldValue == '') {
              if (error_messages[fieldKey] != undefined) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
            ...
            if (billingSameAsShipping == 'yes') {
              if (((fieldKey == 'cc_type' || fieldKey == 'cc_number' || fieldKey == 'expmonth' || fieldKey == 'expyear') && (!fieldValue || fieldValue == '')) && !ignoreCardFields) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if ((fieldKey == 'cvv' && (!fieldValue || fieldValue == '' || !fieldValue.match(cvvValidRegex))) && !ignoreCardFields) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if ((fieldKey == 'cc_number' && !formFields.cc_type) && !ignoreCardFields) {
                ...
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (((fieldKey == 'billingAgreeCheckbox') && (!fieldValue || fieldValue == ''))) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              }
            } else {
              if ((fieldKey == 'firstName' || fieldKey == 'lastName' || fieldKey == 'address' || fieldKey == 'city' || fieldKey == 'country' || fieldKey == 'state' || fieldKey == 'billingFirstName' || fieldKey == 'billingLastName' || fieldKey == 'billingAddress' || fieldKey == 'billingCity' || fieldKey == 'billingCountry' || fieldKey == 'billingState') && (!fieldValue || fieldValue == '') && !ignoreAddressFields) {
                ...
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (((fieldKey == 'cc_type' || fieldKey == 'cc_number' || fieldKey == 'expmonth' || fieldKey == 'expyear' || fieldKey == 'billingAgreeCheckbox') && (!fieldValue || fieldValue == '')) && !ignoreCardFields) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if ((fieldKey == 'cvv' && (!fieldValue || fieldValue == '' || !fieldValue.match(cvvValidRegex))) && !ignoreCardFields) {
                $rootScope.errorMessages.push(error_messages[fieldKey]);
              } else if (hasSquareNonce && (fieldKey == 'nonce' && (!fieldValue || fieldValue == ''))) {
      ...
      // Add slider captcha error
      if ($rootScope.configData.show_slider_capcha && $rootScope.hasSliderCaptcha) {
        if ($rootScope.validateSliderCaptcha === false) {
          $rootScope.errorMessages.push('Captcha verification failed!');
  ...
  // Slider captcha api
  $rootScope.loadSliderCaptcha = function() {
    $rootScope.hasSliderCaptcha = true;
    $("body").append($('<link href="frontend/css/slidercaptcha.min.css" rel="stylesheet"><script src="frontend/js/longbow.slidercaptcha.min.js"></script>'));
    $('#slider-captcha div:first').remove();
    var captchaHTML = null;
    if ($rootScope.configData.show_slider_capcha) {
      captchaHTML = '<div class="slidercaptcha card">\n\
                    ...
                    <input style="display:none" type="text" name="slider-captcha" value="" class="required has-error" data-error-message="Please complete the captcha!">\n\
                    </div>';

      $('#slider-captcha').append(captchaHTML);

      $rootScope.appendSliderCaptcha();

    }
  }
  // slider captcha append js
  $rootScope.appendSliderCaptcha = function() {
    ...
    setTimeout(function() {
      var captcha = sliderCaptcha({
        ...
        onSuccess: function() {
          $("input[name=slider-captcha]").removeClass('required has-error');
          $("input[name=slider-captcha]").attr('value', 'captchaChecked');
          $rootScope.validateSliderCaptcha = true;

        },
        onFail: function() {
          $("input[name=slider-captcha]").addClass('required has-error');
          $("input[name=slider-captcha]").attr('value', ' ');
          $rootScope.validateSliderCaptcha = false;
        },
        onRefresh: function() {
          $("input[name=slider-captcha]").addClass('required has-error');
  ...
  $rootScope.calculateCartTotal = function() {
    $timeout(function() {
      ...
      $rootScope.stripeButton();
    });
  };

  /** Stripe Payment Gateway : Start */
  $rootScope.stripeButton = function() {

    if ($rootScope.controller_name == 'LandingPageCtrl') return;

    if ($rootScope.configData.apple_google_pay_enabled != 1) return;

    if ($rootScope.pival != null) {
      $rootScope.formData.paymentIntentId = $rootScope.pival;
    }

    var frmData = null;
    if ($rootScope.controller_name == 'LandingOfferPageCtrl') {
      $rootScope.getHiddenParameters($rootScope.FullFormData);
      frmData = $rootScope.FullFormData;
    }

    if ($rootScope.controller_name == 'CheckoutPageCtrl') {
      $rootScope.getHiddenParameters($rootScope.CheckoutData);
      frmData = $rootScope.CheckoutData;
    }

    $http.post(SITE_BASE_URL + '/ajax/client_secret', frmData).then(function(response) {
      var data = response.data

      if (data.status == false) return;

      var total_price = parseInt(data.total_price);
      var clientSecret = data.stripe.client_secret;
      var publishableKey = data.stripe.publishableKey;

      if (clientSecret != '' && total_price > 0.5) {
        // 1. Initialize Stripe ......
        ...
        var paymentRequest = stripe.paymentRequest({
          country: 'US',
          currency: $rootScope.configData.currency.toLowerCase(),
          total: {
            label: 'Cart total',
            amount: total_price
          },
        ...
        paymentRequest.on('paymentmethod', async function(e) {
          ...
          $rootScope.redirectToThankYou(paymentIntent.id);
        });
  ...
  $rootScope.redirectToThankYou = function(msg) {
    var forceRedirectPage = $rootScope.redirectTo ? $rootScope.redirectTo : $rootScope.forceRedirect();
    var forceRedirectTo = forceRedirectPage == null ? 'thank-you?order_id=' + msg : forceRedirectPage;
  ...
  $rootScope.hasNewCartStorage = function(cartStorageName) {
    var cart = cartStorageName || 'uniCart';
  ...
  $rootScope.browserAutofill = function(formType, append) {
    ...
    if (document.querySelector('[ng-model="' + formType + '.' + countryDomKey + '"]') !== null) {
      ...
      // Prevent duplicate options in select element (Country, States etc)
      $compile($('[class="fakecountry input-hidden"]'))($rootScope);
      $compile($('[class="fakestate input-hidden"]'))($rootScope);
  ...
  $rootScope.pushErrorMessage = function(response) {
    ...
    } else if (response.data && response.data.apiResponse && typeof response.data.apiResponse === 'string') {
      $rootScope.errorMessages = [response.data.apiResponse];
    } else if ($rootScope.configData.api_type_code == 1 && response.data.apiResponse.errorMessage) {
      $rootScope.errorMessages = [response.data.apiResponse.errorMessage];
    } else {
  ...
  $rootScope.checkout3DValidation = function(formFields) {
    return $q(function(resolve, reject) {
      if ($rootScope.configData.is_3d_validation == 0 || $rootScope.configData.api_type_code != 1) {
        resolve();
      } else {
  ...
  $rootScope.initializePaayThreeDS = function() {
    ...
    if ($rootScope.configData.is_paay3d_validation) {
      return $q(function(resolve, reject) {
        ...
        document.querySelector('[data-threeds=pan]').value = $rootScope[formType]['cc_number'];
        document.querySelector('[data-threeds=month]').value = $rootScope[formType]['expmonth'];
        document.querySelector('[data-threeds=year]').value = $rootScope[formType]['expyear'];
        var paayAmount = document.querySelector('[data-threeds=amount]').value;
        ...
        if (localStorage.getItem("debug") == "yes") {
          postData.endpoint = "https://sandbox-api.3dsintegrator.com";
        ...
        var tds = new ThreeDS(
          formIdName,
          $rootScope.configData.paay_api_key,
          null,
          postData
        );
        console.info('3DS PAAY CALLED');
        tds.verify(function(data) {
          console.info(data);
          if (data.success) {
  ...
  /*** State tax profile :START ***/
  $rootScope.taxPercentage = 0.00;
  $rootScope.totalWithTax = 0.00;
  $rootScope.currentCountry = '';
  $rootScope.currentState = '';

  $rootScope.updateStateTax = function() {
    var formCountry = '';
    var formState = '';

    if ($rootScope.FormData.country !== '' || $rootScope.FormData.state !== '') {
      formCountry = $rootScope.FormData.country;
      formState = $rootScope.FormData.state;
    } else if ($rootScope.FullFormData.country !== '' || $rootScope.FullFormData.state !== '') {
      formCountry = $rootScope.FullFormData.country;
      formState = $rootScope.FullFormData.state;
    }

    if (formCountry !== '' || formState !== '') {
      if (formCountry === 'GB') {
        $rootScope.currentCountry = 'UK';
      } else {
        $rootScope.currentCountry = formCountry;
      }
      // GB-ANT
      if (formState != null && formState.indexOf('-') >= 0) {
        $rootScope.currentState = formState.slice((formState.indexOf('-') + 1));
      } else {
        $rootScope.currentState = formState;
      }
    }
    // Update tax
    $rootScope.getStateTaxPercentage($rootScope.currentCountry, $rootScope.currentState);
  }

  $rootScope.hasActiveTaxProfile = function() {
    if ($rootScope.configData.disabledStateTax || $rootScope.configData.stateTax === false) {
      return false;
    } else return true;
  }

  $rootScope.getStateTaxPercentage = function(country_name, state_name) {
    $rootScope.taxPercentage = 0.00;
    ...
    if (country_name === '' || state_name === '') {
      ...
      country_name = country_name.toLocaleUpperCase();
      state_name = state_name.toLocaleUpperCase();
    }

    if (!$rootScope.hasActiveTaxProfile()) return;
    var countriesArray = $rootScope.configData.countries.split(",");
    var stateTax = $rootScope.configData.stateTax;
    if (country_name in stateTax) {
      if (state_name in stateTax[country_name]) {
        $rootScope.taxPercentage = stateTax[country_name][state_name];
        return;
      } else {
        if ('ALL' in stateTax[country_name]) {
          $rootScope.taxPercentage = stateTax[country_name]['ALL'];
          return;
        }
      }
    }
  ...
  $rootScope.getTaxTotal = function() {
    var t = $rootScope.getCartTotal();
    var tp = $rootScope.taxPercentage;
    var p = (t * (tp / 100));
    return parseFloat(p).toFixed(2);
  }

  $rootScope.cartTotalWithTax = function() {
    var cp = parseFloat($rootScope.getCartTotal());
    var tp = parseFloat($rootScope.getTaxTotal());
    var t = parseFloat(cp + tp).toFixed(2);
    $rootScope.totalWithTax = t;
    return t;
  }
  /*** State tax profile :END ***/
  $rootScope.prepareQueryString = function(skips) {
    var qst = $rootScope.getQueryStrings(),
      rurl = '';
    for (var i in qst) {
      if (qst.hasOwnProperty(i)) {
  ...
  // Method to trigger form submit
  $rootScope.onClickFormSubmit = function(type) {
    $http.post(SITE_BASE_URL + '/ajax/pixel', $rootScope.FormData).then(function(response) {
      if (document.getElementById('header_tag_pixel') && (response.header_tag_pixel != undefined)) {
        document.getElementById('header_tag_pixel').innerHTML = response.header_tag_pixel;
      }
      if (document.getElementById('body_start_tag_pixel') && (response.body_start_tag_pixel != undefined)) {
        document.getElementById('body_start_tag_pixel').innerHTML = response.body_start_tag_pixel;
      }
      if (document.getElementById('body_end_tag_pixel') && (response.body_end_tag_pixel != undefined)) {
        document.getElementById('body_end_tag_pixel').innerHTML = response.body_end_tag_pixel;
      }
    });
  };

  // Check test card
  $rootScope.hasThisTestCard = function(cardNumber) {

    var test_cards = $rootScope.configData.test_cards && $rootScope.configData.test_cards.length ? $rootScope.configData.test_cards : [];

    var btoaCardNumber = btoa(cardNumber);

    if ((test_cards.indexOf(cardNumber) !== -1) || (test_cards.indexOf(btoaCardNumber) !== -1)) {
      return true;
    }
  ...
  $rootScope.detectCardType = function(formType, $scope) {

    var supportedCardTypes = this.cardTypes.map(function(item) {
      return item.type
    });

    for (cType in this.cardPatternFullValidRegex) {
      if ($scope[formType].cc_number.match(this.cardPatternFullValidRegex[cType])) {

        if (cType, supportedCardTypes.indexOf(cType) === -1) continue;

        if (['master', 'mastercard'].indexOf(cType) >= 0) {
          cType = supportedCardTypes.indexOf('master') >= 0 ? 'master' : 'mastercard'
        }
        $scope[formType].cc_type = cType;
        return;
      }
    }
  }

  $rootScope.paypalExpress = function(event, formData) {
    // Check if its from Paypal Button 
    if ($rootScope.configData.paypal_express != null && $rootScope.configData.paypal_express.enabled != false) {
      if ((event.originalEvent && event.originalEvent.submitter.getAttribute('data-paypal-express') !== 'yes') || (event.submitter && event.submitter.getAttribute('data-paypal-express') !== 'yes')) {
        $rootScope.isPaypalFlow = false;
        formData.paypal_payment = !1;
        return;
      }

      $rootScope.isPaypalFlow = true;
      formData.paypal_payment = 1;
      formData.cc_type = "paypal";
    }
  }
...
// Without Shipping offer page controller
app.controller("WithoutShippingLandingOfferPageCtrl", function($rootScope, $scope, $http, $compile, $timeout) {
  $rootScope.controller_name = 'WithoutShippingLandingOfferPageCtrl';
  // Google Captcha
  ...
  $scope.landingOfferPageSubmitHandler = function($event, internalCall) {
    ...
    $rootScope.errorHandler($rootScope.FullFormData, 'WithoutShippingFull').then(function() {
      var campaignId = '';
      ...
      $rootScope.initializePaayThreeDS().then(function() {
        $http.post(SITE_BASE_URL + '/ajax/offer-without-shipping', $rootScope.FullFormData).then(function(response) {
          if (response.data.status == 'success') {

            if (response.data.isPaypalExpressReponse) {
              // Replace the Document is the new paypal repsonse
            ...
            if (response.data.declineReason) {
              $rootScope.processOverlay(false);
            ...
            $rootScope.pushErrorMessage(response);
            $rootScope.modalShow();
            // Handle google captcha if google captcha is enabled with ipfraudscore
            if (response.data.isActiveCaptcha != undefined) {
            ...
            // Handle slider captcha if slider captcha is enabled with the max mind
            if (response.data.result && response.data.result.isSliderCaptcha != undefined) {
              $rootScope.configData.show_slider_capcha = response.data.result.isSliderCaptcha;
...
app.controller("LandingOfferPageCtrl", function($rootScope, $scope, $http, $compile, $timeout) {
  ...
  $rootScope.attachCustomFields = function() {

    // 1) Pick from hidden inputs (custom_*)
    angular.forEach(document.querySelectorAll('input[name^="custom"], input[id^="custom"]'), function(el) {

      var key = el.name || el.id;

      if (!key) return;

      if (/^custom/i.test(key) && el.value !== '') {
        $rootScope.FullFormData[key] = el.value;
      }
    });
  };
      ...
      $rootScope.initializePaayThreeDS().then(function() {
        $http.post(SITE_BASE_URL + '/ajax/offer', $rootScope.FullFormData).then(function(response) {
          if (response.data.status == 'success') {
            ...
            if (response.data.isSticky3dsReponse) {

              // Replace the Document is the new paypal repsonse
              if (response.data.hasOwnProperty('is_html') && response.data.is_html) {

                // Assuming your response is stored in a variable called 'response'
                const htmlData = response.data.html_data;

                // Convert the object to a string if needed
                const htmlString = JSON.stringify(htmlData);

                // Extract the URL using regex
                const urlMatch = htmlString.match(/window\.location\.replace\('([^']+)'\)/);

                if (urlMatch && urlMatch[1]) {
                  let redirectUrl = urlMatch[1];

                  // Clean up the URL: replace "," with & and remove all quotes
                  redirectUrl = redirectUrl
                    .replace(/","/g, '&') // Replace "," with &
                    .replace(/"/g, '') // Remove all remaining quotes
                    .replace(/(?<!https):(?=\w)/g, '='); // Replace : with = (except after https)
                ...
                console.log('in if');
                document.write(response.data.html_data.html);
                document.close();
                return;
              }
            }
...
app.controller("LandingPageCtrl", function($rootScope, $scope, $http, $compile, $timeout) {
  $rootScope.controller_name = 'LandingPageCtrl';
  $rootScope.getCountries();
  $rootScope.fetchGeolocationData();
    ...
    $rootScope.errorHandler($rootScope.FormData, 'prospect').then(function() {
      ...
      $http.post(SITE_BASE_URL + '/ajax/prospect', $rootScope.FormData).then(function(response) {
        if (response.data.status == 'success') {
          ...
          $rootScope.onClickFormSubmit('prospect');
          var prospectID = response.data.apiResponse.prospectId;
          if (response.data.apiResponse.customerAddressId !== undefined) {
            var customerAddressId = response.data.apiResponse.customerAddressId;
            var rurl = './' + forceRedirectTo + '?prospect_id=' + prospectID + '&customers_address_billing_id=' + customerAddressId;
            rurl += $rootScope.prepareQueryString(['prospect_id', 'customers_address_billing_id']);
          } else {
            var rurl = './' + forceRedirectTo + '?prospect_id=' + prospectID;
            rurl += $rootScope.prepareQueryString(['prospect_id']);
          }
...
app.controller("CheckoutPageCtrl", function($rootScope, $scope, $http) {
  $rootScope.controller_name = 'CheckoutPageCtrl';
  // Google Captcha
  $rootScope.loadGoogleCaptcha();
  var captchaCallBack = function(res) {
    $rootScope.validateGogoleCaptcha = true;
  ...
  $scope.loadCountries = function() {
    if ($rootScope.CheckoutData.billingSameAsShipping == 'no') {
  ...
  $scope.$watch('CheckoutData.cc_number', function() {
    $rootScope.detectCardType('CheckoutData', $scope);
  });
  ...
  $scope.CheckoutPageSubmitHandler = function($event, internalCall) {
    ...
    if (queryStrings.elm_cus_id !== undefined) {
      $rootScope.CheckoutData.elmCusId = queryStrings.elm_cus_id;
    }
    if (queryStrings.customers_address_billing_id !== undefined) {
      $rootScope.CheckoutData.customers_address_billing_id = queryStrings.customers_address_billing_id;
    }
    $rootScope.paypalExpress($event, $rootScope.CheckoutData);
    ...
    if ($rootScope.configData.api_type == 'Square') {
      $rootScope.CheckoutData.nonce = $rootScope.SquarePayment.nonce;
    }
    $rootScope.errorHandler($rootScope.CheckoutData, 'checkout').then(function() {
      ...
      $rootScope.initializePaayThreeDS().then(function() {
        $http.post(SITE_BASE_URL + '/ajax/checkout', $rootScope.CheckoutData)
          .then(function(response) {
            if (response.data.status == 'success') {
              $rootScope.onClickFormSubmit('checkout');
              if (response.data.isPaypalExpressReponse) {
                // Replace the Document is the new paypal repsonse
                if (response.data.hasOwnProperty('is_html') && response.data.is_html) {
                  document.write(response.data.html_data);
              ...
              if (response.data.declineReason) {
                $rootScope.CheckoutData.decline_reason = response.data.declineReason;
                $http.post(SITE_BASE_URL + '/ajax/post-mail', $rootScope.CheckoutData).then(function(response) {
                  if (response.data.status == 'success' || response.data.status == true || response.data.status == false) {
                ...
                $rootScope.triggerMail(false, orderID);
                var redirect_to = parseInt($rootScope.configData.enable_upsells) ? 'upsell' : forceRedirectTo;
                var rurl = './' + redirect_to + '?prospect_id=' + prospectID + '&customer_id=' + customerID + '&order_id=' + orderID;
                rurl += $rootScope.prepareQueryString(['prospect_id', 'customer_id', 'order_id']);
                window.location.href = rurl;
              }
            } else {
              $rootScope.processOverlay(false);
...
app.controller("UpsellPageCtrl", function($rootScope, $scope, $http) {
  $rootScope.controller_name = 'UpsellPageCtrl';
  //Open footer links in popup
  $rootScope.initUtilities();
  //Campaign Switcher
  $rootScope.campaignSwitcher();
  ...
  $scope.upsellPageSubmitHandler = function($event) {
    $rootScope.paypalExpress($event, $rootScope.UpsellData);
    ...
    $http.post(SITE_BASE_URL + '/ajax/upsell', data).then(function(response) {
      if (response.data.status == 'success') {
        ...
        var redirect_to = (parseInt($rootScope.configData.upsell_step) - parseInt(response.data.upsell_step)) == 1 ? forceRedirectTo : 'upsell';
        window.location.href = './' + redirect_to + '?' + (hasProspectId !== false ? 'prospect_id=' + hasProspectId + '&' : '') + 'customer_id=' + customerID + '&order_id=' + orderID;
      } else {
        $rootScope.processOverlay(false);
        $rootScope.pushErrorMessage(response);
...
app.controller("ThankyouPageCtrl", function($rootScope, $scope, $http) {
  $rootScope.controller_name = 'ThankyouPageCtrl';
  //Open footer links in popup
  $rootScope.initUtilities();
  // Campaign Switcher
  $rootScope.campaignSwitcher();
...
app.controller("CookieConsentCtrl", function($rootScope, $scope) {

  var cookieConsentCookie = $rootScope.getCookie('cookie_consent');
  ...
  $scope.cookieAccept = function() {
    $("#cookiePopup").hide();
    $rootScope.setCookie("cookie_consent", "yes", 90);
  }

  $scope.cookieDecline = function() {
    $("#cookiePopup").hide();
    $rootScope.setCookie("cookie_consent", "no", 90);
  }
})
...
app.controller("AccessCtrl", function($rootScope, $scope, $http) {
  ...
  $scope.loginSubmitHandler = function() {
    ...
    $http.post(SITE_BASE_URL + '/ajax/access-login', $scope.AccessFormData).then(function(response) {
      if (response.data.status == 'success') {
        ...
        $rootScope.setCookie("access_token", JSON.stringify(response.data.result), loginExpiryDays);
  ...
  $scope.forgetPasswordSubmitHandler = function() {
    $rootScope.errorHandler($scope.AccessFormData, 'reset');
    ...
    var forceRedirectTo = forceRedirectPage == null || forceRedirectPage == undefined ? null : forceRedirectPage;
    $rootScope.processOverlay(true);
    $http.post(SITE_BASE_URL + '/ajax/forget-password', $scope.AccessFormData).then(function(response) {
      if (response.data.status == 'success') {
        $rootScope.processOverlay(false);
        if (!$scope.hidePopup) {
          $rootScope.errorMessages = ['Reset password link has been sent'];
          $rootScope.modalShow();
        }
        if (forceRedirectTo != null) {
          var redirect_to = forceRedirectTo;
  ...
  $scope.validateCookie = function() {
    ...
    $http.post(SITE_BASE_URL + '/reset-validate', params).then(function(response) {
  ...
  $scope.resetPasswordSubmitHandler = function() {
    var forceRedirectPage = $scope.redirectTo;
    var forceRedirectTo = forceRedirectPage == null || forceRedirectPage == undefined ? null : forceRedirectPage;
    var loginExpiryDays = $scope.loginExpiryDays == null || $scope.loginExpiryDays == undefined ? 7 : $scope.loginExpiryDays;
    $rootScope.processOverlay(true);
    $http.post(SITE_BASE_URL + '/ajax/reset-password', $scope.AccessFormData).then(function(response) {
      if (response.data.status == 'success') {
        $rootScope.processOverlay(false);
        document.cookie = 'reset_link =;expires=Thu, 01 Jan 1970 00:00:01 GMT;';
        if (!$scope.hidePopup) {
          $rootScope.errorMessages = [response.data.message];
          $rootScope.modalShow();
        }
...
// Custom Mail controller starts
app.controller("CustomMailCtrl", function($rootScope, $scope, $http) {
  $scope.MailFormData = {
    el_custom_token: {}
  };
  $rootScope.getConfig();
  $scope.cutomMailSubmitHandler = function() {
    var forceRedirectPage = $scope.redirectTo;
    var forceRedirectTo = forceRedirectPage == null || forceRedirectPage == undefined ? null : forceRedirectPage;
    var successMessage = $scope.successMessage == null || $scope.successMessage == undefined ? 'Message has been sent!' : $scope.successMessage;
    $rootScope.processOverlay(true);
    $http.post(SITE_BASE_URL + '/ajax/post-mail', $scope.MailFormData).then(function(response) {
      if (response.data.status == 'success' || response.data.status == true) {
        ...
        $rootScope.processOverlay(false);
        $rootScope.errorMessages = [response.data.api_response.response_message];
        $rootScope.modalShow();
      }
    }).catch(function(response) {
      $rootScope.processOverlay(false);
...
// Custom Mail controller ends

// Order Cancellation controller
app.controller("OrderCancellationCtrl", function($rootScope, $scope, $http) {
  $scope.CancellationFormData = {};
  $rootScope.getConfig();
  $scope.CancellationFormSubmitHandler = function() {
    ...
    $rootScope.processOverlay(true);
    $http.post(SITE_BASE_URL + '/ajax/cancel', $scope.CancellationFormData).then(function(response) {
      if (response.data.status == 'success' || response.data.status == true) {
