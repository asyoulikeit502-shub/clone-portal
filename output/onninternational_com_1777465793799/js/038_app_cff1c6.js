// Keep search toggle resilient even if later JS throws.
$(document).on('click', '#search_toggle', function(e) {
  e.preventDefault();
  $('.search_wrap').toggleClass('active');

  setTimeout(function() {
    $('.search_wrap input[name="query"]').first().focus();
  }, 150);
});
...
//Homepage Revamp Slider
document.addEventListener("DOMContentLoaded", function() {
  // ===========================
  // MENU TRENDING SLIDER
  // ===========================
  // ✅ Select ALL .menutrending__slider elements and init each separately
  document.querySelectorAll(".menutrending__slider").forEach(function(el) {
    new Swiper(el, {
      ...
      allowTouchMove: true,
      slideToClickedSlide: false, // ✅ remove — causes jerk with loop
      // ❌ navigation removed — .collection-left/right don't exist on other pages
      // ❌ thumbs removed — collection__thumb not defined
    });
  });
  // ===========================
  // 1. CATEGORY SLIDER
  // ===========================
  var category__slider = new Swiper(".home-category__slider", {
    direction: "horizontal",
    loop: true,
    speed: 800,
    autoplay: {
  ...
  document.querySelectorAll(".category-left").forEach(function(btn) {
    btn.addEventListener("click", function() {
      category__slider.slidePrev(800);
    });
  });
  document.querySelectorAll(".category-right").forEach(function(btn) {
    btn.addEventListener("click", function() {
      category__slider.slideNext(800);
    });
  });
  ...
  var collection__slider = new Swiper(".home-collection__slider", {
    direction: "horizontal",
    loop: true,
    speed: 500, // ✅ reduce from 800 to 500
    autoplay: {
      delay: 5000,
      disableOnInteraction: true, // ✅ stop autoplay when user clicks
    ...
    allowTouchMove: true,
    slideToClickedSlide: false,
    loopAdditionalSlides: 2, // ✅ reduce from 4 to 2
    breakpoints: {
      0: {
        slidesPerView: 1,
        spaceBetween: 16
  ...
  var isSliding = false; // ✅ prevent rapid clicking causing jerk

  document.querySelectorAll(".home-collection-left").forEach(function(btn) {
    btn.addEventListener("click", function() {
      if (isSliding) return;
      isSliding = true;
      collection__slider.slidePrev(500);
      setTimeout(function() {
        isSliding = false;
      }, 600); // slightly more than speed
    });
  });

  document.querySelectorAll(".home-collection-right").forEach(function(btn) {
    btn.addEventListener("click", function() {
      if (isSliding) return;
      isSliding = true;
      collection__slider.slideNext(500);
      setTimeout(function() {
        isSliding = false;
      }, 600);
    });
  ...
  // ===========================
  // 3. INNER-OUTER SLIDER
  // ===========================
  var innerSlider = new Swiper("#innerwear .home-inner-outer-collection__slider", {
    direction: "horizontal",
    loop: true,
    speed: 800,
    autoplay: {
  ...
  var outerSlider = null;

  // ✅ Initialize outerwear ONLY when tab is clicked (it's hidden on load)
  document.querySelector('[data-bs-target="#outerwear"]').addEventListener('shown.bs.tab', function() {
    if (outerSlider) return;

    outerSlider = new Swiper("#outerwear .home-inner-outer-collection__slider", {
      direction: "horizontal",
      loop: true,
      speed: 800,
      autoplay: {
  ...
  document.querySelectorAll(".inner-outer-collection-left").forEach(function(btn) {
    btn.addEventListener("click", function() {
      var activeTab = document.querySelector(".tab-pane.active");
      if (activeTab.id === "innerwear") innerSlider.slidePrev(800);
      if (activeTab.id === "outerwear" && outerSlider) outerSlider.slidePrev(800);
    });
  });
  document.querySelectorAll(".inner-outer-collection-right").forEach(function(btn) {
    btn.addEventListener("click", function() {
      var activeTab = document.querySelector(".tab-pane.active");
      if (activeTab.id === "innerwear") innerSlider.slideNext(800);
      if (activeTab.id === "outerwear" && outerSlider) outerSlider.slideNext(800);
    });
  });
...
var outerwearcollection__slider = new Swiper(".home-outerwear-collection__slider", {
  direction: "horizontal",
  loop: true,
  speed: 2000,
  // autoplay: {
  ...
  // },
  pagination: {
    el: ".home-outerwear-collection__slider .bullet",
    clickable: true,
  },

  navigation: {
    nextEl: ".outerwear-right",
    prevEl: ".outerwear-left",
  },
  thumbs: {
...
const numbers = document.querySelectorAll('.outerwear-slide-numbers span');

outerwearcollection__slider.on('slideChange', function() {
  const realIndex = outerwearcollection__slider.realIndex;

  numbers.forEach((el, i) => {
    el.classList.toggle('active', i === realIndex);
  });
});

numbers.forEach(el => {
  el.addEventListener('click', function() {
    outerwearcollection__slider.slideToLoop(
...
// var menutrending__slider = new Swiper(".menutrending__slider", {
//     direction: "horizontal",
//     loop: true,
//     speed: 2000,
//     padding: 10,
...
var wintercollection__slider = new Swiper(".home-winter-collection__slider", {
  direction: "horizontal",
  loop: true,
  speed: 2000,
  autoplay: {
...
if (typeof ScrollMagic !== 'undefined' && typeof TimelineMax !== 'undefined' && $('.home-product, .home-collection, .home-category').length) {
...
jQuery('[data-fancybox], [data-fancybox="gallery"]').fancybox({
