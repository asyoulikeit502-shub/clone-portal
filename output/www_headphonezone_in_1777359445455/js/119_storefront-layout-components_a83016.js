(function() {
    ...
    SwymStorefrontLayoutContext.ActionTypes = {
      ...
      ManageSfyListItem: 'ManageSfyListItem',
    ...
    SwymStorefrontLayoutContext.ListItemType = {
      ...
      SavedForYouItem: 'SavedForYouItem'
    }
    ...
    // The list name used by the SDK's auto-wishlist feature in multi-list mode
    const SFY_LIST_NAME = 'Saved For Me';

    // Returns the dedicated "Saved For Me" list object in multi-list mode, or null in single-list mode.
    SwymStorefrontLayoutContext.getSfyList = function() {
      const {
        shouldShowMultiple
      } = SwymStorefrontLayoutContext.getMultiWishlistConfig();
      if (!shouldShowMultiple) return null;
      return (SwymStorefrontLayoutContext.allLists || []).find(l => l.lname === SFY_LIST_NAME) || null;
    };

    /**
     * Helper function to check if education layer (tooltips/toasts on collection and PDP pages) is enabled.
     * Controlled via shop metafield `swym_wishlist.educationLayerEnabled`.
     * Defaults to true (enabled) unless explicitly set to false.
     */
    SwymStorefrontLayoutContext.isEducationLayerEnabled = function() {
      return window.SwymEducationLayerEnabled !== false;
    };

    /**
     * Returns whether the login nudge is enabled for this store.
     * Defaults to true (enabled) unless explicitly set to false.
     */
    SwymStorefrontLayoutContext.isLoginNudgeEnabled = function() {
      return window.SwymLoginNudgeEnabled !== false;
    };
    ...
    /**
     * Applies a transform function to listcontents across all context list references
     * (DefaultList, collections, lists, allLists) to keep them in sync.
     * @param {Function} transformFn - (listcontents: Array) => Array
     */
    SwymStorefrontLayoutContext.updateAllListContents = (transformFn) => {
      if (SwymStorefrontLayoutContext.DefaultList) {
        SwymStorefrontLayoutContext.DefaultList = {
          ...SwymStorefrontLayoutContext.DefaultList,
          listcontents: transformFn(SwymStorefrontLayoutContext.DefaultList.listcontents)
        };
      }
      ['collections', 'lists', 'allLists'].forEach(key => {
        if (SwymStorefrontLayoutContext[key]) {
          SwymStorefrontLayoutContext[key] = SwymStorefrontLayoutContext[key].map(list => ({
            ...list,
            listcontents: transformFn(list.listcontents)
          }));
        }
      });
    };
    ...
    SwymStorefrontLayoutExtensions.applyHeadingFontStyles = (titleElement) => {
      try {
        ...
        // When theme CSS mapping is active, typography is handled via CSS variables.
        // Skip inline overrides for font-family, font-weight, and letter-spacing
        // to avoid clobbering the CSS variable values with computed page styles.
        const themeMappingActive = !!document.querySelector('[data-swym-theme-mapping]');

        if (titleElement && headingFontStyles) {
          if (!themeMappingActive) {
    ...
    SwymStorefrontLayoutExtensions.getShareIconSvg = () => {
      return `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none" aria-hidden="true" focusable="false">
    <path d="M13.5 2.375C14.6669 2.375 15.625 3.33306 15.625 4.5C15.625 5.66694 14.6669 6.625 13.5 6.625C12.8556 6.625 12.2862 6.30928 11.8828 5.81445C11.7533 5.65562 11.5425 5.59393 11.3516 5.65137L11.2715 5.68359L6.77539 7.93262C6.5655 8.03762 6.45726 8.27438 6.51465 8.50195C6.58433 8.77824 6.625 8.86302 6.625 9C6.625 9.13698 6.58433 9.22176 6.51465 9.49805C6.45726 9.72562 6.5655 9.96238 6.77539 10.0674L11.2715 12.3164C11.4808 12.4211 11.7349 12.3669 11.8828 12.1855C12.2862 11.6907 12.8556 11.375 13.5 11.375C14.6669 11.375 15.625 12.3331 15.625 13.5C15.625 14.6669 14.6669 15.625 13.5 15.625C12.3331 15.625 11.375 14.6669 11.375 13.5C11.375 13.363 11.4157 13.2782 11.4854 13.002C11.5427 12.7744 11.4345 12.5376 11.2246 12.4326L6.72852 10.1836C6.54532 10.0919 6.32732 10.1214 6.17676 10.252L6.11719 10.3145C5.71385 10.8093 5.1444 11.125 4.5 11.125C3.33306 11.125 2.375 10.1669 2.375 9C2.375 7.83306 3.33306 6.875 4.5 6.875C5.1444 6.875 5.71385 7.19072 6.11719 7.68555C6.2651 7.86695 6.51918 7.92112 6.72852 7.81641L11.2246 5.56738C11.4345 5.46238 11.5427 5.22562 11.4854 4.99805C11.4157 4.72176 11.375 4.63698 11.375 4.5C11.375 3.33306 12.3331 2.375 13.5 2.375ZM13.5 11.5C12.3888 11.5 11.5 12.3888 11.5 13.5C11.5 14.6112 12.3888 15.5 13.5 15.5C14.6112 15.5 15.5 14.6112 15.5 13.5C15.5 12.3888 14.6112 11.5 13.5 11.5ZM4.5 7C3.38877 7 2.5 7.88877 2.5 9C2.5 10.1112 3.38877 11 4.5 11C5.61123 11 6.5 10.1112 6.5 9C6.5 7.88877 5.61123 7 4.5 7ZM13.5 2.5C12.3888 2.5 11.5 3.38877 11.5 4.5C11.5 5.61123 12.3888 6.5 13.5 6.5C14.6112 6.5 15.5 5.61123 15.5 4.5C15.5 3.38877 14.6112 2.5 13.5 2.5Z" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"/>
  </svg>`;
    }

    /**
     * Returns the wishlist icon SVG markup.
     * Currently uses a hardcoded heart icon. Dynamic icon support (heart/star/bookmark
     * based on retailer config) will be added later.
     * @param {'filled'|'unfilled'} state - Whether to return the filled or unfilled icon variant
     * @param {object} [opts] - Optional overrides
     * @param {string} [opts.color] - Icon stroke/fill color (default: 'currentColor')
     * @param {string} [opts.width] - SVG width attribute (default: '17')
     * @param {string} [opts.height] - SVG height attribute (default: '15')
     * @returns {string} SVG markup string
     */
    SwymStorefrontLayoutExtensions.getWishlistIconSvg = (state = 'filled', opts = {}) => {
      const color = opts.color || 'currentColor';
      const width = opts.width || '17';
      const height = opts.height || '15';

      if (state === 'unfilled') {
        return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 17 16" fill="none" aria-hidden="true" focusable="false">
      <path d="M3.94617 0.518585C4.71879 0.439587 5.46423 0.623711 6.12292 0.918976C7.09924 1.35664 7.94118 2.06375 8.495 2.66019C8.69022 2.42887 8.92463 2.21448 9.18054 2.02249C9.66446 1.65956 10.2568 1.35316 10.8905 1.14944C12.1427 0.746943 13.6652 0.716791 14.8436 1.60062C16.0064 2.47285 16.4537 3.64555 16.3866 4.90921C16.3211 6.14189 15.7705 7.444 14.993 8.66409C13.4387 11.1032 10.8538 13.404 8.76746 14.4473L8.53113 14.5655L8.30066 14.4375C5.98745 13.1524 3.46501 10.932 1.9364 8.58597C1.17217 7.41299 0.633191 6.17363 0.521362 4.97757C0.407722 3.76125 0.739132 2.59768 1.69031 1.64651C2.39175 0.945176 3.16373 0.598678 3.94617 0.518585Z" fill="white" stroke="${color}"/>
    </svg>`;
      }
      return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 17 15" fill="none" aria-hidden="true" focusable="false">
    <path d="M2.04363 2.00013C4.54368 -0.499876 7.71029 2.33346 8.54363 3.50012C9.21029 2.00012 12.5437 0.500119 14.5437 2.00013C18.6166 5.05486 12.5437 12.0001 8.54363 14.0001C4.04367 11.5001 -1.29179 5.3355 2.04363 2.00013Z" fill="${color}"/>
  </svg>`;
    }
    ...
    /**
     * Theme CSS variable mapping — maps Shopify theme CSS variables to Swym's
     * --swym-storefront-layout-* variables so the drawer inherits theme colors,
     * typography, button styles, and input styles automatically.
     *
     * Supports Dawn-family and Horizon-family themes. Unknown themes fall back
     * to the existing hardcoded :root values in storefront-layout.css.
     *
     * Runs once at init — injects a single <style> block, zero ongoing cost.
     */
    SwymStorefrontLayoutExtensions.ThemeCSSMapping = (() => {
      /**
       * Detects theme family using CSS variable feature detection.
       * Dawn-family themes define --font-heading-family on :root.
       * Horizon-family themes define --font-h2--family on :root.
       * This approach works with custom-named / forked themes that still use
       * the same CSS variable system as their parent theme.
       * Falls back to name-based matching as a secondary check.
       */
      const DAWN_THEMES = ['Dawn', 'Craft', 'Crave', 'Sense', 'Taste', 'Studio', 'Ride', 'Colorblock', 'Refresh', 'Origin', 'Publisher', 'Spotlight', 'Trade', 'Rise'];
      const HORIZON_THEMES = ['Horizon', 'Ritual', 'Heritage', 'Atelier', 'Pitch', 'Tinker', 'Dwell', 'Vessel', 'Savor', 'Fabric'];

      function getThemeFamily() {
        try {
          // Primary: CSS variable feature detection (works with custom-named themes)
          const rootStyles = getComputedStyle(document.documentElement);
          const hasDawnVars = rootStyles.getPropertyValue('--font-heading-family').trim();
          const hasHorizonVars = rootStyles.getPropertyValue('--font-h2--family').trim();
          if (hasDawnVars) return 'dawn';
          if (hasHorizonVars) return 'horizon';

          // Fallback: name-based matching
          const themeName = window.Shopify?.theme?.name;
          if (!themeName) return null;
          if (DAWN_THEMES.some(t => themeName.includes(t))) return 'dawn';
          if (HORIZON_THEMES.some(t => themeName.includes(t))) return 'horizon';
          return null;
        } catch (e) {
          return null;
        }
      ...
      function getDawnThemeVariables() {
        return `
      :root {
        /* UI */
        --swym-theme-layout-ui-bg-color: rgb(var(--color-background));
        --swym-theme-layout-ui-text-color: rgba(var(--color-foreground), 0.75);
        --swym-theme-layout-color-bg: rgb(var(--color-background));
        --swym-theme-layout-color-text: rgb(var(--color-foreground));
        --swym-theme-layout-color-text-default: rgb(var(--color-foreground));
        --swym-theme-layout-color-text-sub: rgba(var(--color-foreground), 0.55);
        --swym-theme-layout-color-text-sub2: rgba(var(--color-foreground), 0.4);
        --swym-theme-layout-color-border: rgba(var(--color-foreground), 0.15);
        --swym-theme-layout-border-color: rgba(var(--color-foreground), 0.15);

        /* TYPOGRAPHY */
        --swym-theme-layout-font-family-heading: var(--font-heading-family);
        --swym-theme-layout-font-weight-heading: var(--font-heading-weight);
        --swym-theme-layout-font-style-heading: var(--font-heading-style);
        --swym-theme-layout-text-transform-heading: none;
        --swym-theme-layout-letter-spacing-heading: calc(var(--font-heading-scale) * 0.06rem);
        --swym-theme-layout-text-color-heading: rgb(var(--color-foreground));
        --swym-theme-layout-font-family-body: var(--font-body-family);
        --swym-theme-layout-font-weight-body: var(--font-body-weight);
        --swym-theme-layout-font-weight-body-bold: var(--font-body-weight-bold);
        --swym-theme-layout-font-style-body: var(--font-body-style);
        --swym-theme-layout-letter-spacing-body: 0.06rem;
        --swym-theme-layout-text-transform-body: none;

        /* BUTTONS — primary */
        --swym-theme-layout-button-border-color-primary: rgba(var(--color-button-text), calc(1 - var(--buttons-border-opacity, 1)));
        --swym-theme-layout-button-border-radius: var(--buttons-radius, 0px);
        --swym-theme-layout-button-border-radius-primary: var(--buttons-radius, 0px);
        --swym-theme-layout-button-border-style-primary: solid;
        --swym-theme-layout-button-border-width-primary: var(--buttons-border-width, 0px);
        --swym-theme-layout-button-box-shadow-primary: var(--buttons-shadow-horizontal-offset, 0px) var(--buttons-shadow-vertical-offset, 0px) var(--buttons-shadow-blur-radius, 0px) rgba(var(--color-shadow, 0, 0, 0), var(--buttons-shadow-opacity, 0));
        --swym-theme-layout-button-color-bg-primary: rgba(var(--color-button), var(--alpha-button-background));
        --swym-theme-layout-button-color-text-primary: rgb(var(--color-button-text));
        --swym-theme-layout-button-font-family-primary: var(--font-body-family);
        --swym-theme-layout-button-font-size-primary: 1.5rem;
        --swym-theme-layout-button-font-style-primary: ;
        --swym-theme-layout-button-font-weight-primary: var(--font-body-weight);
        --swym-theme-layout-button-letter-spacing-primary: 0.1rem;
        --swym-theme-layout-button-text-transform-primary: none;

        /* BUTTONS — secondary */
        --swym-theme-layout-button-border-color-secondary: rgba(var(--color-secondary-button-text), var(--buttons-border-opacity, 1));
        --swym-theme-layout-button-border-radius-secondary: var(--buttons-radius, 0px);
        --swym-theme-layout-button-border-style-secondary: solid;
        --swym-theme-layout-button-border-width-secondary: var(--buttons-border-width, 0px);
        --swym-theme-layout-button-box-shadow-secondary: var(--buttons-shadow-horizontal-offset, 0px) var(--buttons-shadow-vertical-offset, 0px) var(--buttons-shadow-blur-radius, 0px) rgba(var(--color-shadow, 0, 0, 0), var(--buttons-shadow-opacity, 0));
        --swym-theme-layout-button-color-bg-secondary: rgb(var(--color-secondary-button));
        --swym-theme-layout-button-color-text-secondary: rgb(var(--color-secondary-button-text));
        --swym-theme-layout-button-font-family-secondary: var(--font-body-family);
        --swym-theme-layout-button-font-size-secondary: 1.5rem;
        --swym-theme-layout-button-font-style-secondary: ;
        --swym-theme-layout-button-font-weight-secondary: var(--font-body-weight);
        --swym-theme-layout-button-letter-spacing-secondary: 0.1rem;
        --swym-theme-layout-button-text-transform-secondary: none;

        /* INPUTS */
        --swym-theme-layout-input-color-bg: rgb(var(--color-background));
        --swym-theme-layout-input-color-text: rgb(var(--color-foreground));
        --swym-theme-layout-input-border-color: rgba(var(--color-foreground), var(--inputs-border-opacity, 0.55));
        --swym-theme-layout-input-border-radius: var(--inputs-radius, 0px);
        --swym-theme-layout-input-border-style: solid;
        --swym-theme-layout-input-border-width: var(--inputs-border-width, 1px);
        --swym-theme-layout-input-box-shadow: 0;
        --swym-theme-layout-input-box-shadow-hover: 0 0 0 var(--inputs-border-width, 1px) rgba(var(--color-foreground), var(--inputs-border-opacity, 0.55));
        --swym-theme-layout-input-box-shadow-focus: 0 0 0 var(--inputs-border-width, 1px) rgba(var(--color-foreground), 1);
        --swym-theme-layout-input-focus-border-color: rgba(var(--color-foreground), 1);
        --swym-theme-layout-input-focus-outline-color: transparent;
        --swym-theme-layout-input-focus-outline-offset: 0;
        --swym-theme-layout-input-focus-outline-style: solid;
        --swym-theme-layout-input-focus-outline-width: 0;
        --swym-theme-layout-input-transition: box-shadow var(--duration-short, 100ms) ease;
      }
    `;
      }

      function getHorizonThemeVariables() {
        return `
      :root {
        /* UI */
        --swym-theme-layout-ui-bg-color: var(--color-background);
        --swym-theme-layout-ui-text-color: var(--color-foreground);
        --swym-theme-layout-color-bg: var(--color-background);
        --swym-theme-layout-color-text: var(--color-foreground);
        --swym-theme-layout-color-text-default: var(--color-foreground);
        --swym-theme-layout-color-text-sub: color-mix(in srgb, var(--color-foreground) 55%, transparent);
        --swym-theme-layout-color-text-sub2: color-mix(in srgb, var(--color-foreground) 40%, transparent);
        --swym-theme-layout-color-border: color-mix(in srgb, var(--color-foreground) 15%, transparent);
        --swym-theme-layout-border-color: color-mix(in srgb, var(--color-foreground) 15%, transparent);

        /* TYPOGRAPHY */
        --swym-theme-layout-font-family-heading: var(--font-h2--family);
        --swym-theme-layout-font-weight-heading: var(--font-h2--weight);
        --swym-theme-layout-font-style-heading: var(--font-h2--style);
        --swym-theme-layout-text-transform-heading: var(--font-h2--case);
        --swym-theme-layout-letter-spacing-heading: var(--font-h2--letter-spacing);
        --swym-theme-layout-text-color-heading: var(--font-h2--color);
        --swym-theme-layout-font-family-body: var(--font-paragraph--family);
        --swym-theme-layout-font-weight-body: var(--font-paragraph--weight);
        --swym-theme-layout-font-weight-body-bold: bold;
        --swym-theme-layout-font-style-body: var(--font-paragraph--style);
        --swym-theme-layout-letter-spacing-body: var(--font-paragraph--letter-spacing);
        --swym-theme-layout-text-transform-body: var(--font-paragraph--case);

        /* BUTTONS — primary */
        --swym-theme-layout-button-border-color-primary: var(--color-primary-button-background);
        --swym-theme-layout-button-border-radius: var(--style-border-radius-buttons-primary);
        --swym-theme-layout-button-border-radius-primary: var(--style-border-radius-buttons-primary);
        --swym-theme-layout-button-border-style-primary: solid;
        --swym-theme-layout-button-border-width-primary: 0;
        --swym-theme-layout-button-box-shadow-primary: inset 0 0 0 var(--style-border-width-primary) var(--color-primary-button-border);
        --swym-theme-layout-button-color-bg-primary: var(--color-primary-button-background);
        --swym-theme-layout-button-color-text-primary: var(--color-primary-button-text);
        --swym-theme-layout-button-font-family-primary: var(--button-font-family-primary);
        --swym-theme-layout-button-font-size-primary: var(--font-paragraph--size);
        --swym-theme-layout-button-font-style-primary: var(--font-paragraph--style);
        --swym-theme-layout-button-font-weight-primary: var(--font-paragraph--weight);
        --swym-theme-layout-button-letter-spacing-primary: var(--font-paragraph--letter-spacing);
        --swym-theme-layout-button-text-transform-primary: var(--button-text-case-primary);

        /* BUTTONS — secondary */
        --swym-theme-layout-button-border-color-secondary: var(--color-secondary-button-background);
        --swym-theme-layout-button-border-radius-secondary: var(--style-border-radius-buttons-secondary);
        --swym-theme-layout-button-border-style-secondary: solid;
        --swym-theme-layout-button-border-width-secondary: 0;
        --swym-theme-layout-button-box-shadow-secondary: inset 0 0 0 var(--style-border-width-secondary) var(--color-secondary-button-border);
        --swym-theme-layout-button-color-bg-secondary: var(--color-secondary-button-background);
        --swym-theme-layout-button-color-text-secondary: var(--color-secondary-button-text);
        --swym-theme-layout-button-font-family-secondary: var(--button-font-family-secondary);
        --swym-theme-layout-button-font-size-secondary: var(--font-paragraph--size);
        --swym-theme-layout-button-font-style-secondary: var(--font-paragraph--style);
        --swym-theme-layout-button-font-weight-secondary: var(--font-paragraph--weight);
        --swym-theme-layout-button-letter-spacing-secondary: var(--font-paragraph--letter-spacing);
        --swym-theme-layout-button-text-transform-secondary: var(--button-text-case-secondary);

        /* INPUTS */
        --swym-theme-layout-input-color-bg: var(--color-input-background);
        --swym-theme-layout-input-color-text: var(--color-input-text);
        --swym-theme-layout-input-border-color: var(--color-input-border);
        --swym-theme-layout-input-border-radius: var(--style-border-radius-inputs);
        --swym-theme-layout-input-border-style: solid;
        --swym-theme-layout-input-border-width: var(--style-border-width-inputs);
        --swym-theme-layout-input-box-shadow: 0;
        --swym-theme-layout-input-box-shadow-hover: 0;
        --swym-theme-layout-input-box-shadow-focus: 0;
        --swym-theme-layout-input-focus-border-color: var(--color-input-border);
        --swym-theme-layout-input-focus-outline-color: currentColor;
        --swym-theme-layout-input-focus-outline-offset: var(--focus-outline-offset);
        --swym-theme-layout-input-focus-outline-style: solid;
        --swym-theme-layout-input-focus-outline-width: var(--focus-outline-width);
        --swym-theme-layout-input-transition: none;
      }
    `;
      }

      function getBridgeVariables() {
        return `
      #swym-storefront-layout-container {
        /* UI */
        --swym-storefront-layout-ui-bg-color: var(--swym-theme-layout-ui-bg-color);
        --swym-storefront-layout-ui-text-color: var(--swym-theme-layout-ui-text-color);
        --swym-storefront-layout-color-bg: var(--swym-theme-layout-color-bg);
        --swym-storefront-layout-color-text: var(--swym-theme-layout-color-text);
        --swym-storefront-layout-color-text-default: var(--swym-theme-layout-color-text-default);
        --swym-storefront-layout-color-text-sub: var(--swym-theme-layout-color-text-sub);
        --swym-storefront-layout-color-text-sub2: var(--swym-theme-layout-color-text-sub2);
        --swym-storefront-layout-color-border: var(--swym-theme-layout-color-border);
        --swym-storefront-layout-border-color: var(--swym-theme-layout-border-color);

        /* TYPOGRAPHY */
        --swym-storefront-layout-font-family-heading: var(--swym-theme-layout-font-family-heading);
        --swym-storefront-layout-font-weight-heading: var(--swym-theme-layout-font-weight-heading);
        --swym-storefront-layout-font-style-heading: var(--swym-theme-layout-font-style-heading);
        --swym-storefront-layout-text-transform-heading: var(--swym-theme-layout-text-transform-heading);
        --swym-storefront-layout-letter-spacing-heading: var(--swym-theme-layout-letter-spacing-heading);
        --swym-storefront-layout-text-color-heading: var(--swym-theme-layout-text-color-heading);
        --swym-storefront-layout-font-family-body: var(--swym-theme-layout-font-family-body);
        --swym-storefront-layout-font-weight-body: var(--swym-theme-layout-font-weight-body);
        --swym-storefront-layout-font-weight-body-bold: var(--swym-theme-layout-font-weight-body-bold);
        --swym-storefront-layout-font-style-body: var(--swym-theme-layout-font-style-body);
        --swym-storefront-layout-letter-spacing-body: var(--swym-theme-layout-letter-spacing-body);

        /* BUTTONS — primary */
        --swym-storefront-layout-button-color-bg-primary: var(--swym-theme-layout-button-color-bg-primary);
        --swym-storefront-layout-button-color-text-primary: var(--swym-theme-layout-button-color-text-primary);
        --swym-storefront-layout-button-color-bg-secondary: var(--swym-theme-layout-button-color-bg-secondary);
        --swym-storefront-layout-button-color-text-secondary: var(--swym-theme-layout-button-color-text-secondary);
        --swym-storefront-layout-button-border-color-primary: var(--swym-theme-layout-button-border-color-primary);
        --swym-storefront-layout-button-border-radius: var(--swym-theme-layout-button-border-radius);
        --swym-storefront-layout-button-border-radius-primary: var(--swym-theme-layout-button-border-radius-primary);
        --swym-storefront-layout-button-border-style-primary: var(--swym-theme-layout-button-border-style-primary);
        --swym-storefront-layout-button-border-width-primary: var(--swym-theme-layout-button-border-width-primary);
        --swym-storefront-layout-button-box-shadow-primary: var(--swym-theme-layout-button-box-shadow-primary);
        --swym-storefront-layout-button-font-family-primary: var(--swym-theme-layout-button-font-family-primary);
        --swym-storefront-layout-button-font-weight-primary: var(--swym-theme-layout-button-font-weight-primary);
        --swym-storefront-layout-button-font-size-primary: var(--swym-theme-layout-button-font-size-primary);
        --swym-storefront-layout-button-font-style-primary: var(--swym-theme-layout-button-font-style-primary);
        --swym-storefront-layout-button-text-transform-primary: var(--swym-theme-layout-button-text-transform-primary);
        --swym-storefront-layout-button-letter-spacing-primary: var(--swym-theme-layout-button-letter-spacing-primary);

        /* BUTTONS — secondary */
        --swym-storefront-layout-button-border-color-secondary: var(--swym-theme-layout-button-border-color-secondary);
        --swym-storefront-layout-button-border-radius-secondary: var(--swym-theme-layout-button-border-radius-secondary);
        --swym-storefront-layout-button-border-style-secondary: var(--swym-theme-layout-button-border-style-secondary);
        --swym-storefront-layout-button-border-width-secondary: var(--swym-theme-layout-button-border-width-secondary);
        --swym-storefront-layout-button-box-shadow-secondary: var(--swym-theme-layout-button-box-shadow-secondary);
        --swym-storefront-layout-button-font-family-secondary: var(--swym-theme-layout-button-font-family-secondary);
        --swym-storefront-layout-button-font-weight-secondary: var(--swym-theme-layout-button-font-weight-secondary);
        --swym-storefront-layout-button-font-style-secondary: var(--swym-theme-layout-button-font-style-secondary);
        --swym-storefront-layout-button-text-transform-secondary: var(--swym-theme-layout-button-text-transform-secondary);
        --swym-storefront-layout-button-letter-spacing-secondary: var(--swym-theme-layout-button-letter-spacing-secondary);
        --swym-storefront-layout-button-font-size-secondary: var(--swym-theme-layout-button-font-size-secondary);

        /* INPUTS */
        --swym-storefront-layout-input-color-bg: var(--swym-theme-layout-input-color-bg);
        --swym-storefront-layout-input-color-text: var(--swym-theme-layout-input-color-text);
        --swym-storefront-layout-input-border-color: var(--swym-theme-layout-input-border-color);
        --swym-storefront-layout-input-border-radius: var(--swym-theme-layout-input-border-radius);
        --swym-storefront-layout-input-border-style: var(--swym-theme-layout-input-border-style);
        --swym-storefront-layout-input-border-width: var(--swym-theme-layout-input-border-width);
        --swym-storefront-layout-input-box-shadow: var(--swym-theme-layout-input-box-shadow);
        --swym-storefront-layout-input-box-shadow-hover: var(--swym-theme-layout-input-box-shadow-hover);
        --swym-storefront-layout-input-box-shadow-focus: var(--swym-theme-layout-input-box-shadow-focus);
        --swym-storefront-layout-input-focus-border-color: var(--swym-theme-layout-input-focus-border-color);
        --swym-storefront-layout-input-focus-outline-color: var(--swym-theme-layout-input-focus-outline-color);
        --swym-storefront-layout-input-focus-outline-offset: var(--swym-theme-layout-input-focus-outline-offset);
        --swym-storefront-layout-input-focus-outline-style: var(--swym-theme-layout-input-focus-outline-style);
        --swym-storefront-layout-input-focus-outline-width: var(--swym-theme-layout-input-focus-outline-width);
        --swym-storefront-layout-input-transition: var(--swym-theme-layout-input-transition);
      }
    `;
      }
      ...
      function inject() {
        try {
          ...
          if (!family) return; // Unknown theme — keep existing hardcoded :root values

          // When block settings are enabled, the merchant controls colors via the
          // theme editor. Skip the entire CSS mapping (theme vars + bridge) so
          // block settings and :root defaults are used — matching pre-mapping behavior.
          const blockSettingsEnabled = window.SwymStorefrontLayoutContext?.BlockSettingsEnabled || false;
          if (blockSettingsEnabled) return;

          const themeVars = family === 'dawn' ? getDawnThemeVariables() : getHorizonThemeVariables();
          const bridgeVars = getBridgeVariables();

          const style = document.createElement('style');
          style.setAttribute('data-swym-theme-mapping', family);
          style.textContent = themeVars + bridgeVars;
          document.head.appendChild(style);
        } catch (e) {
          // Silently fail — fallback to hardcoded :root values
        }
      }
    ...
    SwymStorefrontLayoutExtensions.InstrumentActionCodes = {
      ...
      // Saved For You
      StorefrontLayoutSFYTabViewed: 724,
      StorefrontLayoutSFYItemAddedToCart: 725,
      StorefrontLayoutSFYItemRemoved: 726,
      StorefrontLayoutSFYItemMovedToWishlist: 727,
      // AW Login Nudge
      StorefrontLayoutAWNudge1Shown: 728,
      StorefrontLayoutAWNudge1LoginClicked: 729,
      StorefrontLayoutAWNudge1Dismissed: 730,
      StorefrontLayoutAWNudge2Shown: 731,
      StorefrontLayoutAWNudge2LoginClicked: 732,
      StorefrontLayoutAWNudge2Dismissed: 733,
      // Explicit WL Login Nudge
      StorefrontLayoutExplicitNudge1Shown: 734,
      StorefrontLayoutExplicitNudge1LoginClicked: 735,
      StorefrontLayoutExplicitNudge1Dismissed: 736,
      StorefrontLayoutExplicitNudge2Shown: 737,
      StorefrontLayoutExplicitNudge2LoginClicked: 738,
      StorefrontLayoutExplicitNudge2Dismissed: 739,
      // Education Layer
      EducationLayerToastShownCollections: 812,
      EducationLayerTooltipShownCollections: 813,
      EducationLayerTooltipDismissedCollections: 814,
      EducationLayerTooltipShownPDP: 815,
      EducationLayerTooltipDismissedPDP: 816,
      EducationLayerButtonTransitionShownPDP: 817,
      EducationLayerToastShownPDP: 818,
      EducationLayerToastCTAClicked: 819,
      EducationLayerSessionStopped: 820
    };
    ...
    SwymStorefrontLayoutExtensions.InstrumentUtmTerms = {
      ...
      EducationLayerEngagement: "education",
      StorefrontLayoutAWLoginNudgeEngagement: "aw_login_nudge",
      StorefrontLayoutExplicitLoginNudgeEngagement: "explicit_login_nudge"
    };
    ...
    class SwymWishlistCore extends HTMLElement {
      ...
      initUi() {
        ...
        // Inject theme-aware CSS variables (Dawn/Horizon) — runs once, no-op for unknown themes
        SwymStorefrontLayoutExtensions.ThemeCSSMapping.inject();
        window.addEventListener('hashchange', () => {
          this.checkForSwymDrawerHash();
        });

        document.addEventListener(SwymStorefrontLayoutContext?.CustomEvents?.LayoutInitialized, () => {
          SwymStorefrontLayoutContext?.swat.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutInitialized, {
            "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutUiEngagement
          });
          this.checkForSwymDrawerHash();
          this.initElement();
      ...
      instrumentLayoutType() {
        ...
        if (StorefrontLayoutType === 'as-drawer') {
          SwymStorefrontLayoutContext?.swat.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutSideDrawerRendered, {
            "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutUiEngagement
          });
        } else if (StorefrontLayoutType === 'as-modal') {
          SwymStorefrontLayoutContext?.swat.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutModalRendered, {
            "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutUiEngagement
          });
        } else if (StorefrontLayoutType === 'as-section') {
          SwymStorefrontLayoutContext?.swat.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutPageRendered, {
            "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutUiEngagement
          });
        }
      }
      ...
      loadStrings() {
        try {
          ...
          const {
            ...
            ThemeAppStorefrontLayoutEmptySingleLoggedInTitle,
            ThemeAppStorefrontLayoutEmptySingleLoggedInDescription,
            ThemeAppStorefrontLayoutEmptySingleLoggedInBullet1,
            ThemeAppStorefrontLayoutEmptySingleLoggedInBullet2,
            ThemeAppStorefrontLayoutEmptySingleLoggedInBullet3,
            ThemeAppStorefrontLayoutEmptySingleLoggedInCTA,

            ThemeAppStorefrontLayoutEmptyMultiLoggedInTitle,
            ThemeAppStorefrontLayoutEmptyMultiLoggedInDescription,
            ThemeAppStorefrontLayoutEmptyMultiLoggedInCreateTitle,
            ThemeAppStorefrontLayoutEmptyMultiLoggedInCreateDescription,
            ThemeAppStorefrontLayoutEmptyMultiLoggedInBrowseLabel,
            ThemeAppStorefrontLayoutEmptyMultiLoggedInCTA,
            ThemeAppStorefrontLayoutEmptyMultiLoggedInTag1,
            ThemeAppStorefrontLayoutEmptyMultiLoggedInTag2,
            ThemeAppStorefrontLayoutEmptyMultiLoggedInTag3,
            ThemeAppStorefrontLayoutEmptyMultiLoggedInTag4,
            ThemeAppStorefrontLayoutEmptyMultiLoggedInTag5,

            ThemeAppStorefrontLayoutEmptyGuestTitle,
            ThemeAppStorefrontLayoutEmptyGuestDescription,
            ThemeAppStorefrontLayoutEmptyGuestSingleNotice,
            ThemeAppStorefrontLayoutEmptyGuestMultiNotice,
            ThemeAppStorefrontLayoutEmptyGuestLoginCTA,
            ThemeAppStorefrontLayoutEmptyGuestBrowseCTA,
            ThemeAppStorefrontLayoutEmptyGuestSignupText,
            ThemeAppStorefrontLayoutEmptyGuestSignupCTA,

            ThemeAppStorefrontLayoutEmptySflTitle,
            ThemeAppStorefrontLayoutEmptySflDescription,
            ThemeAppStorefrontLayoutEmptySflStep1Title,
            ThemeAppStorefrontLayoutEmptySflStep1Description,
            ThemeAppStorefrontLayoutEmptySflStep2Title,
            ThemeAppStorefrontLayoutEmptySflStep2Description,
            ThemeAppStorefrontLayoutEmptySflLoggedInNotice,
            ThemeAppStorefrontLayoutEmptySflGuestNotice,
            ThemeAppStorefrontLayoutEmptySflGoToCartCTA,
            ThemeAppStorefrontLayoutEmptySflLoginCTA,
            ThemeAppStorefrontLayoutEmptySflSignupText,
            ThemeAppStorefrontLayoutEmptySflSignupCTA,

            ThemeAppStorefrontLayoutTabSavedForYou,

            // Drawer UI Polish — personalized titles and share footer (single wishlist mode)
            ThemeAppStorefrontLayoutPersonalizedTitle,
            ThemeAppStorefrontLayoutPersonalizedSfyTitle,
            ThemeAppStorefrontLayoutShareCTA,
            ThemeAppStorefrontLayoutShareThisListCTA,
            ThemeAppStorefrontLayoutLoginToSaveCTA,

            // Auto-Wishlist login nudge strings (corner notification)
            ThemeAppStorefrontLayoutAwNudgeMessage,
            ThemeAppStorefrontLayoutAwNudgeMessageSub,
            ThemeAppStorefrontLayoutAwNudgeLoginCTA,
            ThemeAppStorefrontLayoutAwNudgeDismiss,
          } = RetailerStrings;

          SwymStorefrontLayoutContext.Strings = {
            ...SwymStorefrontLayoutContext?.Strings,
            ...(ThemeAppStorefrontLayoutTitle ? {
              'title': ThemeAppStorefrontLayoutTitle
            } : {}),
            ...(VariantSelectorBtnText ? {
              'VariantSelectorBtnText': VariantSelectorBtnText
            } : {}),
            ...(ThemeAppStorefrontLayoutAddToCartCTA ? {
              'addToCart': ThemeAppStorefrontLayoutAddToCartCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutAddedToCartCTA ? {
              'addedToCart': ThemeAppStorefrontLayoutAddedToCartCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutAddingToCartCTA ? {
              'addingToCart': ThemeAppStorefrontLayoutAddingToCartCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutSoldoutCTA ? {
              'soldOut': ThemeAppStorefrontLayoutSoldoutCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutViewCartCTA ? {
              'viewCartCta': ThemeAppStorefrontLayoutViewCartCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutMoveToCartCTA ? {
              'moveToCartCta': ThemeAppStorefrontLayoutMoveToCartCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutMoveingToCartCTA ? {
              'movingToCartCta': ThemeAppStorefrontLayoutMoveingToCartCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutRemoveSFLItemCTA ? {
              'removeSflItemCta': ThemeAppStorefrontLayoutRemoveSFLItemCTA
            } : {}),

            ...(ThemeAppStorefrontLayoutLoginHeading ? {
              'loginHeading': ThemeAppStorefrontLayoutLoginHeading
            } : {}),
            ...(ThemeAppStorefrontLayoutLoginMessage ? {
              'loginText': ThemeAppStorefrontLayoutLoginMessage
            } : {}),
            ...(ThemeAppStorefrontLayoutLoginButtonCTA ? {
              'loginButtonText': ThemeAppStorefrontLayoutLoginButtonCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutLoggedUserWelcomeMessage ? {
              'loggedUserWelcomeMessage': ThemeAppStorefrontLayoutLoggedUserWelcomeMessage
            } : {}),

            ...(ThemeAppStorefrontLayoutWishlistTitle ? {
              'wishlistTitle': ThemeAppStorefrontLayoutWishlistTitle
            } : {}),
            ...(ThemeAppStorefrontLayoutWishlistInfo ? {
              'wishlistInfo': ThemeAppStorefrontLayoutWishlistInfo
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyWishlistTitle ? {
              'emptyWishlistTitle': ThemeAppStorefrontLayoutEmptyWishlistTitle
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyWishlistDescription ? {
              'emptyWishlistDescription': ThemeAppStorefrontLayoutEmptyWishlistDescription
            } : {}),

            ...(ThemeAppStorefrontLayoutCollectionsTitle ? {
              'collectionTitle': ThemeAppStorefrontLayoutCollectionsTitle
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyCarouselCollectionsDescription ? {
              'emptyCarouselCollectionText': ThemeAppStorefrontLayoutEmptyCarouselCollectionsDescription
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyCollectionsTitle ? {
              'emptyCollectionText': ThemeAppStorefrontLayoutEmptyCollectionsTitle
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyCollectionsDescription ? {
              'emptyCollectionDescription': ThemeAppStorefrontLayoutEmptyCollectionsDescription
            } : {}),

            ...(ThemeAppStorefrontLayoutAddToCollectionsTitle ? {
              'addToCollectionTitle': ThemeAppStorefrontLayoutAddToCollectionsTitle
            } : {}),
            ...(ThemeAppStorefrontLayoutAddToCollectionCTA ? {
              'addToCollectionCta': ThemeAppStorefrontLayoutAddToCollectionCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutCreateCollectionCTA ? {
              'createCollectionCta': ThemeAppStorefrontLayoutCreateCollectionCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutSaveNewCollectionCTA ? {
              'saveNewCollectionCta': ThemeAppStorefrontLayoutSaveNewCollectionCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutRemoveItemCTA ? {
              'removeItemCta': ThemeAppStorefrontLayoutRemoveItemCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutRenameCollectionCTA ? {
              'renameCollectionCta': ThemeAppStorefrontLayoutRenameCollectionCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutDeleteCollectionCTA ? {
              'deleteCollectionCta': ThemeAppStorefrontLayoutDeleteCollectionCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutShareCollectionCTA ? {
              'shareCollectionCta': ThemeAppStorefrontLayoutShareCollectionCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutSaveCollectionCTA ? {
              'saveCollectionCta': ThemeAppStorefrontLayoutSaveCollectionCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutEditCollectionCTA ? {
              'editCollectionCta': ThemeAppStorefrontLayoutEditCollectionCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutShareCollectionTitle ? {
              'shareCollectionTitle': ThemeAppStorefrontLayoutShareCollectionTitle
            } : {}),
            ...(ThemeAppStorefrontLayoutShareCollectionMessage ? {
              'shareCollectionMessage': ThemeAppStorefrontLayoutShareCollectionMessage
            } : {}),
            ...(ThemeAppStorefrontLayoutSharedCollectionMessage ? {
              'sharedCollectionMessage': ThemeAppStorefrontLayoutSharedCollectionMessage
            } : {}),
            ...(ThemeAppStorefrontLayoutUpdateCollectionTitle ? {
              'updateCollectionTitle': ThemeAppStorefrontLayoutUpdateCollectionTitle
            } : {}),

            ...(ThemeAppStorefrontLayoutErrorMessageListNameRequired ? {
              'errorMessageListNameRequired': ThemeAppStorefrontLayoutErrorMessageListNameRequired
            } : {}),
            ...(ThemeAppStorefrontLayoutErrorMessageListNameRequired3Char ? {
              'errorMessageListNameRequire3Char': ThemeAppStorefrontLayoutErrorMessageListNameRequired3Char
            } : {}),
            ...(ThemeAppStorefrontLayoutErrorMessageListNameAlreadyExist ? {
              'errorMessageListNameAlreadyExist': ThemeAppStorefrontLayoutErrorMessageListNameAlreadyExist
            } : {}),

            ...(ThemeAppStorefrontLayoutNotificationMessageItemSaved ? {
              'notificationMessageItemSaved': ThemeAppStorefrontLayoutNotificationMessageItemSaved
            } : {}),
            ...(ThemeAppStorefrontLayoutNotificationMessageItemRemoved ? {
              'notificationMessageItemRemoved': ThemeAppStorefrontLayoutNotificationMessageItemRemoved
            } : {}),
            ...(ThemeAppStorefrontLayoutNotificationMessageAddedToCart ? {
              'notificationMessageAddedToCart': ThemeAppStorefrontLayoutNotificationMessageAddedToCart
            } : {}),
            ...(ThemeAppStorefrontLayoutNotificationMessageAddedToCollection ? {
              'notificationMessageAddedToCollection': ThemeAppStorefrontLayoutNotificationMessageAddedToCollection
            } : {}),
            ...(ThemeAppStorefrontLayoutNotificationMessageCollectionSaved ? {
              'notificationMessageCollectionSaved': ThemeAppStorefrontLayoutNotificationMessageCollectionSaved
            } : {}),
            ...(ThemeAppStorefrontLayoutNotificationMessageCollectionDeleted ? {
              'notificationMessageCollectionDeleted': ThemeAppStorefrontLayoutNotificationMessageCollectionDeleted
            } : {}),
            ...(ThemeAppStorefrontLayoutNotificationMessageCollectionUpdated ? {
            ...
            ...(ThemeAppStorefrontLayoutNotificationMessageSFLItemSaved ? {
              'notificationMessageSFLItemSaved': ThemeAppStorefrontLayoutNotificationMessageSFLItemSaved
            } : {}),
            ...(ThemeAppStorefrontLayoutNotificationMessageSFLItemRemoved ? {
              'notificationMessageSFLItemRemoved': ThemeAppStorefrontLayoutNotificationMessageSFLItemRemoved
            } : {}),
            ...(ThemeAppStorefrontLayoutNotificationMessageMovedToCart ? {
              'notificationMessageMovedToCart': ThemeAppStorefrontLayoutNotificationMessageMovedToCart
            } : {}),

            ...(ThemeAppStorefrontLayoutNotificationActionAddToCollection ? {
              'notificationActionAddToCollection': ThemeAppStorefrontLayoutNotificationActionAddToCollection
            } : {}),
            ...(ThemeAppStorefrontLayoutNotificationActionView ? {
              'notificationActionView': ThemeAppStorefrontLayoutNotificationActionView
            } : {}),
            ...(ThemeAppStorefrontLayoutNotificationActionViewCollection ? {
              'notificationActionViewCollection': ThemeAppStorefrontLayoutNotificationActionViewCollection
            } : {}),
            ...(ThemeAppStorefrontLayoutNotificationActionGoToCart ? {
              'notificationActionGoToCart': ThemeAppStorefrontLayoutNotificationActionGoToCart
            } : {}),

            ...(ThemeAppStorefrontLayoutTextItem ? {
              'item': ThemeAppStorefrontLayoutTextItem
            } : {}),
            ...(ThemeAppStorefrontLayoutTextItems ? {
              'items': ThemeAppStorefrontLayoutTextItems
            } : {}),

            ...(ThemeAppStorefrontLayoutTabWishlist ? {
              'tabWishlist': ThemeAppStorefrontLayoutTabWishlist
            } : {}),
            ...(ThemeAppStorefrontLayoutTabSavedForLater ? {
              'tabSavedForLater': ThemeAppStorefrontLayoutTabSavedForLater
            } : {}),
            tabSavedForYou: ThemeAppStorefrontLayoutTabSavedForYou || RetailerStrings?.AutoWishlistedTabText || 'Saved For You',
            ...(ThemeAppStorefrontLayoutSaveForLaterTitle ? {
              'savedForLaterTitle': ThemeAppStorefrontLayoutSaveForLaterTitle
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySavedForLaterTitle ? {
              'emptySavedForLaterTitle': ThemeAppStorefrontLayoutEmptySavedForLaterTitle
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySavedForLaterDescription ? {
              'emptySavedForLaterDescription': ThemeAppStorefrontLayoutEmptySavedForLaterDescription
            } : {}),

            ...(ThemeAppStorefrontLayoutEmptySingleLoggedInTitle ? {
              'emptySingleLoggedInTitle': ThemeAppStorefrontLayoutEmptySingleLoggedInTitle
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySingleLoggedInDescription ? {
              'emptySingleLoggedInDescription': ThemeAppStorefrontLayoutEmptySingleLoggedInDescription
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySingleLoggedInBullet1 ? {
              'emptySingleLoggedInBullet1': ThemeAppStorefrontLayoutEmptySingleLoggedInBullet1
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySingleLoggedInBullet2 ? {
              'emptySingleLoggedInBullet2': ThemeAppStorefrontLayoutEmptySingleLoggedInBullet2
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySingleLoggedInBullet3 ? {
              'emptySingleLoggedInBullet3': ThemeAppStorefrontLayoutEmptySingleLoggedInBullet3
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySingleLoggedInCTA ? {
              'emptySingleLoggedInCTA': ThemeAppStorefrontLayoutEmptySingleLoggedInCTA
            } : {}),

            ...(ThemeAppStorefrontLayoutEmptyMultiLoggedInTitle ? {
              'emptyMultiLoggedInTitle': ThemeAppStorefrontLayoutEmptyMultiLoggedInTitle
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyMultiLoggedInDescription ? {
              'emptyMultiLoggedInDescription': ThemeAppStorefrontLayoutEmptyMultiLoggedInDescription
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyMultiLoggedInCreateTitle ? {
              'emptyMultiLoggedInCreateTitle': ThemeAppStorefrontLayoutEmptyMultiLoggedInCreateTitle
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyMultiLoggedInCreateDescription ? {
              'emptyMultiLoggedInCreateDescription': ThemeAppStorefrontLayoutEmptyMultiLoggedInCreateDescription
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyMultiLoggedInBrowseLabel ? {
              'emptyMultiLoggedInBrowseLabel': ThemeAppStorefrontLayoutEmptyMultiLoggedInBrowseLabel
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyMultiLoggedInCTA ? {
              'emptyMultiLoggedInCTA': ThemeAppStorefrontLayoutEmptyMultiLoggedInCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyMultiLoggedInTag1 ? {
              'emptyMultiLoggedInTag1': ThemeAppStorefrontLayoutEmptyMultiLoggedInTag1
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyMultiLoggedInTag2 ? {
              'emptyMultiLoggedInTag2': ThemeAppStorefrontLayoutEmptyMultiLoggedInTag2
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyMultiLoggedInTag3 ? {
              'emptyMultiLoggedInTag3': ThemeAppStorefrontLayoutEmptyMultiLoggedInTag3
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyMultiLoggedInTag4 ? {
              'emptyMultiLoggedInTag4': ThemeAppStorefrontLayoutEmptyMultiLoggedInTag4
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyMultiLoggedInTag5 ? {
              'emptyMultiLoggedInTag5': ThemeAppStorefrontLayoutEmptyMultiLoggedInTag5
            } : {}),

            ...(ThemeAppStorefrontLayoutEmptyGuestTitle ? {
              'emptyGuestTitle': ThemeAppStorefrontLayoutEmptyGuestTitle
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyGuestDescription ? {
              'emptyGuestDescription': ThemeAppStorefrontLayoutEmptyGuestDescription
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyGuestSingleNotice ? {
              'emptyGuestSingleNotice': ThemeAppStorefrontLayoutEmptyGuestSingleNotice
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyGuestMultiNotice ? {
              'emptyGuestMultiNotice': ThemeAppStorefrontLayoutEmptyGuestMultiNotice
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyGuestLoginCTA ? {
              'emptyGuestLoginCTA': ThemeAppStorefrontLayoutEmptyGuestLoginCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyGuestBrowseCTA ? {
              'emptyGuestBrowseCTA': ThemeAppStorefrontLayoutEmptyGuestBrowseCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyGuestSignupText ? {
              'emptyGuestSignupText': ThemeAppStorefrontLayoutEmptyGuestSignupText
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptyGuestSignupCTA ? {
              'emptyGuestSignupCTA': ThemeAppStorefrontLayoutEmptyGuestSignupCTA
            } : {}),

            ...(ThemeAppStorefrontLayoutEmptySflTitle ? {
              'emptySflTitle': ThemeAppStorefrontLayoutEmptySflTitle
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySflDescription ? {
              'emptySflDescription': ThemeAppStorefrontLayoutEmptySflDescription
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySflStep1Title ? {
              'emptySflStep1Title': ThemeAppStorefrontLayoutEmptySflStep1Title
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySflStep1Description ? {
              'emptySflStep1Description': ThemeAppStorefrontLayoutEmptySflStep1Description
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySflStep2Title ? {
              'emptySflStep2Title': ThemeAppStorefrontLayoutEmptySflStep2Title
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySflStep2Description ? {
              'emptySflStep2Description': ThemeAppStorefrontLayoutEmptySflStep2Description
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySflLoggedInNotice ? {
              'emptySflLoggedInNotice': ThemeAppStorefrontLayoutEmptySflLoggedInNotice
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySflGuestNotice ? {
              'emptySflGuestNotice': ThemeAppStorefrontLayoutEmptySflGuestNotice
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySflGoToCartCTA ? {
              'emptySflGoToCartCTA': ThemeAppStorefrontLayoutEmptySflGoToCartCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySflLoginCTA ? {
              'emptySflLoginCTA': ThemeAppStorefrontLayoutEmptySflLoginCTA
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySflSignupText ? {
              'emptySflSignupText': ThemeAppStorefrontLayoutEmptySflSignupText
            } : {}),
            ...(ThemeAppStorefrontLayoutEmptySflSignupCTA ? {
              'emptySflSignupCTA': ThemeAppStorefrontLayoutEmptySflSignupCTA
            } : {}),

            // Auto-Wishlist login nudge strings
            // Fallback chain: merchant Liquid override → SDK locale strings (strings/*.js via retailerSettings) → empty string
            awNudgeMessage: ThemeAppStorefrontLayoutAwNudgeMessage || RetailerStrings?.ThemeAppStorefrontLayoutAwNudgeMessage || '',
            awNudgeMessageSub: ThemeAppStorefrontLayoutAwNudgeMessageSub || RetailerStrings?.ThemeAppStorefrontLayoutAwNudgeMessageSub || '',
            awNudgeLoginCTA: ThemeAppStorefrontLayoutAwNudgeLoginCTA || RetailerStrings?.ThemeAppStorefrontLayoutAwNudgeLoginCTA || '',
            awNudgeDismiss: ThemeAppStorefrontLayoutAwNudgeDismiss || RetailerStrings?.ThemeAppStorefrontLayoutAwNudgeDismiss || '',
            // Explicit WL login nudge — only bold message differs; sub-message, CTA and dismiss reuse AW strings
            explicitNudgeMessage: RetailerStrings?.ThemeAppStorefrontLayoutExplicitNudgeMessage || '',

            // Drawer UI Polish — personalized titles and share footer
            personalizedTitle: ThemeAppStorefrontLayoutPersonalizedTitle || RetailerStrings?.ThemeAppStorefrontLayoutPersonalizedTitle || "{{name}}'s Wishlist",
            personalizedSfyTitle: ThemeAppStorefrontLayoutPersonalizedSfyTitle || RetailerStrings?.ThemeAppStorefrontLayoutPersonalizedSfyTitle || 'Saved For {{name}}',
            shareCta: ThemeAppStorefrontLayoutShareCTA || RetailerStrings?.ThemeAppStorefrontLayoutShareCTA || 'Share',
            shareThisListCta: ThemeAppStorefrontLayoutShareThisListCTA || RetailerStrings?.ThemeAppStorefrontLayoutShareThisListCTA || 'Share This List',
            loginToSaveCta: ThemeAppStorefrontLayoutLoginToSaveCTA || RetailerStrings?.ThemeAppStorefrontLayoutLoginToSaveCTA || 'Login to Save',
      ...
      async checkForSwymDrawerHash() {
        const {
          ...
          StorefrontLayoutAsSectionPageURL
        } = SwymStorefrontLayoutContext?.Settings;
        const currentHash = window.location.hash;
        const swymOpenParam = new URLSearchParams(window.location.search).get('swymOpen');
        const shouldOpenFromQuery = swymOpenParam === 'wishlist';

        if (!SwymStorefrontLayoutExtensions.SwymStorefrontLayout) {
          SwymStorefrontLayoutExtensions.SwymStorefrontLayout = document.querySelector('swym-storefront-layout #swym-storefront-layout');
        }

        if (!currentHash && !shouldOpenFromQuery && SwymStorefrontLayoutExtensions.SwymStorefrontLayout?.isOpen) {
        ...
        if (currentHash === SwymStorefrontLayoutContext?.StorefrontLayoutUrls?.List || shouldOpenFromQuery) {
      ...
      attachSwymEventListeners(swat) {
        ...
        swat?.evtLayer?.addEventListener(swat?.JSEvents?.addedToWishlist, (event) => {
          if (SwymStorefrontLayoutContext._sfyMoveToWishlist) return;
          SwymStorefrontLayoutExtensions?.refreshWishList();
          ...
          swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutItemAddedToList, {
            "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutItemInteractions,
            epi,
            empi
          });
        ...
        swat?.evtLayer?.addEventListener(swat?.JSEvents?.removedFromWishlist, (event) => {
          ...
          if (epi && empi) {
            ...
            swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutItemRemovedFromList, {
              "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutItemInteractions,
              epi,
              empi
            });
        ...
        if (window._swat?.retailerSettings?.SFL?.SFLFeatureEnabled) {
          swat.evtLayer.addEventListener(swat.JSEvents.addedToSFL, (event) => {
            ...
            window._swat.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutSFLItemAddedToList, {
              "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutItemInteractions,
              epi,
              empi
            });
          ...
          swat.evtLayer.addEventListener(swat.JSEvents.removedFromSFL, (event) => {
            ...
            window._swat.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutSFLItemRemovedFromList, {
              "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutItemInteractions,
              epi,
              empi
            });
    ...
    class SwymStorefrontLayoutAsModal extends SwymWishlistCore {
      ...
      <div id="swym-storefront-layout-container" class="swym-storefront-layout-drawer swym-storefront-layout-hide-view" tabindex="-1" role="dialog" aria-modal="true" aria-labelledby="swym-wishlist-title">
        ...
        <div class="swym-storefront-layout-layout">
          ...
          <button id="swym-storefront-layout-close-drawer-button" aria-label="${SwymStorefrontLayoutContext?.Strings?.closeDrawer || 'Close'}" class="swym-storefront-layout-close-button">
            <svg xmlns="http://www.w3.org/2000/svg" width="27" height="27" viewBox="0 0 27 27" fill="none" aria-hidden="true">
              <path d="M0 13.5C0 6.04416 6.04416 0 13.5 0C20.9558 0 27 6.04416 27 13.5C27 20.9558 20.9558 27 13.5 27C6.04416 27 0 20.9558 0 13.5Z" fill="none" fill-opacity="0.5"/>
              <path d="M16.0245 17.8326C16.2477 18.0558 16.6095 18.0558 16.8326 17.8326C17.0558 17.6095 17.0558 17.2477 16.8326 17.0245L13.8081 14L16.8326 10.9755C17.0558 10.7523 17.0558 10.3905 16.8326 10.1674C16.6095 9.94421 16.2477 9.94421 16.0245 10.1674L13 13.1919L9.97549 10.1674C9.75233 9.94421 9.39052 9.94421 9.16737 10.1674C8.94421 10.3905 8.94421 10.7523 9.16737 10.9755L12.1919 14L9.16737 17.0245C8.94421 17.2477 8.94421 17.6095 9.16737 17.8326C9.39052 18.0558 9.75233 18.0558 9.97549 17.8326L13 14.8081L16.0245 17.8326Z" fill="currentColor" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </button>
    ...
    `;
      ...
      open() {
        ...
        if (this.elements?.drawer) {
          ...
          window._swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutOpened, {
            "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutUiEngagement
          });
        }
      }
      ...
      close() {
        ...
        if (this.elements?.drawer) {
          ...
          window._swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutClosed, {
            "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutUiEngagement
          });
        }
      }
    ...
    <div class="swym-storefront-layout-collection-carousel-container">
      ...
      <div class="swym-storefront-layout-carousel-container" role="region" aria-label="${SwymStorefrontLayoutContext?.Strings?.collectionTitle}">
        <button class="swym-storefront-layout-carousel-btn swym-storefront-layout-carousel-prev-btn" aria-label="${SwymStorefrontLayoutContext?.Strings?.scrollLeft || 'Scroll Left'}">
        ...
        <button class="swym-storefront-layout-carousel-btn swym-storefront-layout-carousel-next-btn" aria-label="${SwymStorefrontLayoutContext?.Strings?.scrollRight || 'Scroll Right'}">
          <svg fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" class="swym-storefront-layout-scroll-button-icon" aria-hidden="true"><path d="m13.333 8 8 8-8 8" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"></path></svg>
        </button>
      </div>
    </div>
    `
      ...
      renderCollections() {
        ...
        if (collectionsContainer) {
          ...
          window._swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutCollectionRendered, {
            "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutUiEngagement
          });
            ...
            collectionsContainer.innerHTML = this.collections
              .map(
                (collection) => {
                  ...
                  const listImagesHtml = Array.from({
                      length: 3
                    })
                    .map((_, i) => {
                  ...
                  <div class="${i === 0 ? 'swym-storefront-layout-collection-grid-item-primary-image' : 'swym-storefront-layout-collection-grid-item-secondary-image'} ">
                    ${item?.iu ? `<img src="${item ? item.iu : null}" alt="${item?.dt ? swymEscAttr(item?.dt) : 'product Image'}" class="swym-storefront-layout-collection-grid-item-image" />` : `<div class="swym-storefront-layout-collection-grid-item-image" ></div>`}
                  </div>
                `;
                    })
                    .join("");
              ...
              <div class="swym-storefront-layout-collection-grid-item" data-lid="${lid}" role="listitem" aria-label="${lname}">
                ...
                <div id="swym-storefront-layout-collection-grid-item-option-button" class="swym-storefront-layout-collection-grid-item-option-button" role="button" tabindex="0" aria-haspopup="dialog" aria-expanded="false" aria-label="More options for ${lname}">
      ...
      attachEventListnerToCollectionGrid() {
        this.collections.forEach((collection) => {
          ...
          collectionItem?.addEventListener("click", (event) => {
            window._swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutCollectionOpened, {
              "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutUiEngagement
            });
    ...
    class SwymStorefrontLayoutWishlistItem extends HTMLElement {
      ...
      renderOptionButton() {
        ...
        const isDrawer = SwymStorefrontLayoutContext?.Settings?.StorefrontLayoutType === 'as-drawer';

        // Drawer UI Polish: In drawer + single wishlist mode, show trash icon instead of 3-dot menu
        // Clicking opens tooltip with "Remove from wishlist" confirmation (no direct delete)
        if (isDrawer && !shouldShowMultiple) {
          if (this.listItemType === SwymStorefrontLayoutContext?.ListItemType?.SavedForYouItem ||
            SwymStorefrontLayoutContext?.checkUserCanEditList(this.list || this.sflList)) {
            const ariaLabel = this.listItemType === SwymStorefrontLayoutContext?.ListItemType?.SavedForYouItem ?
              (SwymStorefrontLayoutContext?.Strings?.removeItemCta || 'Remove from Saved For You') :
              (SwymStorefrontLayoutContext?.Strings?.removeItemCta || 'Remove item');
            return `
      <button id="swym-storefront-layout-grid-item-option-button" class="swym-storefront-layout-grid-item-option-button swym-storefront-layout-delete-btn" tabindex="0" role="button" aria-label="${ariaLabel}" aria-haspopup="dialog" aria-expanded="false">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none" aria-hidden="true" focusable="false">
          <path d="M11.5 9V13.25M8.5 9V13.25M5.75 12.2V6H14.25C14.25 8.421 14.25 9.779 14.25 12.2C14.25 13.053 14.25 13.647 14.212 14.11C14.175 14.563 14.106 14.824 14.005 15.021C13.7822 15.4476 13.4408 15.8008 13.022 16.038C12.825 16.138 12.564 16.208 12.111 16.245C11.648 16.282 11.054 16.283 10.201 16.283H9.801C8.948 16.283 8.354 16.283 7.891 16.245C7.438 16.208 7.177 16.139 6.98 16.038C6.56083 15.8009 6.21911 15.4478 5.996 15.021C5.896 14.824 5.826 14.563 5.789 14.11C5.75 13.647 5.75 13.053 5.75 12.2Z" stroke="currentColor" stroke-linecap="round"/>
          <path d="M4.25 6H15.75M8 5.25C8 4.71957 8.21071 4.21086 8.58579 3.83579C8.96086 3.46071 9.46957 3.25 10 3.25C10.5304 3.25 11.0391 3.46071 11.4142 3.83579C11.7893 4.21086 12 4.71957 12 5.25" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
      </button>
    `;
          }
        ...
        // SavedForYouItem only supports remove — show option button for that
        if (this.listItemType === SwymStorefrontLayoutContext?.ListItemType?.SavedForYouItem) {
          return `
      <button id="swym-storefront-layout-grid-item-option-button" class="swym-storefront-layout-grid-item-option-button" tabindex="0" role="button" aria-label="Remove from Saved For You">
    ...
    `;
        ...
        if (
          ...
          return `
      <button id="swym-storefront-layout-grid-item-option-button" class="swym-storefront-layout-grid-item-option-button" tabindex="0" role="button" aria-label="Open item options" aria-haspopup="dialog" aria-expanded="false">
        <svg xmlns="http://www.w3.org/2000/svg" width="3" height="13" viewBox="0 0 3 13" fill="currentcolor" aria-hidden="true" focusable="false">
          <circle cx="1.5" cy="1.50014" r="1.5" fill="#B1B7C3"/>
          <circle cx="1.5" cy="6.50014" r="1.5" fill="#B1B7C3"/>
          <circle cx="1.5" cy="11.5001" r="1.5" fill="#B1B7C3"/>
    ...
    `;
      ...
      renderItem() {
        ...
        const isAutoWishlist = this.listItemType === SwymStorefrontLayoutContext?.ListItemType?.SavedForYouItem &&
          matchingItem?.source === "auto-wishlist";
      ...
      <div class="swym-storefront-layout-grid-item">
        <a href="${localizedUrl}" class="swym-storefront-layout-grid-item-image-container">
          <img src="${this.getProductImage()}" alt="${swymEscAttr(this.item?.productData?.title || this.item?.dt) || 'Product image'}" class="swym-storefront-layout-grid-item-image" />
        </a>
...
${this.renderVariants()}
          ...
          <div class="swym-storefront-layout-grid-item-action-container">
            ${!isHidden && this.isSfsQuantityFeatureEnabled() ? `<div class="swym-storefront-layout-quantity-container">
              <div class="swym-storefront-layout-quantity-selector">
                <button type="button" class="swym-storefront-layout-quantity-btn swym-storefront-layout-quantity-minus" data-action="quantity-decrease" aria-label="${SwymStorefrontLayoutContext?.Strings?.decreaseQuantity || 'Decrease quantity'}">−</button>
                ...
                <button type="button" class="swym-storefront-layout-quantity-btn swym-storefront-layout-quantity-plus" data-action="quantity-increase" aria-label="${SwymStorefrontLayoutContext?.Strings?.increaseQuantity || 'Increase quantity'}">+</button>
              </div>
            </div>` : ''}
          ${isAutoWishlist && isFirstVariant && !noVariant && !isHidden && !this._variantConfirmed ?
            `<button class="swym-variant-selector" id="swym-variant-selector">
              ...
              <svg xmlns="http://www.w3.org/2000/svg" width="7" height="11" viewBox="0 0 7 11" fill="none">
                            <path id="Vector 6" d="M1 9.5L5.44444 5.5L1 1.5" stroke="currentColor" stroke-width="1.5"/>
                          </svg>
            </button>` : ``}
            ${isAutoWishlist && !isHidden && (
                !isFirstVariant || noVariant || (isFirstVariant && !noVariant && this._variantConfirmed)
              ) ?
              `<div class="swym-sfy-action-row">
                ...
                <button class="swym-sfy-add-to-wishlist-btn ${SwymStorefrontLayoutContext?.Settings?.StorefrontLayoutType === 'as-drawer' ? 'swym-sfy-heart-btn' : ''}" data-action="add-to-wishlist" aria-label="${SwymStorefrontLayoutContext?.Strings?.sfyAddToWishlistCta || 'Add to Wishlist'}">
                  ${SwymStorefrontLayoutContext?.Settings?.StorefrontLayoutType === 'as-drawer' ? (SwymStorefrontLayoutExtensions?.getWishlistIconSvg?.('unfilled', { width: '17', height: '16' }) || '') : (SwymStorefrontLayoutContext?.Strings?.sfyAddToWishlistCta || 'Add to Wishlist')}
                </button>
              </div>` : ""
            }
            ${isAutoWishlist && !isHidden && isFirstVariant && !noVariant && !this._variantConfirmed ?
              `<button class="swym-sfy-add-to-wishlist-btn ${SwymStorefrontLayoutContext?.Settings?.StorefrontLayoutType === 'as-drawer' ? 'swym-sfy-heart-btn' : ''}" data-action="add-to-wishlist" aria-label="${SwymStorefrontLayoutContext?.Strings?.sfyAddToWishlistCta || 'Add to Wishlist'}">
                ${SwymStorefrontLayoutContext?.Settings?.StorefrontLayoutType === 'as-drawer' ? (SwymStorefrontLayoutExtensions?.getWishlistIconSvg?.('unfilled', { width: '17', height: '16' }) || '') : (SwymStorefrontLayoutContext?.Strings?.sfyAddToWishlistCta || 'Add to Wishlist')}
              </button>` : ""
            }
    ...
    `;
      ...
      initItemElements() {
        this.elements = {
          ...
          AddToWishlistButton: this.querySelector('[data-action="add-to-wishlist"]'),
      ...
      addEventListeners() {
        ...
        this.elements.OptionButton?.addEventListener('click', (event) => {
          if (SwymStorefrontLayoutExtensions?.SwymStorefrontLayoutActionTooltip?.isTooltipOpen && SwymStorefrontLayoutExtensions?.SwymStorefrontLayoutActionTooltip?.currentTarget === event.target) {
            ...
            if (this.listItemType === SwymStorefrontLayoutContext?.ListItemType?.WishlistItem || this.listItemType === SwymStorefrontLayoutContext?.ListItemType?.CollectionItem) {
              ...
              SwymStorefrontLayoutExtensions?.SwymStorefrontLayoutActionTooltip?.showOnTarget(event.target);
            } else if (this.listItemType === SwymStorefrontLayoutContext?.ListItemType?.SavedForYouItem) {
              SwymStorefrontLayoutExtensions?.SwymStorefrontLayoutActionTooltip?.setData({
                listId: this.list?.lid,
                list: this.list,
                item: this.item,
                selectedVariantId: this.selectedVariant?.id,
                collections: this.collections,
                actionType: SwymStorefrontLayoutContext?.ActionTypes.ManageSfyListItem,
                canCreateLists: this.canCreateLists
              });
              SwymStorefrontLayoutExtensions?.SwymStorefrontLayoutActionTooltip?.showOnTarget(event.target);
            }
        ...
        this.elements.AddToWishlistButton?.addEventListener('click', (event) => {
          event.preventDefault();
          this.handleMoveToWishlist();
        });
        this.elements.AddToWishlistButton?.addEventListener('keydown', (e) => {
          if (e.key === 'Enter' || e.key === ' ') {
            e.preventDefault();
            this.handleMoveToWishlist();
          }
      ...
      async handleDirectRemove() {
        try {
          ...
          const lid = (this.list || this.sflList)?.lid;

          if (this.listItemType === SwymStorefrontLayoutContext?.ListItemType?.SavedForYouItem) {
            // SFY item: optimistic UI update + API call (same pattern as addSfyOptionEventListeners)
            SwymStorefrontLayoutContext.updateAllListContents((listcontents = []) =>
              listcontents.filter(item => String(item.epi) !== String(epi))
            );
            if (SwymStorefrontLayoutContext?.CustomEvents?.WishlistFetched) {
              document.dispatchEvent(new CustomEvent(SwymStorefrontLayoutContext.CustomEvents.WishlistFetched, {
                detail: {
                  defaultList: SwymStorefrontLayoutContext.DefaultList,
                  collections: SwymStorefrontLayoutContext.collections,
                  lists: SwymStorefrontLayoutContext.lists
            ...
            await SwymStorefrontLayoutAPI?.SwymWLAsyncApis?.removeProductFromList({
              epi,
              empi,
              du
            }, lid);
            SwymStorefrontLayoutExtensions?.refreshWishList();
            window._swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutSFYItemRemoved, {
              "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutItemInteractions,
              epi,
              empi
            });
          } else if (this.listItemType === SwymStorefrontLayoutContext?.ListItemType?.SaveForLaterItem) {
            // SFL item: use removeProductFromSFL (same pattern as addSflOptionEventListeners)
            let selectedEpi = this.selectedVariant?.id || epi;
            await SwymStorefrontLayoutAPI?.SwymSFLAsyncApis?.removeProductFromSFL({
              epi: selectedEpi,
              empi,
              du
            }, SwymStorefrontLayoutContext?.sflListId);
            this.remove();
            window._swat?.triggerSwymEvent(window._swat?.JSEvents?.removedFromSFL, this.item);
          } else {
            // Regular wishlist item: use same pattern as removeFromListBtn in addOptionEventListeners
            let selectedEpi = this.selectedVariant?.id || epi;
            await SwymStorefrontLayoutAPI?.SwymWLAsyncApis?.removeProductFromList({
              epi: selectedEpi,
              empi,
            ...
            this.remove();
            SwymStorefrontLayoutExtensions?.refreshWishList();
            window._swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutItemRemovedFromList, {
              "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutItemInteractions,
              epi: selectedEpi,
              empi
            });
          ...
          window._swat?.utils.error('Failed to remove product:', error);
        }
      }

      async handleMoveToWishlist() {
        try {
          const {
            empi,
            du
          } = this.item;
          const epi = this.selectedVariant?.id || this.item.epi;
          const lid = this.list?.lid;

          // Check multi-wishlist config
          const {
            shouldShowMultiple,
            canCreateLists
          } = SwymStorefrontLayoutContext.getMultiWishlistConfig();
          ...
          if (shouldShowMultiple) {
            // Multi-list: open AddToCollection drawer so user picks the target list
          ...
          // Single list: move to default wishlist directly
          const atwBtn = this.querySelector('[data-action="add-to-wishlist"]');
          if (atwBtn) {
            atwBtn.innerHTML = SwymStorefrontLayoutContext?.Strings?.addingToWishlist || 'Adding...';
            atwBtn.setAttribute('disabled', 'true');
          }

          // Instant local state update across ALL context references
          SwymStorefrontLayoutContext.updateAllListContents((listcontents = []) =>
            listcontents.map(item =>
              String(item.epi) === String(epi) ? {
                ...item,
                source: undefined
              } : item
            )
          ...
          // Switch to Wishlist tab and update UI immediately
          SwymStorefrontLayoutContext.SelectedLayoutView = SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.Wishlist;
          if (SwymStorefrontLayoutContext?.CustomEvents?.WishlistFetched) {
            document.dispatchEvent(new CustomEvent(SwymStorefrontLayoutContext.CustomEvents.WishlistFetched, {
              detail: {
                defaultList: SwymStorefrontLayoutContext.DefaultList,
          ...
          // Fire API calls THEN refresh; suppress duplicate notifications from SDK event listeners
          SwymStorefrontLayoutContext._sfyMoveToWishlist = true;
          try {
            await SwymStorefrontLayoutAPI?.SwymWLAsyncApis?.removeProductFromList({
              epi,
              empi,
            ...
            SwymStorefrontLayoutContext._sfyMoveToWishlist = false;
            SwymStorefrontLayoutExtensions?.refreshWishList();
          }

          window._swat?.instrumentV3(
            SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutSFYItemMovedToWishlist, {
              "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutItemInteractions,
              epi,
              empi
            }
          ...
          window._swat?.utils.error('[SFY] Move to wishlist failed', err);
        }
      }
        ...
        _swat.api.replayAddToCart({
          ...
          (success) => {
            ...
            SwymStorefrontLayoutExtensions?.Notification?.setMessage({
              ...
              image: this.item.iu,
              product: this.item,
              title: this.item?.productData?.title || this.item?.dt,
              duration: SwymStorefrontLayoutContext?.Settings?.StorefrontLayoutNotificationDuration,
              action: {
            ...
            if (this.listItemType === SwymStorefrontLayoutContext?.ListItemType?.SavedForYouItem) {
              window._swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutSFYItemAddedToCart, {
                "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutCartConversion,
                epi,
                empi
              });
            } else {
              window._swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutItemAddedToCart, {
                "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutCartConversion,
                epi,
                empi
              });
      ...
      handleMoveToCart(event) {
        ...
        window._swat?.api.replayAddToCart({
            empi: +empi,
            du: du,
            qty: quantity
          },
          ...
          async (success) => {
              try {
                ...
                window._swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutSFLItemMovedToCart, {
                  "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutCartConversion,
                  epi,
                  empi
                });
      ...
      async handleVariantSelectionChange() {
        ...
        if (selectedVariant) {
          ...
          if (SwymStorefrontLayoutContext?.checkUserCanEditList(this.list)) {
            ...
            window._swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutVariantChanged, {
              "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutItemInteractions,
              epi,
              empi
            });
            await SwymStorefrontLayoutAPI?.SwymWLAsyncApis?.updateProductFromList({
              epi,
              empi,
              du,
              cprops: {
    ...
    `
      ...
      async renderWishlist() {
        if (this.wishlist?.listcontents?.length && this.elements.itemsContainer) {
          ...
          window._swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutWishlistItemsRendered, {
            "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutUiEngagement
          });
          ...
          const isLoggedIn = SwymStorefrontLayoutContext?.isShopperLoggedIn;
          const {
            shouldShowMultiple
          } = SwymStorefrontLayoutContext.getMultiWishlistConfig();
          const rootUrl = (window.Shopify?.routes?.root || '/').replace(/\/$/, '');
          const accountUrl = `${rootUrl}/account`;
          const homeUrl = window.Shopify?.routes?.root || '/';

          let emptyStateHtml = '';
          let noticeText = '';

          if (!shouldShowMultiple && isLoggedIn) {
            // State 1: Single wishlist – Logged in (new rich design)
            emptyStateHtml = `
          <div class="swym-storefront-layout-empty-container swym-storefront-layout-empty-state-single-logged">
            <div class="swym-storefront-layout-empty-wishlist-container">
              <div class="swym-storefront-layout-empty-wishlist-content">
                <div class="swym-storefront-layout-empty-state-icon">
                  <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48" fill="none">
                    <circle cx="24" cy="24" r="24" fill="#F5F5F5"/>
                    <path d="M20 18H18C17.4696 18 16.9609 18.2107 16.5858 18.5858C16.2107 18.9609 16 19.4696 16 20V32C16 32.5304 16.2107 33.0391 16.5858 33.4142C16.9609 33.7893 17.4696 34 18 34H30C30.5304 34 31.0391 33.7893 31.4142 33.4142C31.7893 33.0391 32 32.5304 32 32V20C32 19.4696 31.7893 18.9609 31.4142 18.5858C31.0391 18.2107 30.5304 18 30 18H28M20 18V16C20 15.4696 20.2107 14.9609 20.5858 14.5858C20.9609 14.2107 21.4696 14 22 14H26C26.5304 14 27.0391 14.2107 27.4142 14.5858C27.7893 14.9609 28 15.4696 28 16V18M20 18H28M24 24V28M22 26H26" stroke="#9B9B9B" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                  <span class="swym-storefront-layout-empty-state-badge">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none">
                      <circle cx="7" cy="7" r="7" fill="#E8A0BF"/>
                      <path d="M7 4V10M4 7H10" stroke="white" stroke-width="1.5" stroke-linecap="round"/>
                    </svg>
                  </span>
                </div>
                <div class="swym-storefront-layout-empty-wishlist-title">${SwymStorefrontLayoutContext?.Strings?.emptySingleLoggedInTitle}</div>
                <div class="swym-storefront-layout-empty-wishlist-description">${SwymStorefrontLayoutContext?.Strings?.emptySingleLoggedInDescription}</div>
                <div class="swym-storefront-layout-empty-state-card">
                  <ul class="swym-storefront-layout-empty-state-bullet-list">
                    <li>${SwymStorefrontLayoutContext?.Strings?.emptySingleLoggedInBullet1}</li>
                    <li>${SwymStorefrontLayoutContext?.Strings?.emptySingleLoggedInBullet2}</li>
                    <li>${SwymStorefrontLayoutContext?.Strings?.emptySingleLoggedInBullet3}</li>
                  </ul>
                </div>
                <hr class="swym-storefront-layout-empty-state-divider" />
                <a href="${homeUrl}" class="swym-storefront-layout-empty-state-cta-button swym-storefront-layout-empty-state-start-browsing">
                  ${SwymStorefrontLayoutContext?.Strings?.emptySingleLoggedInCTA}
                  <svg xmlns="http://www.w3.org/2000/svg" width="14" height="9" viewBox="0 0 14 9" fill="none"><path d="M13.8536 4.85368C14.0488 4.65841 14.0488 4.34183 13.8536 4.14657L10.6716 0.964589C10.4763 0.769327 10.1597 0.769327 9.96447 0.964589C9.7692 1.15985 9.7692 1.47643 9.96447 1.6717L12.7929 4.50012L9.96447 7.32855C9.7692 7.52381 9.7692 7.84039 9.96447 8.03566C10.1597 8.23092 10.4763 8.23092 10.6716 8.03566L13.8536 4.85368ZM0.5 5.00012L13.5 5.00012L13.5 4.00012L0.5 4.00012L0.5 5.00012Z" fill="currentColor"/></svg>
                </a>
              </div>
            </div>
          </div>
        `;
          } else if (!shouldShowMultiple && !isLoggedIn) {
            // State 2: Single wishlist – Guest (new rich design)
            noticeText = SwymStorefrontLayoutContext?.Strings?.emptyGuestSingleNotice;

            emptyStateHtml = `
          <div class="swym-storefront-layout-empty-container swym-storefront-layout-empty-state-guest">
            <div class="swym-storefront-layout-empty-wishlist-container">
              <div class="swym-storefront-layout-empty-wishlist-content">
                <div class="swym-storefront-layout-empty-state-icon">
                  <svg xmlns="http://www.w3.org/2000/svg" width="56" height="56" viewBox="0 0 56 56" fill="none">
                    <circle cx="28" cy="28" r="28" fill="#F5F3FF"/>
                    <path d="M28 38L17.55 28.03C16.6264 27.0846 16.0453 25.8579 15.8955 24.5427C15.7457 23.2275 16.0356 21.8986 16.72 20.766C17.4045 19.6334 18.4445 18.7606 19.6772 18.2842C20.9099 17.8079 22.2674 17.7551 23.533 18.134C24.468 18.4101 25.3178 18.916 26.005 19.604L28 21.6L29.995 19.604C30.6822 18.916 31.532 18.4101 32.467 18.134C33.7326 17.7551 35.0901 17.8079 36.3228 18.2842C37.5555 18.7606 38.5955 19.6334 39.28 20.766C39.9644 21.8986 40.2543 23.2275 40.1045 24.5427C39.9547 25.8579 39.3736 27.0846 38.45 28.03L28 38Z" stroke="#8B7EC8" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" fill="#C8B8F0"/>
                  </svg>
                </div>
                <div class="swym-storefront-layout-empty-wishlist-title">${SwymStorefrontLayoutContext?.Strings?.emptyGuestTitle}</div>
                <div class="swym-storefront-layout-empty-wishlist-description">${SwymStorefrontLayoutContext?.Strings?.emptyGuestDescription}</div>
                <div class="swym-storefront-layout-empty-state-notice">
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
                    <circle cx="9" cy="9" r="8" stroke="#B45309" stroke-width="1.2"/>
                    <path d="M9 5V9L11 11" stroke="#B45309" stroke-width="1.2" stroke-linecap="round"/>
                  </svg>
                  <span class="swym-storefront-layout-empty-state-notice-text"></span>
                </div>
                <hr class="swym-storefront-layout-empty-state-divider" />
                <a href="${accountUrl}" class="swym-storefront-layout-empty-state-cta-button swym-storefront-layout-empty-state-login-cta">
                  ${SwymStorefrontLayoutContext?.Strings?.emptyGuestLoginCTA}
                  <svg xmlns="http://www.w3.org/2000/svg" width="14" height="9" viewBox="0 0 14 9" fill="none"><path d="M13.8536 4.85368C14.0488 4.65841 14.0488 4.34183 13.8536 4.14657L10.6716 0.964589C10.4763 0.769327 10.1597 0.769327 9.96447 0.964589C9.7692 1.15985 9.7692 1.47643 9.96447 1.6717L12.7929 4.50012L9.96447 7.32855C9.7692 7.52381 9.7692 7.84039 9.96447 8.03566C10.1597 8.23092 10.4763 8.23092 10.6716 8.03566L13.8536 4.85368ZM0.5 5.00012L13.5 5.00012L13.5 4.00012L0.5 4.00012L0.5 5.00012Z" fill="currentColor"/></svg>
                </a>
                <button class="swym-storefront-layout-empty-state-cta-button swym-storefront-layout-empty-state-browse-cta" data-action="close-drawer">
                  ${SwymStorefrontLayoutContext?.Strings?.emptyGuestBrowseCTA}
                </button>
                <div class="swym-storefront-layout-empty-state-signup-text">
                  ${SwymStorefrontLayoutContext?.Strings?.emptyGuestSignupText} <a href="${accountUrl}" class="swym-storefront-layout-empty-state-signup-link">${SwymStorefrontLayoutContext?.Strings?.emptyGuestSignupCTA}</a>
                </div>
              </div>
            </div>
          </div>
        `;
            ...
            // Multi wishlist – old simple empty state (logged in or guest)
            emptyStateHtml = `
        ...
        `;
          ...
          this.elements.itemsContainer.innerHTML = emptyStateHtml;
          this.elements.itemsContainer.classList.add('swym-storefront-layout-items-has-empty');

          // Safely set notice text as innerHTML (controlled locale string with <strong> tags)
          const noticeEl = this.elements.itemsContainer.querySelector('.swym-storefront-layout-empty-state-notice-text');
          if (noticeEl) {
            noticeEl.innerHTML = noticeText || '';
          }

          // Attach event listener for "Keep browsing first" button (closes the drawer)
          const browseBtn = this.elements.itemsContainer.querySelector('[data-action="close-drawer"]');
          if (browseBtn) {
          ...
          // Attach event listener for multi-logged-in "+ Create a new list" card in items empty state
          const createCardItems = this.elements.itemsContainer.querySelector('[data-action="create-collection"]');
          if (createCardItems) {
            createCardItems.addEventListener('click', () => {
              SwymStorefrontLayoutExtensions?.SwymStorefrontLayoutActions?.setData({
                listId: SwymStorefrontLayoutContext?.DefaultList?.lid,
                list: SwymStorefrontLayoutContext?.DefaultList,
                item: {
        ...
        let wishlistCount = (this.wishlist?.listcontents || []).filter(item => item.source !== 'auto-wishlist').length;
        if (this.elements.title) {
      ...
      <div class="swym-storefront-layout-action-layout">
        ...
        <div class="swym-storefront-layout-action-content">
          <div class="swym-storefront-layout-action-header">
            ...
            <button type="button" aria-label="${SwymStorefrontLayoutContext?.Strings?.closeDrawer || 'Close'}" class="swym-storefront-layout-action-close-button">
    ...
    `
      ...
      async renderAddToCollection() {
        ...
        if (this.collections && this.elements.optionsContainer) {
          ...
          if (this.wishlistItem.iu) {
            this.elements.imageContainer.innerHTML = `<img src="${this.wishlistItem.iu}" alt="${swymEscAttr(this.wishlistItem.dt) || 'Product image'}">`;
            this.elements.imageContainer.style.display = '';
          } else {
      ...
      renderCollectionOptions() {
        ...
        const listImagesHtml = Array.from({ length: 3 })
          ...
          $ {
            item?.iu ? `<img src="${item ? item.iu : null}" alt="${swymEscAttr(item?.dt) || ''}" class="swym-storefront-layout-action-collection-item-image" />` : `<div class="swym-storefront-layout-action-collection-item-image" ></div>`
          } <
          /div>
        `;
          })
        ...
        return ` <
        div class = "swym-storefront-layout-collection-action-btn add-item-to-collection swym-storefront-layout-action-collection-item-container"
        data - lid = "${lid}" >
          <
          ...
          <
          div class = "swym-storefront-layout-action-collection-item-name" > $ {
            lname
          } < /div> <
          div class = "swym-storefront-layout-action-collection-item-count" > $ {
            count
          }
        $ {
          count === 1 ? SwymStorefrontLayoutContext?.Strings?.item : SwymStorefrontLayoutContext?.Strings?.items
        } < /div> <
        /div> <
        ...
        input type = "checkbox"
        $ {
          productExists ? 'checked' : ''
        }
        name = "swym-storefront-layout-collections" >
          <
      ...
      addCreateCollectionEventListeners() {
        ...
        createCollectionForm?.addEventListener('submit', async (event) => {
          try {
            ...
            // If item is from the SFY list, remove it from there when adding to a collection
            const sfyList = SwymStorefrontLayoutContext.getSfyList();
            if (sfyList && this.listId === sfyList.lid) {
              this.newlyUncheckedLids.push(sfyList.lid);
            }
            ...
            window._swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutCollectionCreated, {
              "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutCollectionInteractions
            });
          } catch (error) {
    ...
    class SwymStorefrontLayoutActionTooltip extends HTMLElement {
      constructor() {
        ...
        this._tooltipKeydownController = null;
        this.actionType = SwymStorefrontLayoutContext?.ActionTypes.ManageListItem;
    ...
    <div class="swym-storefront-layout-action-tooltip-layout">
      <div id="swym-storefront-layout-action-tooltip-options" tabindex="-1" aria-label="Wishlist options"></div>
    </div>
  `;
      }
      ...
      setData({
        ...
        } else if (actionType === SwymStorefrontLayoutContext?.ActionTypes.ManageSfyListItem) {
          this.renderSfyListItemOptions();
      ...
      async renderWishlistItemOptions() {
        if (this.listItem && this.elements.optionsContainer) {
          ...
          this.elements.optionsContainer.innerHTML = `
        ${shouldShowMultiple? `<div class="swym-storefront-layout-action-tooltip-btn add-to-collection-button" role="button" tabindex="0" aria-label="${SwymStorefrontLayoutContext?.Strings?.addToCollectionCta || 'Add to collection'}">
        ...
        ${canEdit?`<div class="swym-storefront-layout-action-tooltip-btn remove-from-list-button" role="button" tabindex="0" aria-label="${SwymStorefrontLayoutContext?.Strings?.removeItemCta || 'Remove from list'}">
          ...
          ${SwymStorefrontLayoutContext?.Strings?.removeItemCta || 'Remove from list'}
        </div>`:''}
      `;
      ...
      async renderShareSingleWishlist() {
        ...
        if (this.elements.optionsContainer) {
          this.elements.optionsContainer.innerHTML = `<div class="swym-storefront-layout-action-tooltip-btn share-collection-button" role="button" tabindex="0" aria-label="${SwymStorefrontLayoutContext?.Strings?.shareCollectionCta || 'Share collection'}">
      ...
      addEditCollectionEventListeners() {
        ...
        deleteCollectionBtn?.addEventListener('click', async () => {
          try {
            ...
            window._swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutCollectionDeleted, {
              "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutCollectionInteractions
            });
      ...
      async renderEditCollection() {
        ...
        <div class="swym-storefront-layout-action-tooltip-btn share-collection-button" role="button" tabindex="0" aria-label="${SwymStorefrontLayoutContext?.Strings?.shareCollectionCta || 'Share collection'}">
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="currentColor" aria-hidden="true">
            <path d="M9.91666 12.8337C9.43055 12.8337 9.01735 12.6635 8.67707 12.3232C8.3368 11.983 8.16666 11.5698 8.16666 11.0837C8.16666 11.0253 8.18124 10.8892 8.21041 10.6753L4.11249 8.28366C3.95693 8.42949 3.77707 8.54373 3.57291 8.62637C3.36874 8.70901 3.14999 8.75033 2.91666 8.75033C2.43055 8.75033 2.01735 8.58019 1.67707 8.23991C1.3368 7.89963 1.16666 7.48644 1.16666 7.00033C1.16666 6.51421 1.3368 6.10102 1.67707 5.76074C2.01735 5.42046 2.43055 5.25033 2.91666 5.25033C3.14999 5.25033 3.36874 5.29164 3.57291 5.37428C3.77707 5.45692 3.95693 5.57116 4.11249 5.71699L8.21041 3.32533C8.19096 3.25727 8.17881 3.19164 8.17395 3.12845C8.16909 3.06526 8.16666 2.99477 8.16666 2.91699C8.16666 2.43088 8.3368 2.01769 8.67707 1.67741C9.01735 1.33713 9.43055 1.16699 9.91666 1.16699C10.4028 1.16699 10.816 1.33713 11.1562 1.67741C11.4965 2.01769 11.6667 2.43088 11.6667 2.91699C11.6667 3.4031 11.4965 3.8163 11.1562 4.15658C10.816 4.49685 10.4028 4.66699 9.91666 4.66699C9.68332 4.66699 9.46457 4.62567 9.26041 4.54303C9.05624 4.46039 8.87638 4.34616 8.72082 4.20033L4.62291 6.59199C4.64235 6.66005 4.6545 6.72567 4.65936 6.78887C4.66423 6.85206 4.66666 6.92255 4.66666 7.00033C4.66666 7.0781 4.66423 7.14859 4.65936 7.21178C4.6545 7.27498 4.64235 7.3406 4.62291 7.40866L8.72082 9.80033C8.87638 9.65449 9.05624 9.54026 9.26041 9.45762C9.46457 9.37498 9.68332 9.33366 9.91666 9.33366C10.4028 9.33366 10.816 9.5038 11.1562 9.84408C11.4965 10.1844 11.6667 10.5975 11.6667 11.0837C11.6667 11.5698 11.4965 11.983 11.1562 12.3232C10.816 12.6635 10.4028 12.8337 9.91666 12.8337ZM9.91666 11.667C10.0819 11.667 10.2205 11.6111 10.3323 11.4993C10.4441 11.3875 10.5 11.2489 10.5 11.0837C10.5 10.9184 10.4441 10.7798 10.3323 10.668C10.2205 10.5562 10.0819 10.5003 9.91666 10.5003C9.75138 10.5003 9.61284 10.5562 9.50103 10.668C9.38923 10.7798 9.33332 10.9184 9.33332 11.0837C9.33332 11.2489 9.38923 11.3875 9.50103 11.4993C9.61284 11.6111 9.75138 11.667 9.91666 11.667ZM2.91666 7.58366C3.08193 7.58366 3.22048 7.52776 3.33228 7.41595C3.44409 7.30415 3.49999 7.1656 3.49999 7.00033C3.49999 6.83505 3.44409 6.69651 3.33228 6.5847C3.22048 6.4729 3.08193 6.41699 2.91666 6.41699C2.75138 6.41699 2.61284 6.4729 2.50103 6.5847C2.38923 6.69651 2.33332 6.83505 2.33332 7.00033C2.33332 7.1656 2.38923 7.30415 2.50103 7.41595C2.61284 7.52776 2.75138 7.58366 2.91666 7.58366ZM9.91666 3.50033C10.0819 3.50033 10.2205 3.44442 10.3323 3.33262C10.4441 3.22081 10.5 3.08227 10.5 2.91699C10.5 2.75171 10.4441 2.61317 10.3323 2.50137C10.2205 2.38956 10.0819 2.33366 9.91666 2.33366C9.75138 2.33366 9.61284 2.38956 9.50103 2.50137C9.38923 2.61317 9.33332 2.75171 9.33332 2.91699C9.33332 3.08227 9.38923 3.22081 9.50103 3.33262C9.61284 3.44442 9.75138 3.50033 9.91666 3.50033Z" fill="currentColor" fill-opacity="0.8"/>
          </svg>
          ${SwymStorefrontLayoutContext?.Strings?.shareCollectionCta}
          ...
          this.elements.optionsContainer.innerHTML = `
        ${shareButton}
        <div class="swym-storefront-layout-action-tooltip-btn delete-list-button" role="button" tabindex="0"        aria-label="${SwymStorefrontLayoutContext?.Strings?.deleteCollectionCta || 'Delete collection'}">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="17" viewBox="0 0 16 17" fill="currentColor" aria-hidden="true">
                    <path d="M4.66667 14.5C4.3000114.5 3.98612 14.3694 3.72501 14.1083C3.46389 13.8472 3.33334 13.5333 3.33334 13.1667V4.5H2.66667V3.16667H6.00001V2.5H10V3.16667H13.3333V4.5H12.6667V13.1667C12.6667 13.5333 12.5361 13.8472 12.275 14.1083C12.0139 14.3694 11.7 14.5 11.3333 14.5H4.66667ZM11.3333 4.5H4.66667V13.1667H11.3333V4.5ZM6.00001 11.8333H7.33334V5.83333H6.00001V11.8333ZM8.66667 11.8333H10V5.83333H8.66667V11.8333Z" fill="currentColor"/>
          </svg>
      ...
      async renderSaveCollection() {
        if (this.list && this.elements.optionsContainer) {
          this.elements.optionsContainer.innerHTML = `
        <div class="swym-storefront-layout-action-tooltip-btn save-collection-button" role="button" tabindex="0"        aria-label="${SwymStorefrontLayoutContext?.Strings?.saveCollectionCta || 'Save collection'}">
                  <svg xmlns="http://www.w3.org/2000/svg" width="12px" height="12px" viewBox="0 0 48 48" fill="none" aria-hidden="true">
      ...
      async handleSaveCollection() {
        ...
        let itemsToAdd = this.list.listcontents.map(({
          empi,
          epi,
          du
        }) => ({
      ...
      async renderSflListItemOptions() {
        if (this.listItem && this.elements.optionsContainer) {
          ...
          let canEdit = SwymStorefrontLayoutContext?.checkUserCanEditList(this.list);
          this.elements.optionsContainer.innerHTML = `
        ${canEdit?`<div class="swym-storefront-layout-action-tooltip-btn remove-from-list-button" role="button" tabindex="0"        aria-label="${SwymStorefrontLayoutContext?.Strings?.removeSflItemCta || 'Remove from list'}">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="17" viewBox="0 0 16 17" fill="currentColor" aria-hidden="true">
                    <path d="M4.66669 14.5C4.30002 14.5 3.98613 14.3694 3.72502 14.1083C3.46391 13.8472 3.33335 13.5333 3.33335 13.1667V4.5H2.66669V3.16667H6.00002V2.5H10V3.16667H13.3334V4.5H12.6667V13.1667C12.6667 13.5333 12.5361 13.8472 12.275 14.1083C12.0139 14.3694 11.7 14.5 11.3334 14.5H4.66669ZM11.3334 4.5H4.66669V13.1667H11.3334V4.5ZM6.00002 11.8333H7.33335V5.83333H6.00002V11.8333ZM8.66669 11.8333H10V5.83333H8.66669V11.8333Z" fill="currentColor"></path>
                  </svg>
                  ${SwymStorefrontLayoutContext?.Strings?.removeSflItemCta || 'Remove from list'}
        </div>`:''}
      `;
      ...
      async renderSfyListItemOptions() {
        if (this.listItem && this.elements.optionsContainer) {
          this.elements.optionsContainer.innerHTML = `
        <div class="swym-storefront-layout-action-tooltip-btn swym-sfy-remove-button" role="button" tabindex="0" aria-label="${SwymStorefrontLayoutContext?.Strings?.removeItemCta || 'Remove'}">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="17" viewBox="0 0 16 17" fill="currentColor" aria-hidden="true">
            <path d="M4.66669 14.5C4.30002 14.5 3.98613 14.3694 3.72502 14.1083C3.46391 13.8472 3.33335 13.5333 3.33335 13.1667V4.5H2.66669V3.16667H6.00002V2.5H10V3.16667H13.3334V4.5H12.6667V13.1667C12.6667 13.5333 12.5361 13.8472 12.275 14.1083C12.0139 14.3694 11.7 14.5 11.3334 14.5H4.66669ZM11.3334 4.5H4.66669V13.1667H11.3334V4.5ZM6.00002 11.8333H7.33335V5.83333H6.00002V11.8333ZM8.66669 11.8333H10V5.83333H8.66669V11.8333Z" fill="currentColor"></path>
          </svg>
          ${SwymStorefrontLayoutContext?.Strings?.removeItemCta || 'Remove'}
        </div>
      `;
          this.addSfyOptionEventListeners();
        }
      }

      addSfyOptionEventListeners() {
        const removeBtn = this.querySelector('.swym-sfy-remove-button');
        removeBtn?.addEventListener('click', async () => {
          try {
            ...
            const lid = this.listId;

            // Remove item from ALL context references (String comparison for type safety)
            SwymStorefrontLayoutContext.updateAllListContents((listcontents = []) =>
              listcontents.filter(item => String(item.epi) !== String(epi))
            );

            // Dispatch WishlistFetched to update UI immediately
            if (SwymStorefrontLayoutContext?.CustomEvents?.WishlistFetched) {
              document.dispatchEvent(new CustomEvent(SwymStorefrontLayoutContext.CustomEvents.WishlistFetched, {
                detail: {
                  defaultList: SwymStorefrontLayoutContext.DefaultList,
            ...
            SwymStorefrontLayoutExtensions?.Notification?.setMessage({
              ...
              status: SwymStorefrontLayoutContext?.NotificationStatusTypes?.Toast,
              duration: SwymStorefrontLayoutContext?.Settings?.StorefrontLayoutNotificationDuration,
              image: this.listItem?.iu
            });

            // Fire API THEN refresh to reconcile with server
            await SwymStorefrontLayoutAPI?.SwymWLAsyncApis?.removeProductFromList({
              epi,
              empi,
              du
            ...
            window._swat?.utils.error('[SFY] Remove failed', err);
          }
        });
      ...
      openTooltip() {
        ...
        this._closedViaKeyboard = false;
        const tl = this.elements.tooltipLayout;

        // Abort any previous keydown listener to prevent accumulation
        if (this._tooltipKeydownController) {
          this._tooltipKeydownController.abort();
        }
        this._tooltipKeydownController = new AbortController();

        tl.classList.add("swym-storefront-layout-show-action-tooltip-view");
        tl.setAttribute("role", "dialog");
        tl.setAttribute("aria-label", "Wishlist options");
        tl.setAttribute("aria-hidden", "false");
        tl.setAttribute("tabindex", "-1");

        const firstBtn = this.elements.optionsContainer?.querySelector('.swym-storefront-layout-action-tooltip-btn');
        if (firstBtn) {
          firstBtn.focus();
        } else {
          tl.focus();
        ...
        tl.addEventListener(
          ...
          (e) => {
            if (["Enter", " "].includes(e.key)) {
              const activeBtn = document.activeElement?.closest('[role="button"]');
              if (activeBtn && tl.contains(activeBtn)) {
                e.preventDefault();
                activeBtn.click();
              }
            }
            if (e.key === "Escape") {
              e.preventDefault();
              this._closedViaKeyboard = true;
              this.closeTooltip();
            }
          }, {
            signal: this._tooltipKeydownController.signal
          }
        );
      ...
      closeTooltip() {
        this.isTooltipOpen = false;
        const tl = this.elements.tooltipLayout;
        tl.classList.remove('swym-storefront-layout-show-action-tooltip-view');
        tl.setAttribute("aria-hidden", "true");
        tl.removeAttribute("role");
        tl.removeAttribute("aria-label");
        if (this._tooltipKeydownController) {
          this._tooltipKeydownController.abort();
          this._tooltipKeydownController = null;
        }
        if (this._closedViaKeyboard && this.currentTarget) {
    ...
    class SwymStorefrontLayoutNotification extends HTMLElement {
      ...
      setMessage(data, config = {}) {
        // Login nudge is intentionally persistent — suppress transient toasts while it is visible.
        // Pass config.priority = 'high' (e.g. errors) to break through and dismiss the nudge.
        if (this.hasAttribute('data-nudge-active')) {
          if (config.priority !== 'high') return;
          this.removeAttribute('data-nudge-active');
        }
        const {
          actionType,
          message,
    ...
    `;
      ...
      /**
       * setLoginNudge — shows a persistent corner notification prompting guest users to
       * log in when auto-wishlist has accumulated >= THRESHOLD items this session.
       *
       * Design choices:
       *  - Reuses this element (already bottom-left corner, clear of ATC bars).
       *  - No autoHide — stays until the user acts (login or dismiss).
       *  - _swat.wrapLogin respects the merchant's configured LoginPath and restores
       *    the current page after login via the POSTLOGINRD cookie.
       *  - Two thresholds: first nudge at 3 items (no +N), second nudge at 5 items (3 thumbs + +N).
       *  - sessionStorage keys are set immediately on show so each threshold fires at most once per session.
       */
      setLoginNudge() {
        const THRESHOLD_FIRST = 3;
        const THRESHOLD_SECOND = 5;
        const SESSION_KEY_FIRST = 'swym-login-nudge-3';
        const SESSION_KEY_SECOND = 'swym-login-nudge-5';
        const AUTO_DISMISS_MS = 2 * 60 * 1000;
        if (!SwymStorefrontLayoutContext.isLoginNudgeEnabled()) return;
        if (SwymStorefrontLayoutContext?.isShopperLoggedIn) return;
        if (!SwymStorefrontLayoutContext?.Settings?.EnableStorefrontLayoutNotification) return;
        const {
          shouldShowMultiple
        } = SwymStorefrontLayoutContext.getMultiWishlistConfig?.() || {};
        const SFY_LIST_NAME = 'Saved For Me';
        // B1: separate AW vs explicit counts — whichever hits threshold first fires
        let awItems, explicitItems;
        if (shouldShowMultiple) {
          awItems = SwymStorefrontLayoutContext.getSfyList?.()?.listcontents || [];
          explicitItems = (SwymStorefrontLayoutContext?.allLists || [])
            .filter(list => list.lname !== SFY_LIST_NAME)
            .flatMap(list => list.listcontents || []);
        } else {
          const defaultContents = SwymStorefrontLayoutContext?.DefaultList?.listcontents || [];
          awItems = defaultContents.filter(item => item.source === 'auto-wishlist');
          explicitItems = defaultContents.filter(item => item.source !== 'auto-wishlist');
        }
        const awCount = awItems.length;
        const explicitCount = explicitItems.length;
        // Priority: AW threshold-2 > AW threshold-1 > explicit threshold-2 > explicit threshold-1
        // Build priority-ordered candidate list, then walk it skipping session-suppressed entries.
        // Shared session keys (swym-login-nudge-3 / -5) are intentional: once a threshold nudge
        // fires for either AW or explicit items, that same threshold won't fire again this session
        // regardless of item type — preventing double-nudging within a session.
        const candidates = [
          ...(awCount >= THRESHOLD_SECOND ? [{
            sessionKey: SESSION_KEY_SECOND,
            showExtras: true,
            nudgeItems: awItems,
            isAW: true
          }] : []),
          ...(awCount >= THRESHOLD_FIRST ? [{
            sessionKey: SESSION_KEY_FIRST,
            showExtras: false,
            nudgeItems: awItems,
            isAW: true
          }] : []),
          ...(explicitCount >= THRESHOLD_SECOND ? [{
            sessionKey: SESSION_KEY_SECOND,
            showExtras: true,
            nudgeItems: explicitItems,
            isAW: false
          }] : []),
          ...(explicitCount >= THRESHOLD_FIRST ? [{
            sessionKey: SESSION_KEY_FIRST,
            showExtras: false,
            nudgeItems: explicitItems,
            isAW: false
        ...
        for (const c of candidates) {
          try {
            if (sessionStorage.getItem(c.sessionKey)) continue;
          } catch (e) {}
          chosen = c;
          break;
        }
        ...
        const InstrumentActionCodes = SwymStorefrontLayoutExtensions?.InstrumentActionCodes;
        const InstrumentUtmTerms = SwymStorefrontLayoutExtensions?.InstrumentUtmTerms;
        const nudgeCodes = isAW ?
          (sessionKey === SESSION_KEY_FIRST ?
            {
              shown: InstrumentActionCodes?.StorefrontLayoutAWNudge1Shown,
              loginClicked: InstrumentActionCodes?.StorefrontLayoutAWNudge1LoginClicked,
              dismissed: InstrumentActionCodes?.StorefrontLayoutAWNudge1Dismissed,
            } :
            {
              shown: InstrumentActionCodes?.StorefrontLayoutAWNudge2Shown,
              loginClicked: InstrumentActionCodes?.StorefrontLayoutAWNudge2LoginClicked,
              dismissed: InstrumentActionCodes?.StorefrontLayoutAWNudge2Dismissed,
            }) :
          (sessionKey === SESSION_KEY_FIRST ?
            {
              shown: InstrumentActionCodes?.StorefrontLayoutExplicitNudge1Shown,
              loginClicked: InstrumentActionCodes?.StorefrontLayoutExplicitNudge1LoginClicked,
              dismissed: InstrumentActionCodes?.StorefrontLayoutExplicitNudge1Dismissed,
            } :
            {
              shown: InstrumentActionCodes?.StorefrontLayoutExplicitNudge2Shown,
              loginClicked: InstrumentActionCodes?.StorefrontLayoutExplicitNudge2LoginClicked,
              dismissed: InstrumentActionCodes?.StorefrontLayoutExplicitNudge2Dismissed,
            });
        const nudgeUtmTerm = isAW ?
          InstrumentUtmTerms?.StorefrontLayoutAWLoginNudgeEngagement :
          InstrumentUtmTerms?.StorefrontLayoutExplicitLoginNudgeEngagement;
        const count = nudgeItems.length;
        const strings = SwymStorefrontLayoutContext?.Strings;
        const thumbs = nudgeItems.slice(0, THRESHOLD_FIRST); // always first 3
        const extraCount = showExtras ? count - THRESHOLD_FIRST : 0;
        // XSS: escape item.iu — alt is already escaped with &quot; but src was not
        const escAttr = (s) => (s || '').replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/'/g, '&#39;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
        const thumbsHtml = thumbs.map(item => `<img class="swym-aw-nudge-thumb" src="${escAttr(item.iu)}" alt="${escAttr(item.dt)}" loading="lazy" />`).join('');
        const extraHtml = extraCount > 0 ? `<span class="swym-aw-nudge-extra">+${extraCount}</span>` : '';
        // No hardcoded English fallbacks — strings come from strings/*.js locale files via retailerSettings
        const boldMessage = (isAW ?
          (strings?.awNudgeMessage || '') :
          (strings?.explicitNudgeMessage || '')
        ).replace('{count}', count);
        const subMessage = strings?.awNudgeMessageSub || '';
        const dismissLabel = strings?.awNudgeDismiss || '';
        const loginLabel = strings?.awNudgeLoginCTA || '';
        clearTimeout(this.timeout);
        // Use data-nudge-active instead of status to avoid inheriting the dark info background
        this.setAttribute('data-nudge-active', '');
        this.removeAttribute('status');
        // Mark as shown immediately — each threshold fires at most once per session
        try {
          sessionStorage.setItem(sessionKey, '1');
        } catch (e) {}
        this.innerHTML = `
      <div class="swym-aw-nudge-card">
        <div class="swym-aw-nudge-header">
          <div class="swym-aw-nudge-thumbs-row">
            ${thumbsHtml}${extraHtml}
          </div>
          <button type="button" class="swym-aw-nudge-dismiss-btn" aria-label="${escAttr(dismissLabel)}"><svg width="10" height="10" viewBox="0 0 10 10" fill="none" aria-hidden="true" focusable="false"><path d="M1 1l8 8M9 1L1 9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg></button>
        </div>
        <p class="swym-aw-nudge-message"><strong>${escAttr(boldMessage)}</strong> ${escAttr(subMessage)}</p>
        <button type="button" class="swym-aw-nudge-login-btn" aria-label="${escAttr(loginLabel)}">${escAttr(loginLabel)}</button>
      </div>
    `;
        this.querySelector('.swym-aw-nudge-login-btn')?.addEventListener('click', () => {
          clearTimeout(this._nudgeAutoDismissTimer);
          window._swat?.instrumentV3(nudgeCodes.loginClicked, {
            "utm-term": nudgeUtmTerm
          });
          window._swat?.wrapLogin({
            redirectPath: window.location.href
          });
        });
        this.querySelector('.swym-aw-nudge-dismiss-btn')?.addEventListener('click', () => {
          clearTimeout(this._nudgeAutoDismissTimer);
          window._swat?.instrumentV3(nudgeCodes.dismissed, {
            "utm-term": nudgeUtmTerm
          });
          this.removeAttribute('data-nudge-active');
          this.hideNotification();
        ...
        this.showNotification();
        window._swat?.instrumentV3(nudgeCodes.shown, {
          "utm-term": nudgeUtmTerm
        });
        // Auto-dismiss after 2 minutes — unblocks setMessage callers after a reasonable window
        this._nudgeAutoDismissTimer = setTimeout(() => {
          if (this.hasAttribute('data-nudge-active')) {
            this.removeAttribute('data-nudge-active');
            this.hideNotification();
          }
    ...
    class SwymStorefrontLayoutLoader extends HTMLElement {
      ...
      renderLoader() {
        ...
        const loaderListItemHtml = Array.from({
            length: 5
          })
          .map((_, i) => {
            return `
    ...
    `;
      ...
      `
          ...
          const loaderCollectionListHtml = Array.from({
              length: 3
            })
            .map((_, i) => {
              return `
    ...
    class SwymStorefrontLayoutTitle extends HTMLElement {
      ...
      renderTitle() {
        // Exclude the Saved For You list and auto-wishlist items — those are shown in the Saved For You tab
        const totalCount = (SwymStorefrontLayoutContext?.allLists || [])
          .filter(list => list.lname !== SFY_LIST_NAME)
          .reduce((total, list) => total + (list.listcontents || []).filter(item => item.source !== 'auto-wishlist').length, 0);

        // Drawer UI Polish: personalize title for single wishlist + logged-in users (drawer only)
        const {
          shouldShowMultiple
        } = SwymStorefrontLayoutContext.getMultiWishlistConfig();
        const isDrawer = SwymStorefrontLayoutContext?.Settings?.StorefrontLayoutType === 'as-drawer';
        const isLoggedIn = SwymStorefrontLayoutContext?.isShopperLoggedIn;
        const customerName = swymEscAttr(SwymStorefrontLayoutContext?.SwymCustomerData?.name?.split(' ')?.[0]);
        let titleText = SwymStorefrontLayoutContext?.Strings?.title;
        if (isDrawer && !shouldShowMultiple && isLoggedIn && customerName) {
          titleText = (SwymStorefrontLayoutContext?.Strings?.personalizedTitle || "{{name}}'s Wishlist").replace('{{name}}', customerName);
        }

        const hideCount = isDrawer && !shouldShowMultiple;
        const titleIcon = isDrawer ? (SwymStorefrontLayoutExtensions?.getWishlistIconSvg?.('filled') || '') : '';
        this.innerHTML = `
    <div>
      ...
      ${ !hideCount && totalCount?`<div class="swym-storefront-layout-total-list-items-count" id="swym-storefront-layout-total-list-items-count" aria-live="polite">${totalCount} ${totalCount<=1? SwymStorefrontLayoutContext?.Strings?.item : SwymStorefrontLayoutContext?.Strings?.items }</div>`:''}
    </div>
    `;
    ...
    class SwymStorefrontLayoutLoggedUser extends HTMLElement {
      constructor() {
        ...
        // Re-evaluate login prompt visibility after wishlist data is fetched,
        // since DefaultList may not be populated when swymCallbackFn first fires
        document.addEventListener(SwymStorefrontLayoutContext?.CustomEvents?.WishlistFetched, () => {
          this.updateLoginPromptVisibility();
        });
      }
      ...
      swymCallbackFn(swat) {
        ...
        } else if (isUserLogged && showLogin) {
          // For drawer + single wishlist: render share button in the login-user container
          // (same background, no heading/description text)
          this.renderLoggedInShareContainer();
        }
      ...
      updateLoginPromptVisibility() {
        ...
        // Always show login prompt inside collection list view for guests,
        // since the collection list overlays the main empty state
        const isInsideCollectionList = this.closest('swym-storefront-layout-collection-list');
        if (isInsideCollectionList) {
          this.renderLoginContainer();
          return;
        }

        // Hide the separate login prompt when wishlist is empty, since
        // the guest empty state already includes login CTAs
        const wishlistIsEmpty = !SwymStorefrontLayoutContext?.DefaultList?.listcontents?.length;
        if (wishlistIsEmpty) {
          this.innerHTML = '';
          return;
        }
    ...
    `
      ...
      renderLoginContainer() {
        ...
        const isDrawer = SwymStorefrontLayoutContext?.Settings?.StorefrontLayoutType === 'as-drawer';
        const showInlineButtons = isDrawer && !shouldShowMultiple;

        // Drawer + single wishlist: show inline Login to Save + Share buttons
        if (showInlineButtons) {
          const shareIcon = SwymStorefrontLayoutExtensions.getShareIconSvg();

          const inlineButtons = `
        <div class="swym-storefront-layout-login-actions">
          <button class="swym-storefront-layout-share-footer-login-btn" data-action="login" aria-label="${SwymStorefrontLayoutContext?.Strings?.loginToSaveCta || 'Login to Save'}">
            ${SwymStorefrontLayoutContext?.Strings?.loginToSaveCta || 'Login to Save'} <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none" aria-hidden="true" focusable="false"><path fill-rule="evenodd" clip-rule="evenodd" d="M3.49951 10C3.49951 9.58579 3.8353 9.25 4.24951 9.25L13.9391 9.25L11.2192 6.53036C10.9263 6.23748 10.9263 5.7626 11.2192 5.4697C11.512 5.17679 11.9869 5.17676 12.2798 5.46964L16.2802 9.46964C16.4209 9.6103 16.4999 9.80107 16.4999 10C16.4999 10.1989 16.4209 10.3897 16.2802 10.5304L12.2798 14.5304C11.9869 14.8232 11.512 14.8232 11.2192 14.5303C10.9263 14.2374 10.9263 13.7625 11.2192 13.4696L13.9391 10.75L4.24951 10.75C3.8353 10.75 3.49951 10.4142 3.49951 10Z" fill="currentColor"/></svg>
          </button>
          <button class="swym-storefront-layout-share-footer-share-btn" data-action="share" aria-label="${SwymStorefrontLayoutContext?.Strings?.shareCta || 'Share'}">
            ${shareIcon} ${SwymStorefrontLayoutContext?.Strings?.shareCta || 'Share'}
          </button>
        </div>`;
      ...
      `;
          this.attachShareFooterEvents();
          return;
        }

        // Default: original login container with <a> link for modal/section/multi-wishlist
        this.innerHTML = `
      <div class="swym-storefront-layout-login-user-container">
        <div class="swym-storefront-layout-login-content">
          <div class="swym-storefront-layout-login-heading">${SwymStorefrontLayoutContext?.Strings?.loginHeading}</div>
        ...
        <a href="${window.Shopify?.routes?.root || '/'}account" class="swym-storefront-layout-login-button">
    ...
    `;
      ...
      attachShareFooterEvents() {
        const shareBtn = this.querySelector('[data-action="share"]');
        const loginBtn = this.querySelector('[data-action="login"]');
        ...
        shareBtn?.addEventListener('click', (event) => {
          ...
          const isLoggedIn = window._swat?.platform?.isLoggedIn();
          if (!isLoggedIn) {
            const loginUrl = SwymStorefrontLayoutContext?.StorefrontLayoutUrls?.Login || '/account/login';
            window.location.href = loginUrl;
            return;
          }
          const activeTab = document.querySelector('.swym-storefront-layout-tab-button.swym-storefront-layout-tab-button-active');
          const activeDataView = activeTab?.getAttribute('data-view');
          const list = SwymStorefrontLayoutContext?.DefaultList;
          window._swat?.evtLayer?.dispatchEvent(new CustomEvent('sw:renderShareWishlistUI', {
            detail: {
              lid: list?.lid || '',
              lname: '',
              list: activeDataView === 'tabSavedForLater' ? SwymStorefrontLayoutContext?.sflList : list,
              layoutType: SwymStorefrontLayoutContext?.Settings?.StorefrontLayoutType,
              senderName: '',
              activeDataView
            }
      ...
      renderLoggedInShareContainer() {
        ...
        const selectedView = SwymStorefrontLayoutContext?.SelectedLayoutView;
        const isSfyTab = selectedView === SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.SavedForYou;
        const shareLabel = isSfyTab ?
          (SwymStorefrontLayoutContext?.Strings?.shareThisListCta || 'Share This List') :
          (SwymStorefrontLayoutContext?.Strings?.shareCta || 'Share');
        ...
        this.innerHTML = `
          <div class="swym-storefront-layout-login-user-container">
        <div class="swym-storefront-layout-login-actions">
          <button class="swym-storefront-layout-share-footer-share-btn swym-storefront-layout-share-footer-share-btn-full" data-action="share" aria-label="${shareLabel}">
    ...
    `;
        ...
        // Listen for tab changes to update share button label.
        // Remove any previous listener first to prevent stacking on re-render.
        if (this._tabChangeListener) {
          document.removeEventListener('click', this._tabChangeListener);
        }
        this._tabChangeListener = (e) => {
          if (e.target?.closest?.('.swym-storefront-layout-tab-button')) {
            requestAnimationFrame(() => this.renderLoggedInShareContainer());
          }
        };
        document.addEventListener('click', this._tabChangeListener);
      }
    ...
    class SwymStorefrontLayoutTabs extends HTMLElement {
      ...
      connectedCallback() {
        this.initUI();
        this._layoutInitHandler = () => {
          SwymStorefrontLayoutContext.Tabs = [SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.Wishlist];
          if (window._swat.retailerSettings.SFL?.SFLFeatureEnabled) {
            SwymStorefrontLayoutContext.Tabs.push(SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.SaveForLater);
          }
          // SavedForYou tab is added/removed dynamically by the WishlistFetched listener below
          this.initUI();
        };
        this._wishlistFetchedHandler = () => {
          if (window.SwymEnabledCommonFeatures?.['auto-add-to-wishlist-from-pdp']?.enabled) {
            const {
              shouldShowMultiple
            } = SwymStorefrontLayoutContext.getMultiWishlistConfig();
            const hasAutoWishlistItems = shouldShowMultiple ?
              (SwymStorefrontLayoutContext.getSfyList()?.listcontents?.length > 0) :
              (SwymStorefrontLayoutContext?.DefaultList?.listcontents || []).some(item => item.source === 'auto-wishlist');
            const savedForYouViewType = SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.SavedForYou;
            const currentIdx = SwymStorefrontLayoutContext.Tabs?.indexOf(savedForYouViewType);
            if (hasAutoWishlistItems && currentIdx === -1) {
              SwymStorefrontLayoutContext.Tabs.splice(1, 0, savedForYouViewType);
            } else if (!hasAutoWishlistItems && currentIdx !== -1) {
              SwymStorefrontLayoutContext.Tabs.splice(currentIdx, 1);
              if (SwymStorefrontLayoutContext.SelectedLayoutView === savedForYouViewType) {
                SwymStorefrontLayoutContext.SelectedLayoutView = SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.Wishlist;
              }
            }
          }
          // Always re-render to update tab count badges
          this.initUI();
        };
        document.addEventListener(SwymStorefrontLayoutContext?.CustomEvents?.LayoutInitialized, this._layoutInitHandler);
        document.addEventListener(SwymStorefrontLayoutContext?.CustomEvents?.WishlistFetched, this._wishlistFetchedHandler);
      }

      disconnectedCallback() {
        if (this._layoutInitHandler) {
          document.removeEventListener(SwymStorefrontLayoutContext?.CustomEvents?.LayoutInitialized, this._layoutInitHandler);
        }
        if (this._wishlistFetchedHandler) {
          document.removeEventListener(SwymStorefrontLayoutContext?.CustomEvents?.WishlistFetched, this._wishlistFetchedHandler);
        }
      }
      ...
      initUI() {
        ...
        // Always sync content panels visibility with SelectedLayoutView
        const selectedView = SwymStorefrontLayoutContext?.SelectedLayoutView || SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.Wishlist;
        const tabContent = document.querySelector('swym-storefront-layout-tab-content');
        tabContent?.querySelectorAll('.swym-storefront-layout-tab-container').forEach(container => {
          const isActive = container.id === selectedView;
          container.classList.toggle('swym-storefront-layout-tab-content-active', isActive);
          if (isActive) container.removeAttribute('hidden');
          else container.setAttribute('hidden', '');
        });

        if (this.elements.currentSelectedTab) {
      ...
      <div class="swym-storefront-layout-tabs-view">
        <div class="swym-storefront-layout-tabs" role="tablist" aria-label="Wishlist views">
          ${tabs.map((tab, index)=>{
            ...
            const isDrawerLayout = SwymStorefrontLayoutContext?.Settings?.StorefrontLayoutType === 'as-drawer';
            const { shouldShowMultiple } = SwymStorefrontLayoutContext.getMultiWishlistConfig();
            const tabCount = isDrawerLayout ? this.getTabItemCount(tab) : 0;
            const hideCount = !isDrawerLayout || shouldShowMultiple || !isActive;
            const countBadge = isDrawerLayout ? ` < span class = "swym-storefront-layout-tab-count${hideCount ? ' swym-storefront-layout-tab-count-hidden' : ''}" > $ {
          tabCount
        } < /span>` : '';
        return `<button class="swym-storefront-layout-tab-button ${isActive?'swym-storefront-layout-tab-button-active':''}" data-view="${tab}" role="tab" aria-selected="${isActive}" aria-controls="${tab}" tabindex="${isActive ? '0' : '-1'}" id="tab-${tab}">${SwymStorefrontLayoutContext?.Strings?.[tab]}${countBadge}</button>`
      }).join('')
  } <
  /div> <
  ...
  getTabItemCount(tab) {
    if (tab === SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.Wishlist) {
      const allItems = SwymStorefrontLayoutContext?.DefaultList?.listcontents || [];
      const hasSfyTab = SwymStorefrontLayoutContext?.Tabs?.includes(SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.SavedForYou);
      return hasSfyTab ? allItems.filter(item => item.source !== 'auto-wishlist').length : allItems.length;
    }
    if (tab === SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.SavedForYou) {
      return SwymStorefrontLayoutContext?.savedForYouItems?.length || 0;
    }
    if (tab === SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.SaveForLater) {
      return SwymStorefrontLayoutContext?.sflList?.listcontents?.length || 0;
    }
    return 0;
  }
  ...
  setTabSelection(selectedTab){
    ...
    this.elements.tabs?.forEach(tab => {
      ...
      // Toggle tab count visibility (opacity) for drawer layout — always hidden for multi-wishlist
      const countBadge = tab.querySelector('.swym-storefront-layout-tab-count');
      if (countBadge) {
        const { shouldShowMultiple } = SwymStorefrontLayoutContext.getMultiWishlistConfig();
        countBadge.classList.toggle('swym-storefront-layout-tab-count-hidden', shouldShowMultiple || !isSelected);
      }
    });
    ...
    // Wait for next frame to ensure layout is complete
      requestAnimationFrame(() => {
        ...
        this.elements.tabIndicator?.style.setProperty(
          ...
          `
  $ {
    tabCenter - (this.elements.tabIndicator.offsetWidth ?? 0) / 2
  }
  px`
        );
      })
  ...
  attachEvents() {
    this.elements.tabs?.forEach((tab) => {
      ...
      tab.addEventListener('click', (event) => {
        ...
        if (clickedTab.getAttribute('data-view') === SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.SavedForYou) {
          window._swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutSFYTabViewed, { "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutUiEngagement });
        }
      });
      ...
      tab.addEventListener('keydown', (event) => {
        ...
        switch(event.key) {
          ...
          case ' ':
            ...
            if (eventTarget.getAttribute('data-view') === SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.SavedForYou) {
              window._swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutSFYTabViewed, { "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutUiEngagement });
            }
            return;
          default:
...
class SwymStorefrontLayoutTabContent extends HTMLElement {
  ...
  renderUI() {
    this.innerHTML = ` <
  swym - storefront - layout - wishlist - container id = "${SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.Wishlist}"
  ...
  swym - storefront - layout - sfl - container id = "${SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.SaveForLater}"
  ...
  swym - storefront - layout - saved -
  for -you - container id = "${SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.SavedForYou}"
  class = "swym-storefront-layout-tab-container"
  role = "tabpanel"
  aria - labelledby = "tab-${SwymStorefrontLayoutContext?.StorefrontLayoutViewType?.SavedForYou}"
  tabindex = "0"
  hidden >
  < /swym-storefront-layout-saved-for-you-container>
...
class SwymStorefrontLayoutWishlistContainer extends HTMLElement {
  ...
  $ {
    isLoggedIn ?
      `<button id="swym-storefront-layout-collection-option-button" class="swym-storefront-layout-collection-option-button swym-single-share-wishlist-btn swym-hidden" tabindex="0" aria-haspopup="dialog" aria-expanded="false" aria-label="Action menu options">
          <svg xmlns="http://www.w3.org/2000/svg" width="3" height="13" viewBox="0 0 3 13" fill="currentcolor">
            <circle cx="1.5" cy="1.50014" r="1.5" fill="currentColor"></circle>
            <circle cx="1.5" cy="6.50014" r="1.5" fill="currentColor"></circle>
            <circle cx="1.5" cy="11.5001" r="1.5" fill="currentColor"></circle>
  ...
  role = "region"
  aria - label = "Wishlist Content Section" >
  <
  swym - storefront - layout - collection - carousel > < /swym-storefront-layout-collection-carousel> <
  swym - storefront - layout -
  default -wishlist > < /swym-storefront-layout-default-wishlist> <
  swym - storefront - layout - collection - list id = "swym-storefront-layout-collection-list"
  role = "list"
  aria - label = "Your Wishlist Collections" > < /swym-storefront-layout-collection-list> <
  /div>
  `;
  }
  ...
  initElement() {
    ...
    document.addEventListener(SwymStorefrontLayoutContext?.CustomEvents?.WishlistFetched, () => {
      ...
      // Use allLists as the source for collections, excluding default list and the SFY list
      // (SFY list is shown in its own dedicated tab, not in the collections carousel)
      ...
      const { shouldShowMultiple } = SwymStorefrontLayoutContext.getMultiWishlistConfig();
      const sfyListId = shouldShowMultiple ? SwymStorefrontLayoutContext.getSfyList()?.lid : null;
      this.collections = allLists.filter(list => list.lid !== defaultListId && list.lid !== sfyListId);
  ...
  renderWishlistAndCollections() {
    ...
    // Check for auto-wishlist items directly — don't rely on Tabs array state since
    // the Tabs listener and this listener fire in undefined order on the same event.
    const autoWishlistEnabled = window.SwymEnabledCommonFeatures?.['auto-add-to-wishlist-from-pdp']?.enabled;
    // Multi-list: SFY items live in the dedicated "Saved For Me" list, not in DefaultList
    const hasSfyItems = autoWishlistEnabled && (
      shouldShowMultiple
        ? (SwymStorefrontLayoutContext.getSfyList()?.listcontents?.length > 0)
        : (SwymStorefrontLayoutContext?.DefaultList?.listcontents || []).some(item => item.source === 'auto-wishlist')
    );
    ...
    if(this.collections?.length > 0 || (this.wishlist && this.wishlist?.listcontents?.length > 0)) {
      // Hide the 3-dot menu in drawer + single wishlist — share is handled by the footer button
      if (!(isDrawer && !shouldShowMultiple)) {
    ...
    if ( this.wishlist || this.collections) {
      ...
      // Filter auto-wishlist items from the Wishlist tab — they belong in the SFY tab only.
      // In single-list mode, both share DefaultList. In multi-list mode, this is a safety net
      // in case DefaultList somehow contains auto-wishlist items (e.g. SFY list was lists[0]).
      const wishlistForDisplay = hasSfyItems
        ? { ...this.wishlist, listcontents: (this.wishlist?.listcontents || []).filter(item => item.source !== 'auto-wishlist') }
        : this.wishlist;
...
class SwymStorefrontLayoutSaveForLaterContainer extends HTMLElement{
  ...
  renderUI(){
    ...
    const isDrawer = SwymStorefrontLayoutContext?.Settings?.StorefrontLayoutType === 'as-drawer';
    const { shouldShowMultiple } = SwymStorefrontLayoutContext.getMultiWishlistConfig();
    const hideOptionButton = isDrawer && !shouldShowMultiple;
    this.innerHTML = ` <
  div class = "swym-storefront-layout-header" >
  <
  swym - storefront - layout - sfl - title > < /swym-storefront-layout-sfl-title>
  $ {
    isLoggedIn && !hideOptionButton ?
...
class SwymStorefrontLayoutSaveForLaterTitle extends HTMLElement {
  ...
  aria - level = "2" >
  $ {
    SwymStorefrontLayoutContext?.Settings?.StorefrontLayoutType === 'as-drawer' ? (SwymStorefrontLayoutExtensions?.getWishlistIconSvg?.('filled') || '') : ''
  }
  $ {
...
class SwymStorefrontLayoutSaveForLaterList extends HTMLElement {
  ...
  renderUI() {
    ...
    this.innerHTML = ` <
  div id = "swym-storefront-layout-items-container"
  class = "swym-storefront-layout-items-container swym-storefront-layout-sfl-list-container" >
  $ {
  ...
  async renderSFL() {
    ...
    if (slfListItems?.length && this.elements.itemsContainer) {
      ...
      slfListItems.forEach((item) => {
        ...
        sflItem.innerHTML = ` < swym - storefront - layout - item id = "swym-storefront-layout-sfl-item-${item.empi}-${item.epi}"
  class = "swym-storefront-layout-sfl-item" > < /swym-storefront-layout-item>`;
...
this.elements.itemsContainer.classList.remove('swym-storefront-layout-items-has-empty');
window._swat?.instrumentV3(SwymStorefrontLayoutExtensions?.InstrumentActionCodes?.StorefrontLayoutSFLItemsRendered, {
  "utm-term": SwymStorefrontLayoutExtensions?.InstrumentUtmTerms?.StorefrontLayoutUiEngagement
});
}
else {
  const isLoggedIn = SwymStorefrontLayoutContext?.isShopperLoggedIn;
  const {
    shouldShowMultiple: sflShouldShowMultiple
  } = SwymStorefrontLayoutContext.getMultiWishlistConfig();
  const cartUrl = (window.Shopify?.routes?.root || '/').replace(/\/$/, '') + '/cart';
  const accountUrl = (window.Shopify?.routes?.root || '/').replace(/\/$/, '') + '/account';

  if (sflShouldShowMultiple) {
    // Multi wishlist – old simple SFL empty state
    this.elements.itemsContainer.innerHTML = `
          <div class="swym-storefront-layout-empty-container">
            <div class="swym-storefront-layout-empty-sfl-container">
              <div class="swym-storefront-layout-empty-sfl-content">
                ...
                <a class="swym-storefront-layout-empty-sfl-view-cart-button" href="${(window.Shopify?.routes?.root || '/').replace(/\/$/, '') + '/cart'}">${SwymStorefrontLayoutContext?.Strings?.viewCartCta}</a>
              </div>
            </div>
          </div>
        `;
  ...
  if (isLoggedIn) {
    sflNoticeText = SwymStorefrontLayoutContext?.Strings?.emptySflLoggedInNotice;
    this.elements.itemsContainer.innerHTML = `
          <div class="swym-storefront-layout-empty-container swym-storefront-layout-empty-state-sfl swym-storefront-layout-empty-state-sfl-logged">
            <div class="swym-storefront-layout-empty-wishlist-container">
              <div class="swym-storefront-layout-empty-wishlist-content">
                <div class="swym-storefront-layout-empty-state-icon">
                  <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48" fill="none">
                    <circle cx="24" cy="24" r="24" fill="#F5F5F5"/>
                    <path d="M20 18H18C17.4696 18 16.9609 18.2107 16.5858 18.5858C16.2107 18.9609 16 19.4696 16 20V32C16 32.5304 16.2107 33.0391 16.5858 33.4142C16.9609 33.7893 17.4696 34 18 34H30C30.5304 34 31.0391 33.7893 31.4142 33.4142C31.7893 33.0391 32 32.5304 32 32V20C32 19.4696 31.7893 18.9609 31.4142 18.5858C31.0391 18.2107 30.5304 18 30 18H28M20 18V16C20 15.4696 20.2107 14.9609 20.5858 14.5858C20.9609 14.2107 21.4696 14 22 14H26C26.5304 14 27.0391 14.2107 27.4142 14.5858C27.7893 14.9609 28 15.4696 28 16V18M20 18H28" stroke="#9B9B9B" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M20 24H28M20 28H25" stroke="#9B9B9B" stroke-width="1.5" stroke-linecap="round"/>
                  </svg>
                  <span class="swym-storefront-layout-empty-state-badge">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none">
                      <circle cx="7" cy="7" r="7" fill="#4CAF77"/>
                      <path d="M4 7L6 9L10 5" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                  </span>
                </div>
                <div class="swym-storefront-layout-empty-wishlist-title">${SwymStorefrontLayoutContext?.Strings?.emptySflTitle}</div>
                <div class="swym-storefront-layout-empty-wishlist-description">${SwymStorefrontLayoutContext?.Strings?.emptySflDescription}</div>
                <div class="swym-storefront-layout-empty-sfl-steps-card">
                  <div class="swym-storefront-layout-empty-sfl-step">
                    <div class="swym-storefront-layout-empty-sfl-step-icon swym-storefront-layout-empty-sfl-step-icon--green">
                      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
                        <path d="M1.5 1.5H3.5L4.8 8.8C4.9 9.3 5.3 9.6 5.8 9.6H13.2C13.7 9.6 14.1 9.2 14.2 8.7L15.5 4H4" stroke="#4CAF77" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>
                        <circle cx="6.5" cy="12.5" r="1.3" stroke="#4CAF77" stroke-width="1.3"/>
                        <circle cx="13" cy="12.5" r="1.3" stroke="#4CAF77" stroke-width="1.3"/>
                      </svg>
                    </div>
                    <div class="swym-storefront-layout-empty-sfl-step-content">
                      <div class="swym-storefront-layout-empty-sfl-step-title">${SwymStorefrontLayoutContext?.Strings?.emptySflStep1Title}</div>
                      <div class="swym-storefront-layout-empty-sfl-step-desc">${SwymStorefrontLayoutContext?.Strings?.emptySflStep1Description}</div>
                    </div>
                  </div>
                  <div class="swym-storefront-layout-empty-sfl-step-divider"></div>
                  <div class="swym-storefront-layout-empty-sfl-step">
                    <div class="swym-storefront-layout-empty-sfl-step-icon swym-storefront-layout-empty-sfl-step-icon--red">
                      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
                        <path d="M3 5H15" stroke="#E53E3E" stroke-width="1.3" stroke-linecap="round"/>
                        <path d="M6 5V3H12V5" stroke="#E53E3E" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M4.5 5L5.5 15H12.5L13.5 5" stroke="#E53E3E" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M7.5 8V12M10.5 8V12" stroke="#E53E3E" stroke-width="1.3" stroke-linecap="round"/>
                      </svg>
                    </div>
                    <div class="swym-storefront-layout-empty-sfl-step-content">
                      <div class="swym-storefront-layout-empty-sfl-step-title">${SwymStorefrontLayoutContext?.Strings?.emptySflStep2Title}</div>
                      <div class="swym-storefront-layout-empty-sfl-step-desc">${SwymStorefrontLayoutContext?.Strings?.emptySflStep2Description}</div>
                    </div>
                  </div>
                </div>
                <div class="swym-storefront-layout-empty-sfl-notice swym-storefront-layout-empty-sfl-notice--grey">
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
                    <circle cx="9" cy="9" r="8" stroke="#9B9B9B" stroke-width="1.2"/>
                    <path d="M6 9L8 11L12 7" stroke="#9B9B9B" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                  <span class="swym-storefront-layout-empty-sfl-notice-text"></span>
                </div>
                <hr class="swym-storefront-layout-empty-state-divider" />
                <a href="${cartUrl}" class="swym-storefront-layout-empty-state-cta-button swym-storefront-layout-empty-sfl-go-to-cart">
                  ${SwymStorefrontLayoutContext?.Strings?.emptySflGoToCartCTA}
                  <svg xmlns="http://www.w3.org/2000/svg" width="14" height="9" viewBox="0 0 14 9" fill="none"><path d="M13.8536 4.85368C14.0488 4.65841 14.0488 4.34183 13.8536 4.14657L10.6716 0.964589C10.4763 0.769327 10.1597 0.769327 9.96447 0.964589C9.7692 1.15985 9.7692 1.47643 9.96447 1.6717L12.7929 4.50012L9.96447 7.32855C9.7692 7.52381 9.7692 7.84039 9.96447 8.03566C10.1597 8.23092 10.4763 8.23092 10.6716 8.03566L13.8536 4.85368ZM0.5 5.00012L13.5 5.00012L13.5 4.00012L0.5 4.00012L0.5 5.00012Z" fill="currentColor"/></svg>
                </a>
              </div>
            </div>
    ...
    sflNoticeText = SwymStorefrontLayoutContext?.Strings?.emptySflGuestNotice;
    this.elements.itemsContainer.innerHTML = `
          <div class="swym-storefront-layout-empty-container swym-storefront-layout-empty-state-sfl swym-storefront-layout-empty-state-sfl-guest">
            <div class="swym-storefront-layout-empty-wishlist-container">
              <div class="swym-storefront-layout-empty-wishlist-content">
                <div class="swym-storefront-layout-empty-state-icon">
                  <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48" fill="none">
                ...
                <div class="swym-storefront-layout-empty-sfl-notice swym-storefront-layout-empty-sfl-notice--amber">
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
                    <circle cx="9" cy="9" r="8" stroke="#B45309" stroke-width="1.2"/>
                    <path d="M9 5V9L11 11" stroke="#B45309" stroke-width="1.2" stroke-linecap="round"/>
                  </svg>
                ...
                <a href="${accountUrl}" class="swym-storefront-layout-empty-state-cta-button swym-storefront-layout-empty-sfl-login-cta">
                  ${SwymStorefrontLayoutContext?.Strings?.emptySflLoginCTA}
                </a>
                <div class="swym-storefront-layout-empty-state-signup-text">
                  ${SwymStorefrontLayoutContext?.Strings?.emptySflSignupText} <a href="${accountUrl}" class="swym-storefront-layout-empty-state-signup-link">${SwymStorefrontLayoutContext?.Strings?.emptySflSignupCTA}</a>
                </div>
              </div>
            </div>
          </div>
  ...
  const sflNoticeEl = this.elements.itemsContainer.querySelector('.swym-storefront-layout-empty-sfl-notice-text');
  if (sflNoticeEl) {
    sflNoticeEl.innerHTML = sflNoticeText || '';
  }
}
...
class SwymStorefrontLayoutSavedForYouContainer extends HTMLElement {
  constructor() {
    super();
    this.elements = {};
    this.initUI();
    document.addEventListener(SwymStorefrontLayoutContext?.CustomEvents?.WishlistFetched, () => {
      this.loadSavedForYouItems();
    });
  }
  ...
  renderUI() {
    this.innerHTML = `
      <div class="swym-storefront-layout-header">
        <swym-storefront-layout-saved-for-you-title></swym-storefront-layout-saved-for-you-title>
      </div>
      <div class="swym-storefront-layout-body" role="region" aria-labelledby="swym-saved-for-you-title">
        <swym-storefront-layout-saved-for-you-list></swym-storefront-layout-saved-for-you-list>
      </div>
    `;
  }
  ...
  initElement() {
    this.elements = {
      savedForYouList: this.querySelector('swym-storefront-layout-saved-for-you-list')
    };
  }

  loadSavedForYouItems() {
    const {
      shouldShowMultiple
    } = SwymStorefrontLayoutContext.getMultiWishlistConfig();
    if (shouldShowMultiple) {
      // Multi-list: items live in the dedicated "Saved For Me" list
      const sfyList = SwymStorefrontLayoutContext.getSfyList();
      SwymStorefrontLayoutContext.savedForYouItems = sfyList?.listcontents || [];
      SwymStorefrontLayoutContext.savedForYouList = sfyList || SwymStorefrontLayoutContext?.DefaultList;
    } else {
      // Single-list: items are source-tagged inside DefaultList
      const allItems = SwymStorefrontLayoutContext?.DefaultList?.listcontents || [];
      SwymStorefrontLayoutContext.savedForYouItems = allItems.filter(item => item.source === 'auto-wishlist');
      SwymStorefrontLayoutContext.savedForYouList = SwymStorefrontLayoutContext?.DefaultList;
    }
    document.dispatchEvent(new CustomEvent(SwymStorefrontLayoutContext?.CustomEvents?.SavedForYouFetched));
  }
}

class SwymStorefrontLayoutSavedForYouTitle extends HTMLElement {
  constructor() {
    super();
    this.renderTitle();
    document.addEventListener(SwymStorefrontLayoutContext?.CustomEvents?.SavedForYouFetched, () => {
      this.renderTitle();
    });
  }

  renderTitle() {
    const totalCount = SwymStorefrontLayoutContext?.savedForYouItems?.length || 0;

    // Drawer UI Polish: personalize SFY title for single wishlist + logged-in users (drawer only)
    const {
      shouldShowMultiple
    } = SwymStorefrontLayoutContext.getMultiWishlistConfig();
    const isDrawer = SwymStorefrontLayoutContext?.Settings?.StorefrontLayoutType === 'as-drawer';
    const isLoggedIn = SwymStorefrontLayoutContext?.isShopperLoggedIn;
    const customerName = swymEscAttr(SwymStorefrontLayoutContext?.SwymCustomerData?.name?.split(' ')?.[0]);
    let sfyTitleText = SwymStorefrontLayoutContext?.Strings?.tabSavedForYou || 'Saved For You';
    if (isDrawer && !shouldShowMultiple && isLoggedIn && customerName) {
      sfyTitleText = (SwymStorefrontLayoutContext?.Strings?.personalizedSfyTitle || 'Saved For {{name}}').replace('{{name}}', customerName);
    }

    const hideCount = isDrawer && !shouldShowMultiple;
    const sfyTitleIcon = isDrawer ? (SwymStorefrontLayoutExtensions?.getWishlistIconSvg?.('filled') || '') : '';

    this.innerHTML = `
      <div>
        <div class="swym-storefront-layout-title" id="swym-saved-for-you-title" role="heading" aria-level="2">
          ${sfyTitleIcon}
          ${sfyTitleText}
        </div>
        ${ !hideCount && totalCount ? `<div class="swym-storefront-layout-total-list-items-count" id="swym-saved-for-you-items-count" aria-live="polite">${totalCount} ${totalCount <= 1 ? SwymStorefrontLayoutContext?.Strings?.item : SwymStorefrontLayoutContext?.Strings?.items}</div>` : ''}
      </div>
    `;
    const titleElement = this.querySelector('.swym-storefront-layout-title');
    SwymStorefrontLayoutExtensions?.applyHeadingFontStyles(titleElement);
...
class SwymStorefrontLayoutSavedForYouList extends HTMLElement {
  constructor() {
    super();
    this.elements = {};
    this.renderUI();
  ...
  renderUI() {
    this.innerHTML = `
      <div id="swym-storefront-layout-sfy-items-container" class="swym-storefront-layout-items-container swym-storefront-layout-saved-for-you-list-container" role="list" aria-label="${SwymStorefrontLayoutContext?.Strings?.tabSavedForYou || 'Saved For You'} items" aria-busy="true" aria-live="polite">
        <swym-storefront-layout-loader loading="true" loadertype="${SwymStorefrontLayoutContext?.LoaderViewType?.WishlistListItem}" buttonCta="${SwymStorefrontLayoutContext?.Strings?.addToCart}" height="100px"></swym-storefront-layout-loader>
      </div>
    `;
    this.elements = {
      itemsContainer: this.querySelector('#swym-storefront-layout-sfy-items-container')
    };
  }

  async renderItems() {
    const items = SwymStorefrontLayoutContext?.savedForYouItems || [];
    // In multi-list mode this is the "Saved For Me" list; in single-list it's DefaultList.
    // The correct lid is needed so remove/move operations target the right list.
    const list = SwymStorefrontLayoutContext?.savedForYouList || SwymStorefrontLayoutContext?.DefaultList;

    if (!this.elements.itemsContainer) return;

    if (items.length === 0) {
      this.elements.itemsContainer.innerHTML = `
        <div class="swym-storefront-layout-empty-container swym-storefront-layout-empty-sfy"></div>
      `;
      this.elements.itemsContainer.classList.add('swym-storefront-layout-items-has-empty');
      this.elements.itemsContainer.setAttribute('aria-busy', 'false');
      return;
    }

    // Reverse-chronological: most recently auto-wishlisted appears first
    const sortedItems = [...items].sort((a, b) => (b.cdate || b.ts || 0) - (a.cdate || a.ts || 0));

    const fragment = document.createDocumentFragment();
    sortedItems.forEach((item) => {
      const el = document.createElement('template');
      el.innerHTML = `<swym-storefront-layout-item id="swym-storefront-layout-sfy-item-${item.empi}-${item.epi}" class="swym-storefront-layout-sfy-item"></swym-storefront-layout-item>`;
      fragment.appendChild(el.content);
    });
    this.elements.itemsContainer.innerHTML = '';
    this.elements.itemsContainer.appendChild(fragment);

    sortedItems.forEach((item) => {
      const el = document.querySelector(`#swym-storefront-layout-sfy-item-${item.empi}-${item.epi}`);
      el?.setData({
        item,
        listItemType: SwymStorefrontLayoutContext?.ListItemType?.SavedForYouItem,
        list
      });
    });
    this.elements.itemsContainer.classList.remove('swym-storefront-layout-items-has-empty');
...
// Show the login nudge when wishlist data is available.
document.addEventListener(
  SwymStorefrontLayoutContext?.CustomEvents?.WishlistFetched,
  () => {
    SwymStorefrontLayoutExtensions?.Notification?.setLoginNudge?.();
  }
);
...
customElements.define('swym-storefront-layout-saved-for-you-container', SwymStorefrontLayoutSavedForYouContainer);
customElements.define('swym-storefront-layout-saved-for-you-title', SwymStorefrontLayoutSavedForYouTitle);
customElements.define('swym-storefront-layout-saved-for-you-list', SwymStorefrontLayoutSavedForYouList);
})();