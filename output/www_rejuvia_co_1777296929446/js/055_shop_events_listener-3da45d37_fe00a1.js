(() => {
  var AddToCartListener = class _AddToCartListener {
    static ADD_TO_CART_REGEX = /^(?:\/[a-zA-Z]+(?:-[a-zA-Z]+)?)?\/+cart\/+add(?:\.js|\.json)?\/*$/;
    ...
    static handleXhrDone(payload) {
      ...
      try {
        ...
        const path = hyperlinkElement.pathname ? hyperlinkElement.pathname : payload.url;
        if (_AddToCartListener.ADD_TO_CART_REGEX.test(path)) {
          _AddToCartListener._parsePayloadResponse(payload, parsedResponse => {
            try {
            ...
            if (keys.length === 1 && keys[0] === "items") {
              ...
              try {
                ...
                parsedPayloadBodyItemsList = parseFormEncodedBulkPayload(payload.body, parsedResponseItemsList.length)
              }
              handleBulkItemCartAddResponse(parsedResponseItemsList, parsedPayloadBodyItemsList, "add-xhr-bulk")
            } else {
              handleItemAdded(parsedResponse, parseSingleItemQuantity(payload.body), "add-xhr")
            }
          })
        }
      } catch (error) {
        if (console && console.warn) {
          console.warn(`[shop_events_listener] Error in handleXhrDone:  ${error.message}`)
        }
      }
    }
    static parseBlobToJson(blob, parsedPayloadHandler) {
      const reader = new FileReader;
      reader.addEventListener("loadend", () => parsedPayloadHandler(JSON.parse(String.fromCharCode(...new Uint8Array(reader.result)))));
      reader.readAsArrayBuffer(blob)
    }
    static _parsePayloadResponse(payload, parsedPayloadHandler) {
      if (payload.xhr.response instanceof Blob) {
        _AddToCartListener.parseBlobToJson(payload.xhr.response, parsedPayloadHandler)
  ...
  (function() {
    ...
    function handleSubmitToCartAdd(event) {
      ...
      if (target && (target.getAttribute("action") || target.getAttribute("href"))) {
        try {
          ...
          properties.quantity = String(target.quantity ? target.quantity.value : 1);
          const eventProperties = extend({
            cartToken
          }, properties);
          const monorailProperties = extend({
            referer: window.location.href
          }, properties);
          window.ShopifyAnalytics.lib.track("Added Product", eventProperties, void 0, void 0, {
            addApiSource: "add-form",
            shopifyEmitted: true
          });
          window.ShopifyAnalytics.lib.track("monorail://trekkie_storefront_track_added_product/1.1", monorailProperties)
        } catch (error) {
          if (console && console.warn) {
            console.warn(`[shop_events_listener] Error in handleSubmitCartAdd: ${error.message}`)
          }
        }
      }
    }
    ...
    function handleSubmitToPaymentAdd(event) {
      event = event || window.event;
      const target = event.target || event.srcElement;
      if (target && target.getAttribute("action") && target.getAttribute("data-payment-form") !== null) {
        try {
          ...
          if (console && console.warn) {
            console.warn(`[shop_events_listener] Error in handleSubmitToPaymentAdd: ${error.message}`)
          }
        }
      }
    }
    ...
    function trackViewedProductVariant(target) {
      try {
        ...
        if (window.ShopifyAnalytics.meta.selectedVariantId && window.ShopifyAnalytics.meta.selectedVariantId == newVariantId) {
          return
        }
        window.ShopifyAnalytics.meta.selectedVariantId = newVariantId;
        const properties = trackProperties(newVariantId);
        window.ShopifyAnalytics.lib.track("Viewed Product Variant", properties, void 0, void 0, {
          shopifyEmitted: true
        })
      } catch (error) {
        if (console && console.warn) {
          console.warn(`[shop_events_listener] Error in trackViewedProductVariant: ${error.message}`)
        }
      }
    }
  ...
  (function(XHR) {
    ...
    XHR.prototype.send = function(data) {
      const addToCartListener = new AddToCartListener(this, this._url, this._method, data);
      if (this.addEventListener) {
        this.addEventListener("readystatechange", addToCartListener.onReadyStateChange.bind(addToCartListener), false)
      } else {
  ...
  (function(window2, fetch) {
    if (typeof fetch === "function") {
      window2.fetch = function() {
        return fetch.apply(this, Array.prototype.slice.call(arguments)).then(response => {
          if (!response.ok) {
            return response
          }
          const hyperlinkElement = document.createElement("a");
          hyperlinkElement.href = response.url;
          const path = hyperlinkElement.pathname ? hyperlinkElement.pathname : response.url;
          try {
    ...
    function handleFetchAddToCart(response, requestBody) {
      response.clone().json().then(responseJson => {
        if (Object.prototype.toString.call(responseJson) !== "[object Object]") {
          return
        }
        try {
        ...
        if (responseJson.items) {
          const parsedPayloadBodyItemsList = parseRequestBody(requestBody).items;
          const parsedResponseItemsList = responseJson.items;
          handleBulkItemCartAddResponse(parsedResponseItemsList, parsedPayloadBodyItemsList, "add-fetch-bulk");
          return
        }
    ...
    function consoleWarn(error) {
      if (console && console.warn) {
        console.warn(`[shop_events_listener] Error in handleFetchRequest:  ${error.message}`)
      }
    }
  })(window, window.fetch);

  function handleAddResponse(response, apiSource) {
    const itemsAdded = response && response.items_changelog && response.items_changelog.added;
    ...
    const itemsAddedWithPayload = itemsAdded.map(addedItem => {
      const matchedItem = itemsInCart.find(item => {
        return String(item.variant_id || item.id) === String(addedItem.variant_id)
      });
      if (matchedItem) {
        const result = {};
  ...
  function handleBulkItemCartAddResponse(parsedResponseItemsList, parsedPayloadBodyItemsList, addApiSource) {
    ...
    parsedResponseItemsList.forEach((item, idx) => {
      ...
      try {
        ...
        if (console && console.warn) {
          console.warn(`[shop_events_listener] Error in handleBulkItemCartAddResponse: ${error.message}`)
        }
      }
  ...
  function parseFormData(formData) {
    ...
    for (const entry of itemsMap.entries()) {
      ...
      if (existingItem) {
        existingItem.quantity = (parseInt(existingItem.quantity || "1", 10) + parseInt(item.quantity || "1", 10)).toString()
      } else {
  ...
  function handleItemAddedServerSideChangelogTest(item, quantity, addApiSource) {
    const cartToken = getCookie("cart");
    const properties = extend({
      variantId: String(item.id),
      productId: item.product_id,
    ...
    window.ShopifyAnalytics.lib.track("test_product_added_to_cart_server_changelog", eventProperties, void 0, void 0, {
      addApiSource,
      shopifyEmitted: true
    })
  }
